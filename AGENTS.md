# MindSpore Lite - Agent 使用说明

本文件用于约束与指导在本仓库中进行代码修改、构建、测试与交付的行为规范，适用于任何 AI coding agent 与人类开发者。请在开始任何改动前阅读并遵守。
...
任何 C++/Python/Shell/CMake 代码改动**必须**按下列顺序走完两步，不得跳过、不得简化、不得以"只改了注释/只改了一行"为由绕过：

1. **改动前** — 涉及云侧/端侧任一阶段时，先 `Read` 对应 SKILL.md（见下方速查表）再动手；跨侧通用代码（C++/Python/Shell/CMake）先 `Read` [clean-code-check SKILL.md](.claude/skills/common/clean-code-check/SKILL.md)。
2. **改动后、commit 前** — **必须**跑 `bash scripts/pre_commit/githooks/pre-push`（10 个 lint 全自动：clang-format/cmakelint/codespell/cpplint/pylint/shellcheck/tab/lizard/markdownlint/cppcheck）。工具缺失先 `cd scripts/pre_commit && bash install_tools.sh` 装齐。**禁止**在 lint 未过时 commit。
3. **commit 前 review** — **必须**按 [clean-code-check SKILL.md](.claude/skills/common/clean-code-check/SKILL.md) 的 30 秒快速清单 + 语义/安全章，对工作树相对 master 的所有改动做一次 PR 级 review。Critical 项未修完**不得** commit。
4. **commit message** — 遵守下方提交规范。

> 说明：第 2 步抓机械问题（格式/命名/拼写/CCN/NLOC/入参），第 3 步抓 lint 抓不到的语义问题（指针/边界/逻辑反转/错误码 vs 异常/注入/溢出/安全函数）。两者**不相互替代**——pre-push 过了不代表语义正确，review 过了不代表格式干净。
>
> Hook 安装：`git config core.hooksPath scripts/pre_commit/githooks`（同时启用 pre-commit、pre-push 和 commit-msg）。pre-push hook 本地未装时第 2 步仍**必须**手动跑，不得以此为由跳过。
...
通用（跨侧共享，**强制**）：[clean-code-check](.claude/skills/common/clean-code-check/SKILL.md) — C++/Python/Shell/CMake 代码质量与 CI 门禁。改动前 `Read` SKILL.md，改动后按其两步流程（pre-push + 30 秒清单）走完，详见上方[代码改动与提交流程](#代码改动与提交流程强制)。
...
> 完整端侧规范见 [.claude/device.md](.claude/device.md)，需要时再 `Read`。通用跨侧 skill（clean-code-check，**强制**）见上方云侧章节。
