...
1. Check the environment.
  ...
  Ensure that the Git tool, Python, and pip have been installed in the local environment. (The `python --version` command output indicates that the version is 3.9, 3.10, 3.11 or 3.12.)

2. Use scripts to install code check tools.

  Run the following command in the `mindspore-lite/` directory to perform automatic installation:
  ...
  The pip-installable tools (`cmakelint`, `codespell`, `cpplint`, `lizard`, `pylint`, `clang-format`) and the system tool (`shellcheck`) are installed by `install_tools.sh` in two phases. Optional Ruby gems (`chef-utils`, `mdl`) are installed when `INSTALL_GEMS=true` is set in the environment.

  **Note**:

- The `sudo` command is involved when `install_tools.sh` installs `shellcheck` via the system package manager (Linux). Ensure that you have the sudo permission. If sudo is unavailable, `shellcheck` is skipped with a warning and the other tools are still installed.
...
3. Using pre-push
    ...
    (2) Set the `hooks` path of Git to the directory where the `pre-push` file is located. The `pre-push` file path is `mindspore-lite/scripts/pre_commit/githooks/pre-push`. Run the following command in `mindspore-lite/`:
...
7. shellcheck
   ...
   (1) Install shellcheck via the system package manager.

   ```bash
   sudo apt-get install shellcheck      # Ubuntu/Debian
   sudo dnf install shellcheck          # Fedora
   sudo yum install shellcheck          # CentOS (requires EPEL)
   ```
...
Pinned versions are sourced from `scripts/pre_commit/versions.sh`. Bump a version there and `install_tools.sh` + `githooks/pre-push` pick it up automatically. The "Latest Version" column below tracks upstream at the time of writing and is editorial; the per-OS columns mirror `versions.sh`.
