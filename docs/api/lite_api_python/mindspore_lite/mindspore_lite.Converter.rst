    ...
    .. py:method:: set_config_info(section="", config_info=None)
        ...
        参数：
            - **section** (str，可选) - 配置参数的类别。配合 `config_info` 一起，设置confile的个别参数。例如：对于 `section` 是 ``"common_quant_param"`` ， `config_info` 是{"quant_type":"WEIGHT_QUANT"}。默认值： ``""`` 。
              ...
              有关训练后量化的配置参数，请参见 `量化 <https://www.mindspore.cn/lite/docs/zh-CN/master/advanced/quantization.html>`_ 。

              有关扩展的配置参数，请参见 `扩展配置 <https://www.mindspore.cn/lite/docs/zh-CN/master/advanced/third_party/converter_register.html#扩展配置>`_ 。
