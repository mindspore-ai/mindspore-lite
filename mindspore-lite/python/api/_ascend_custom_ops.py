...
one per SoC. On first ``import mindspore_lite`` (see ``__init__.py``) we detect
the host NPU via ``npu-smi`` and prepend the matching vendor folder to
``ASCEND_CUSTOM_OPP_PATH``. CANN discovers the op from there at convert *and*
execution time -- no files are copied to ``$ASCEND_HOME_PATH``, no extraction,
no stamp. Failures are logged but never raise, so they cannot break
``import mindspore_lite``.

Design notes (why this is conflict-safe vs. a copy-to-$ASCEND_HOME_PATH flow)
----------------------------------------------------------------------------
...
* **Prepend, never overwrite.** Our folder goes to the *front* of the list, so
  ...
  deterministic, and duplicate same-named vendors are tolerated (the first-listed
  wins).
...
def _log(message):
    sys.stderr.write("[mindspore_lite] " + message)
