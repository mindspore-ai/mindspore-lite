...
constexpr auto kNameFAFormatBNSD = "BNSD";
constexpr auto kNameFAFormatBSH = "BSH";
...
CNodePtr BSNDTOBSH(const FuncGraphPtr &func_graph, const CNodePtr &cnode_BSND, const std::vector<int64_t> &BSND_shape) {
  MS_CHECK_TRUE_RET(func_graph != nullptr, nullptr);
  MS_CHECK_TRUE_RET(cnode_BSND != nullptr, nullptr);
  int64_t auto_compute = -1;
  if (BSND_shape.size() != kNumShapeSize4) {
    MS_LOG(INFO) << "BSND_shape is not BSND! size is: " << BSND_shape.size();
    return nullptr;
  }
  std::vector<int64_t> BSH_shape = {BSND_shape[kNumIndex0], BSND_shape[kNumIndex1], auto_compute};
  auto shape_parm_node =
    BuildInt64VecParameterNode(func_graph, BSH_shape, cnode_BSND->fullname_with_scope() + "_input_shape_perm");
  MS_CHECK_TRUE_MSG(shape_parm_node != nullptr, nullptr, "create shape_parm_node return nullptr");
  std::vector<AnfNodePtr> op_inputs = {cnode_BSND, shape_parm_node};
  ...
  reshape_node->set_fullname_with_scope(cnode_BSND->fullname_with_scope() + "_reshape");
  reshape_node->set_abstract(cnode_BSND->abstract()->Clone());
  return reshape_node;
}
...
CNodePtr FlashAttentionFusion::CreateTikFlashAttentionCnodeForBNSD(const FuncGraphPtr &func_graph,
                                                                   const AnfNodePtr &node, const AnfNodePtr &q,
                                                                   const AnfNodePtr &k, const AnfNodePtr &v,
                                                                   float scale_value) const {
  auto q1 = CreateMulNode(func_graph, q, scale_value);

  MS_LOG(INFO) << "CreateTikFlashAttentionCnodeForBNSD";
  MS_LOG(INFO) << "scale value: " << scale_value;
  ...
  // create op
  auto tik_flash_attention_prim = std::make_unique<ops::Custom>();
  if (tik_flash_attention_prim == nullptr) {
    MS_LOG(ERROR) << "new prompt flash attention prim failed.";
    return nullptr;
  }
  // add attr
  std::vector<std::string> input_name = {"q", "k", "v"};
  std::vector<std::string> output_name = {"y"};
  tik_flash_attention_prim->AddAttr("input_names", api::MakeValue(input_name));
  tik_flash_attention_prim->AddAttr("output_names", api::MakeValue(output_name));
  tik_flash_attention_prim->set_type("FlashAttentionTik");
  tik_flash_attention_prim->AddAttr("reg_op_name", api::MakeValue("FlashAttentionTik"));

  auto fa_prim_c = tik_flash_attention_prim->GetPrim();
  ...
  tik_flash_attention_cnode->set_fullname_with_scope(node->fullname_with_scope() + "_tikfa");
  if (node->abstract() != nullptr) {
    tik_flash_attention_cnode->set_abstract(node->abstract()->Clone());
  }
  MS_LOG(INFO) << "create TikFlashAttention success.";
  return tik_flash_attention_cnode;
}

CNodePtr FlashAttentionFusion::CreateFlashAttentionCnodeForBNSD(
  ...
  if (fa_parm->implement_mode == "tik" && atten_mask == nullptr) {
    return CreateTikFlashAttentionCnodeForBNSD(func_graph, node, q, k, v, scale_value);
  }

  return CreatePromptFlashAttentionCnodeForBNSD(func_graph, node, q, k, v, atten_mask, num_heads, next_token,
                                                scale_value, fa_parm, num_key_value_heads);
}
...
CNodePtr FlashAttentionFusion::PreMulBSH(const FuncGraphPtr &func_graph, const AnfNodePtr &node,
                                         const CNodePtr &q_trans_BNSD, const CNodePtr &k_trans_BNDS,
                                         const CNodePtr &v_trans_BNSD, const AnfNodePtr &atten_mask, int64_t num_head,
                                         int64_t next_tokens, float scale_value,
  ...
  auto v_reshape_BSND = v_trans_BNSD->input(kNumIndex1)->cast<CNodePtr>();
  MS_CHECK_TRUE_RET(v_reshape_BSND != nullptr, nullptr);
  auto q_BSND_shape = GetTensorShape(q_reshape_BSND, kNumIndex1);
  auto k_BSND_shape = GetTensorShape(k_reshape_BSND, kNumIndex1);
  auto v_BSND_shape = GetTensorShape(v_reshape_BSND, kNumIndex1);
  auto q_BSH = BSNDTOBSH(func_graph, q_reshape_BSND, q_BSND_shape);
  MS_CHECK_TRUE_RET(q_BSH != nullptr, nullptr);
  auto k_BSH = BSNDTOBSH(func_graph, k_reshape_BSND, k_BSND_shape);
  MS_CHECK_TRUE_RET(k_BSH != nullptr, nullptr);
  auto v_BSH = BSNDTOBSH(func_graph, v_reshape_BSND, v_BSND_shape);
  MS_CHECK_TRUE_RET(v_BSH != nullptr, nullptr);
  auto fa_node = CreatePromptFlashAttentionCnodeForBSH(func_graph, node, q_BSH, k_BSH, v_BSH, atten_mask, num_head,
                                                       next_tokens, scale_value, fa_parm);
  (void)manager->Replace(output_reshape, fa_node);
  MS_LOG(ERROR) << "Create prompt BSH PFA success for pre-mul pattern.";
  return nullptr;
}

CNodePtr FlashAttentionFusion::PreMulInBNSDOutBSND(const FuncGraphPtr &func_graph, const AnfNodePtr &node,
                                                   const CNodePtr &q_trans_BNSD, const CNodePtr &k_trans_BNDS,
                                                   const CNodePtr &v_trans_BNSD, const AnfNodePtr &atten_mask,
  ...
  auto fa_node = CreatePromptFlashAttentionCnodeForBNSDBSND(func_graph, node, q_trans_BNSD, k_trans_BNDS, v_trans_BNSD,
                                                            atten_mask, num_head, next_tokens, scale_value, fa_parm);
  (void)manager->Replace(output_trans, fa_node);
  MS_LOG(INFO) << "Create prompt BNSDBSND PFA success for pre-mul pattern.";
  return nullptr;
}
...
CNodePtr FlashAttentionFusion::CreatePromptFlashAttentionCnodeForLayout(
  const FuncGraphPtr &func_graph, const AnfNodePtr &node, const AnfNodePtr &q, const AnfNodePtr &k, const AnfNodePtr &v,
  const AnfNodePtr &atten_mask, int64_t num_heads, int64_t next_token, float scale_value,
  const std::shared_ptr<FlashAttentionParm> &fa_parm, int64_t layout_mode, const std::string &layout_name,
  const std::string &cnode_suffix) const {
  MS_LOG(INFO) << "CreatePromptFlashAttentionCnodeForLayout: " << layout_name;
  ...
  // add attr
  prompt_flash_attention_prim->AddAttr("num_heads", api::MakeValue(num_heads));
  prompt_flash_attention_prim->AddAttr("input_layout",
                                       api::MakeValue(static_cast<mindspore::ops::FASInputLayoutMode>(layout_mode)));
  prompt_flash_attention_prim->AddAttr("next_tokens", api::MakeValue(next_token));
  prompt_flash_attention_prim->AddAttr("scale_value", api::MakeValue(scale_value));
  ...
  MS_LOG(INFO) << "num heads: " << num_heads << ", input layout: " << layout_name << ", next tokens: " << next_token
  ...
  prompt_flash_attention_cnode->set_fullname_with_scope(node->fullname_with_scope() + cnode_suffix);
  if (node->abstract() != nullptr) {
    prompt_flash_attention_cnode->set_abstract(node->abstract()->Clone());
  }
  MS_LOG(INFO) << "create PromptFlashAttention success.";
...
CNodePtr FlashAttentionFusion::CreatePromptFlashAttentionCnodeForBSH(
  ...
  const AnfNodePtr &atten_mask, int64_t num_heads, int64_t next_token, float scale_value,
  const std::shared_ptr<FlashAttentionParm> &fa_parm) const {
  return CreatePromptFlashAttentionCnodeForLayout(func_graph, node, q, k, v, atten_mask, num_heads, next_token,
                                                  scale_value, fa_parm, mindspore::ops::FASInputLayoutMode::BSH, "BSH",
                                                  "_prompt_flash_attention_bsh");
}

CNodePtr FlashAttentionFusion::CreatePromptFlashAttentionCnodeForBNSDBSND(
  const FuncGraphPtr &func_graph, const AnfNodePtr &node, const AnfNodePtr &q, const AnfNodePtr &k, const AnfNodePtr &v,
  const AnfNodePtr &atten_mask, int64_t num_heads, int64_t next_token, float scale_value,
  const std::shared_ptr<FlashAttentionParm> &fa_parm) const {
  return CreatePromptFlashAttentionCnodeForLayout(func_graph, node, q, k, v, atten_mask, num_heads, next_token,
                                                  scale_value, fa_parm, mindspore::ops::FASInputLayoutMode::BNSD_BSND,
                                                  "BNSD_BSND", "_prompt_flash_attention_bsh");
}

CNodePtr FlashAttentionFusion::CreateFAForBNSDFromInputs(const FuncGraphPtr &func_graph, const AnfNodePtr &node,
                                                         const AnfNodePtr &q, const AnfNodePtr &k, const AnfNodePtr &v,
                                                         const CNodePtr &atten_mask,
                                                         const std::vector<int64_t> &q_shape,
                                                         const std::vector<int64_t> &k_shape,
                                                         const std::shared_ptr<FlashAttentionParm> &fa_parm) const {
  // check input shape
  if (q_shape[kNumIndex3] <= 0 || q_shape[kNumIndex1] <= 0) {
    MS_LOG(ERROR) << "D is -1";
    return nullptr;
  }
  float scale_value = 1 / (pow(q_shape[kNumIndex3], kNumPowerHalf));
  int64_t seq_len = q_shape[kNumIndex2];
  int64_t num_key_value_heads = k_shape[kNumIndex1];
  if (seq_len != 1) {
    return CreateFlashAttentionCnodeForBNSD(func_graph, node, q, k, v, atten_mask, q_shape[kNumIndex1], 0, scale_value,
                                            fa_parm, num_key_value_heads);
  } else {
    MS_LOG(INFO) << "seq len is 1, incre flash attention.";
    return CreateIncreFlashAttentionCnodeForBNSD(func_graph, node, q, k, v, atten_mask, q_shape[kNumIndex1],
...
CNodePtr FlashAttentionFusion::CreateFAForBNSDWithAttenMask(const FuncGraphPtr &func_graph, const AnfNodePtr &node,
  ...
  return CreateFAForBNSDFromInputs(func_graph, node, q, k, v, atten_mask, input_tensor_q_shape, input_tensor_k_shape,
                                   fa_parm);
}
...
CNodePtr FlashAttentionFusion::CreateFAForSD15(const FuncGraphPtr &func_graph, const AnfNodePtr &node,
  ...
  auto fa_node = CreateFlashAttentionCnodeForBNSD(func_graph, node, q_pad_node, k_pad_node, v_pad_node, nullptr,
...
CNodePtr FlashAttentionFusion::CreateFlashAttentionNodeForMsSDXL(
  ...
  auto fa_node = CreateFlashAttentionCnodeForBNSD(func_graph, node, q, k, v, nullptr, num_head, next_tokens,
...
CNodePtr FlashAttentionFusion::CreateFlashAttentionNodeForMsSD21(
  ...
  auto fa_node = CreateFlashAttentionCnodeForBNSD(func_graph, node, q_trans, k_trans, v_trans, nullptr, num_head,
...
CNodePtr FlashAttentionFusion::CreateSDFlashAttentionNode(const FuncGraphPtr &func_graph, const AnfNodePtr &node,
                                                          const AnfNodePtr &q_trans, const AnfNodePtr &k_trans,
                                                          const AnfNodePtr &v_trans, const AnfNodePtr &mul_const_input,
                                                          const std::vector<int64_t> &q_shape,
                                                          const std::vector<int64_t> &k_shape,
  ...
  if (q_shape.size() != kNumShapeSize4) {
    ...
    // for static shape: get scale value
    scale_value = 1 / (pow(q_shape[kNumIndex3], kNumPowerHalf));
    num_head = q_shape[kNumIndex1];
    d_value = q_shape[kNumIndex3];
  } else {
    MS_LOG(WARNING) << "need check Q input tensor shape: " << q_shape;
    return nullptr;
  }
  if (!PFACheckShape(scale_value, q_shape, k_shape, v_shape, fa_parm->seq_threshold)) {
    return nullptr;
  }
  if (d_value == kNumDValue) {
    return CreateFAForSD15(func_graph, node, q_trans, k_trans, v_trans, num_head, next_tokens, scale_value, fa_parm);
  } else {
    return CreateFlashAttentionCnodeForBNSD(func_graph, node, q_trans, k_trans, v_trans, nullptr, num_head, next_tokens,
                                            scale_value, fa_parm, num_head);
  }
}
...
CNodePtr FlashAttentionFusion::CreateFlashAttentionNodeForVideoComposer(
  ...
  auto fa_node = CreateSDFlashAttentionNode(func_graph, node, q_trans, k_trans, v_trans, mul_const_input,
                                            input_tensor_q_shape, input_tensor_k_shape, input_tensor_v_shape, fa_parm);
  if (fa_node == nullptr) {
    return nullptr;
  }
  auto manager = Manage(func_graph);
...
CNodePtr FlashAttentionFusion::CreateFlashAttentionNodeForSDPreMul(
  ...
  if (d_value == kNumDValue) {
    fa_node = CreateFAForSD15(func_graph, node, q_trans_BNSD, k_trans_BNDS, v_trans_BNSD, num_head, next_tokens,
                              scale_value, fa_parm);
  } else if (fa_parm->input_layout == kNameFAFormatBSH) {
    return PreMulBSH(func_graph, node, q_trans_BNSD, k_trans_BNDS, v_trans_BNSD, nullptr, num_head, next_tokens,
                     scale_value, fa_parm);
  } else if (fa_parm->input_layout == kNameFAFormatBNSDBSND) {
    return PreMulInBNSDOutBSND(func_graph, node, q_trans_BNSD, k_trans_BNDS, v_trans_BNSD, nullptr, num_head,
                               next_tokens, scale_value, fa_parm);
  } else {
    fa_node = CreateFlashAttentionCnodeForBNSD(func_graph, node, q_trans_BNSD, k_trans_BNDS, v_trans_BNSD, nullptr,
                                               num_head, next_tokens, scale_value, fa_parm, num_head);
  }
  MS_CHECK_TRUE_RET(fa_node != nullptr, nullptr);
  std::vector<int32_t> new_perm = {kNumIndex0, kNumIndex2, kNumIndex1, kNumIndex3};
...
CNodePtr FlashAttentionFusion::CreateFlashAttentionNodeForSDWithoutCast(
  const std::string &pattern_name, const FuncGraphPtr &func_graph, const AnfNodePtr &node, const EquivPtr &equiv,
  const std::shared_ptr<FlashAttentionParm> &fa_parm) const {
  MS_LOG(INFO) << "input_layout: " << fa_parm->input_layout << ", seq_threshold: " << fa_parm->seq_threshold
  ...
  if (d_value == kNumDValue) {
    ...
    fa_node = CreateFlashAttentionCnodeForBNSD(func_graph, node, q_trans, k_trans, v_trans, nullptr, num_head,
                                               next_tokens, scale_value, fa_parm, num_head);
  }
  MS_CHECK_TRUE_RET(fa_node != nullptr, nullptr);
  auto manager = Manage(func_graph);
...
CNodePtr FlashAttentionFusion::CreateFlashAttentionNodeForPanGu(
  ...
  if (seq_len != 1) {
    return CreateFlashAttentionCnodeForBNSD(func_graph, node, q, k, v, atten_mask, input_tensor_q_shape[kNumIndex1], 0,
                                            scale_value, fa_parm, input_tensor_k_shape[kNumIndex1]);
  } else {
    MS_LOG(INFO) << "seq len is 1, incre flash attention.";
...
bool checkBinaryInputValidInt(int checkValue, int v1, int v2) {
