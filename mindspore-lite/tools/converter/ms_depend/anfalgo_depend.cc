...
std::vector<KernelWithIndex> GetAllOutputWithIndexInner(const AnfNodePtr &node,
  ...
  if (std::any_of(return_types.begin(), return_types.end(), [&node](const PrimitivePtr &prim_type) -> bool {
        return lite::common::AnfAlgo::CheckPrimitiveType(node, prim_type);
      })) {
  ...
  if (AnfUtils::IsRealCNodeKernel(node)) {
    if (node->abstract() != nullptr &&
        (lite::common::AnfAlgo::IsDynamicSequence(node) || IsMultiLayerTuple(node->abstract()))) {
      outputs_num = lite::common::AnfAlgo::GetOutputNumByAbstract(node->abstract());
    } else {
  ...
  //    return D'''
  if (lite::common::AnfAlgo::IsCallNode(node) || (!AnfUtils::IsRealCNodeKernel(node))) {
...
KernelWithIndex VisitKernelWithReturnTypeForTupleGetItem(const AnfNodePtr &anf_node, size_t index, bool skip_nop_node,
  ...
  if (!lite::common::AnfAlgo::CheckPrimitiveType(anf_node, prim::kPrimTupleGetItem)) {
  ...
  auto item_with_index_tmp = lite::common::AnfAlgo::VisitKernelWithReturnType(
    lite::common::AnfAlgo::GetTupleGetItemRealInput(cnode), lite::common::AnfAlgo::GetTupleGetItemOutIndex(cnode),
    skip_nop_node, return_types, &abs, true);
  if (IsOneOfPrimitiveCNode(item_with_index_tmp.first, expand_prims)) {
    ...
    return lite::common::AnfAlgo::VisitKernelWithReturnType(make_tuple_inputs[make_tuple_input_index], index,
                                                            skip_nop_node, return_types);
  }
  if (lite::common::AnfAlgo::IsCallNode(item_with_index_tmp.first) || item_with_index_tmp.first->isa<Parameter>() ||
    ...
    if (abs->isa<abstract::AbstractSequence>()) {
      ...
      if (sub_abstracts.size() <= lite::common::AnfAlgo::GetTupleGetItemOutIndex(cnode)) {
        MS_LOG(INTERNAL_EXCEPTION) << "Invalid index:" << lite::common::AnfAlgo::GetTupleGetItemOutIndex(cnode)
                                   << " for abstract:" << abs->ToString();
      }
      for (size_t i = 0; i < lite::common::AnfAlgo::GetTupleGetItemOutIndex(cnode); ++i) {
        MS_EXCEPTION_IF_NULL(sub_abstracts[i]);
        real_index += AnfAlgo::GetOutputNumByAbstract(sub_abstracts[i]);
      }
      if (abstract != nullptr) {
        (*abstract) = sub_abstracts[lite::common::AnfAlgo::GetTupleGetItemOutIndex(cnode)];
        MS_EXCEPTION_IF_NULL((*abstract));
      } else {
...
KernelWithIndex AnfAlgo::FetchRealNodeSkipMonadControl(const KernelWithIndex &node_with_index) {
  ...
  if (IsOneOfPrimitiveCNode(node_with_index.first, auto_monad_prims)) {
    return lite::common::AnfAlgo::VisitKernelWithReturnType(node_with_index.first, node_with_index.second, false);
  } else {
...
std::vector<KernelWithIndex> AnfAlgo::GetAllOutputWithOutMonadAndParameter(const AnfNodePtr &node) {
  MS_EXCEPTION_IF_NULL(node);
  const auto &graph_outputs = lite::common::AnfAlgo::GetAllOutputWithIndex(node);
...
void AnfAlgo::SetNodeAttrSafely(const std::string &key, const ValuePtr &value, const AnfNodePtr &node) {
  ...
  // Set attr secondly.
  lite::common::AnfAlgo::SetNodeAttr(key, value, node);
}
...
TypeId AnfAlgo::GetOutputInferDataType(const AnfNodePtr &node, size_t output_idx) {
  ...
  if (IsCallNode(node) || IsTupleInTupleValueNode(node)) {
    ...
    const auto &abs = lite::common::AnfAlgo::FetchAbstractByIndex(node->abstract(), output_idx);
    MS_EXCEPTION_IF_NULL(abs);
...
bool AnfAlgo::IsOutputAnchorDynamicRank(const AnfNodePtr &node, size_t idx) {
  MS_EXCEPTION_IF_NULL(node);
  const auto &out_shape = lite::common::AnfAlgo::GetOutputInferShape(node, idx);
...
bool AnfAlgo::IsDynamicShapeFuncGraph(const FuncGraphPtr &func_graph) {
  ...
  return std::any_of(nodes.begin(), nodes.end(), [](const AnfNodePtr &node) {
    if (node == nullptr || lite::common::AnfAlgo::IsCallNode(node) || IsPrimitiveCNode(node, prim::kPrimReturn)) {
      return false;
    }
    return lite::common::AnfAlgo::IsDynamic(node);
  });
}

bool AnfAlgo::IsDynamic(const AnfNodePtr &node) {
  return lite::common::AnfAlgo::IsDynamicShape(node) || lite::common::AnfAlgo::IsDynamicSequence(node) ||
         lite::common::AnfAlgo::IsNodeMutableScalar(node);
}
...
std::string AnfAlgo::GetMoveToDstStr(const AnfNodePtr &node) {
  ...
  const auto &kernel_with_index =
    lite::common::AnfAlgo::VisitKernelWithReturnType(cnode->input(dst_input_idx), 0, true);
...
bool AnfAlgo::IsNoneInput(const AnfNodePtr &node, size_t index) {
  MS_EXCEPTION_IF_NULL(node);
  auto kernel_with_index = lite::common::AnfAlgo::GetPrevNodeOutput(node, index);
...
bool AnfAlgo::HasTupleInput(const CNodePtr &node) {
  ...
  for (size_t i = 0; i < input_num; ++i) {
    auto input_node = lite::common::AnfAlgo::GetInputNode(node, i);
    MS_EXCEPTION_IF_NULL(input_node);
    if (lite::common::AnfAlgo::IsTupleOutput(input_node)) {
      return true;
    }
  }
  return false;
...
bool AnfAlgo::HasDynamicTupleInput(const CNodePtr &node) {
  ...
  for (size_t i = 0; i < input_num; ++i) {
    auto input_node = lite::common::AnfAlgo::GetInputNode(node, i);
    MS_EXCEPTION_IF_NULL(input_node);
    if (lite::common::AnfAlgo::IsDynamicSequence(input_node)) {
      return true;
    }
  }
  return false;
...
abstract::AbstractBasePtr AnfAlgo::FetchAbstractByIndex(const AbstractBasePtr &abstract, size_t index) {
  ...
  for (const auto &sub_abstract : sub_abstracts) {
    size_t tmp_index = lite::common::AnfAlgo::GetOutputNumByAbstract(sub_abstract);
...
bool SupportInlinePartial(const AnfNodePtr &input0) {
  ...
  auto idx = lite::common::AnfAlgo::GetTupleGetItemOutIndex(tuple_get_node);
  ...
  size_t tuple_input_num = lite::common::AnfAlgo::GetInputTensorNum(graph_out);
