...
/**
 * @brief Common custom inference interface implementation
 *
 * Extracts common logic from DSP and ACL adapters to reduce code duplication.
 *
 * Usage: Compose this class in DSP/ACL CustomInterface classes and call the corresponding methods.
 */
class CustomInferImpl {
 public:
  /**
   * @brief Infer output tensor shapes from custom operator attributes
   * @param inputs Input tensor list
   * @param outputs Output tensor list (will be modified)
   * @param primitive Primitive operator pointer
   * @return Status code
   */
  static Status Infer(std::vector<mindspore::MSTensor> *inputs, std::vector<mindspore::MSTensor> *outputs,
  ...
  /**
   * @brief Get specified attribute from custom operator
   * @param op Custom operator pointer
   * @param attr_name Attribute name
   * @param buf Output buffer
   * @return Status code
   */
  static Status GetCustomAttr(const mindspore::schema::Custom *op, const std::string &attr_name,
                              std::vector<char> *buf) {
  ...
  /**
   * @brief Create CustomInfer interface instance
   * @return Shared pointer to custom inference interface
   */
  template <typename T>
  static std::shared_ptr<mindspore::kernel::KernelInterface> CreateInfer() {
    auto infer = new (std::nothrow) T();
    if (infer == nullptr) {
...
#endif  // MINDSPORE_LITE_TOOLS_CONVERTER_ADAPTER_COMMON_CUSTOM_INFER_IMPL_H_
