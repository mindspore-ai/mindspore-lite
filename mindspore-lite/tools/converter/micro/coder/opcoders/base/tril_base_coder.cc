 ...
 */
#include "coder/opcoders/base/tril_base_coder.h"
...
int TrilBaseCoder::DoCode(CoderContext *const context) {
  ...
  if (input_tensor_->data_type() == kNumberTypeInt8) {
    ...
    code.CodeFunction("TrilInt8", input_tensor_, height, width, diagonal, output_tensor_, num, in_scale, in_zp,
                      out_scale, out_zp);
  } else if (input_tensor_->data_type() == kNumberTypeFloat32) {
    Collect(context, {"nnacl_c/fp32/triu_tril_fp32.h"}, {"triu_tril_fp32.c"});
    code.CodeFunction("TrilByte4", input_tensor_, output_tensor_, diagonal, height, width, num);
  } else {
    MS_LOG(ERROR) << "Tril only support FP32/INT8. ";
    return RET_ERROR;
  }

  MS_LOG(DEBUG) << "TrilBaseCoder invoked";
  context->AppendCode(code.str());
  return lite::RET_OK;
}

REG_OPERATOR_CODER(kAllTargets, kNumberTypeFloat32, PrimitiveType_Tril, CPUOpCoderCreator<TrilBaseCoder>)
// INT8: Tril is a pure data-movement op; int8 data is handled in place by TrilInt8
// (masked elements are written as the output zero point). This is genuine int8, not an fp32 fallback.
REG_OPERATOR_CODER(kAllTargets, kNumberTypeInt8, PrimitiveType_Tril, CPUOpCoderCreator<TrilBaseCoder>)
}  // namespace mindspore::lite::micro::nnacl
