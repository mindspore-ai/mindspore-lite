...
void LogTiling(const ShapeDims &dims, uint32_t chunkSize, uint32_t numChunks, float scaleValue, int64_t ubSize,
    ...
    fprintf(stderr, "ChunkGatedDeltaRule tiling: T=%u, chunkSize=%u, numChunks=%u, hqk=%u, dk=%u, hv=%u, dv=%u, "
                    "scaleValue=%f, ubSize=%ld, vStep=%u, blockDim=%u, outQueue=%u, tbuf=%u, restBytes=%u\n",
            dims.t, chunkSize, numChunks, dims.hqk, dims.dk, dims.hv, dims.dv, scaleValue, ubSize, vStepVal,
            blockDim, outQueueMax, tbufTotal, restBytes);
    fflush(stderr);
}
...
static uint32_t ChunkGatedDeltaRuleTilingFunc(TilingContext *context) {
    ...
    LogTiling(dims, chunkSize, numChunks, scaleValueAttr, ubSize, vStepVal,
              blockDim, outQueueMax, tbufTotal, restBytes);
...
IMPL_OP_OPTILING(ChunkGatedDeltaRule)
    .Tiling(ChunkGatedDeltaRuleTilingFunc, sizeof(ChunkGatedDeltaRuleTilingData));
