...
extern "C" __global__ __aicore__ void recurrent_gated_delta_rule(GM_ADDR query, GM_ADDR key, GM_ADDR value,
  ...
  RecurrentGatedDeltaRule::RGDRInitParams initParams{
    query, key, value, g, gk, beta, state, cuSeqlens, ssmStateIndices, numAcceptedTokens, out, stateOut};
  if (TILING_KEY_IS(0)) {
    TPipe pipe;
    RecurrentGatedDeltaRule::RGDR<half, half> op(&tilingData);
