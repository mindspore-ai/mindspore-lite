...
/*!
 * \file inner_prompt_flash_attention_infershape.cpp
 * \brief
 */
...
namespace ops {
static ge::graphStatus InferShapeInnerPromptFlashAttention(gert::InferShapeContext *context) {
  if (context == nullptr) {
    OP_LOGE("InnerPromptFlashAttention", "Context for inferring shape is nullptr!");
    return ge::GRAPH_FAILED;
  }
  OP_LOGD(context->GetNodeName(), "Enter InnerPromptFlashAttention inferShape impl.");
  // query shape : (B, S, H)
  ...
  *attentionOutShape = *queryShape;
  ...
  if (strcmp(inputLayoutPtr, "BNSD_BSND") == 0) {
    ...
    if (queryShape->GetDimNum() != PFA_LAYOUT_TND_DIMS || valueShape->GetDimNum() != PFA_LAYOUT_TND_DIMS) {
      OP_LOGE(context->GetNodeName(), "Layout %s, queryDims(%zu) valueDims(%zu) must be 3!", inputLayoutPtr,
    ...
    if (queryShape->GetDimNum() != PFA_LAYOUT_NTD_DIMS || valueShape->GetDimNum() != PFA_LAYOUT_NTD_DIMS) {
      OP_LOGE(context->GetNodeName(), "Layout NTD, queryDims(%zu) valueDims(%zu) must be 3!", queryShape->GetDimNum(),
  ...
  OP_LOGD(context->GetNodeName(), "InnerPromptFlashAttention inferShape end.");
  return ge::GRAPH_SUCCESS;
}

static ge::graphStatus InferDataTypeInnerPromptFlashAttention(gert::InferDataTypeContext *context) {
  if (context == nullptr) {
    OP_LOGE("InnerPromptFlashAttention", "Context for inferring data type is nullptr!");
    return ge::GRAPH_FAILED;
  }
  OP_LOGD(context->GetNodeName(), "Enter InnerPromptFlashAttention inferDataType impl.");
  ...
  OP_LOGD(context->GetNodeName(), "InnerPromptFlashAttention inferDataType end.");
  return GRAPH_SUCCESS;
}

IMPL_OP_INFERSHAPE(InnerPromptFlashAttention)
  .InferShape(InferShapeInnerPromptFlashAttention)
  .InferDataType(InferDataTypeInnerPromptFlashAttention)
