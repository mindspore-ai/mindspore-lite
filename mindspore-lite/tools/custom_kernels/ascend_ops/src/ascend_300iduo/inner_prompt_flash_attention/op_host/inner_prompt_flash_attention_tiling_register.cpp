...
/*!
 * \file inner_prompt_flash_attention_tiling_register.cpp
 * \brief
 */
...
static ge::graphStatus TilingPrepareForInnerPromptFlashAttention(gert::TilingParseContext *context) {
  auto platformInfoPtr = context->GetPlatformInfo();
  OP_CHECK_IF(platformInfoPtr == nullptr,
              OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "platformInfoPtr is null!"), return ge::GRAPH_FAILED);
  auto compileInfoPtr = context->GetCompiledInfo<InnerPromptFlashAttentionCompileInfo>();
...
IMPL_OP_OPTILING(InnerPromptFlashAttention)
  .TilingInputsDataDependency({ACTUAL_SEQ_Q_INDEX_PFA, ACTUAL_SEQ_KV_INDEX_PFA})
  .Tiling(TilingInnerPromptFlashAttention)
  .TilingParse<InnerPromptFlashAttentionCompileInfo>(
    TilingPrepareForInnerPromptFlashAttention);  // Register entrance functions to the framework
}  // namespace optiling
