...
#include <string>
#include <memory>
#include "fia_tiling_base.h"  // NOLINT(build/include_subdir)
...
class FiaTilingCases {
 public:
  ...
  const std::map<int32_t, FiaTilingClassCase> &GetTilingCases() { return cases_; }
 ...
 public:
  ...
  const std::map<int32_t, FiaTilingClassCase> &GetTilingTemplates(const std::string &op_type, int32_t soc_version) {
    ...
    OP_CHECK_IF(
      ...
      OPS_REPORT_VECTOR_INNER_ERR(op_type, "Get op tiling func failed, please check the soc version %d", soc_version),
