...
#include "kernel_tensor.h"  // NOLINT(build/include_subdir)
...
__aicore__ inline void BinaryComputeWithSpecialMaskPFA(
  ...
  if (tail != 0) {
    ...
    func(dst[repeat * FLOAT_REPEAT_SIZE], src0[repeat * FLOAT_REPEAT_SIZE], src1[repeat * FLOAT_REPEAT_SIZE], tailMask,
...
__aicore__ inline void ExpFast(const LocalTensor<T> &dst, const LocalTensor<T> &src, const uint8_t repeatTimes,
  ...
  if constexpr (1) {
    ...
    PipeBarrier<PIPE_V>();
    Mul<T, false>(
      dst, dst, dst, MASK_PLACEHOLDER, repeatTimes,
      {(uint8_t)repeatParams.dstBlkStride, (uint8_t)repeatParams.srcBlkStride, (uint8_t)repeatParams.srcBlkStride,
       (uint8_t)repeatParams.dstRepStride, (uint8_t)repeatParams.srcRepStride, (uint8_t)repeatParams.srcRepStride});
    PipeBarrier<PIPE_V>();
...
__aicore__ inline void FlashV2NZUpdateGenericImplPFA(const LocalTensor<half> &dst, const LocalTensor<half> &sumTensor,
  ...
  PipeBarrier<PIPE_V>();
  for (uint32_t j = 0; j < splitNZBlockCount; j++) {
    Sub<half, false>(dst[offset1 + splitOffset * j], src[offset1 + splitOffset * j], halfReduceBuffer, MASK_PLACEHOLDER,
                     1, {1, 1, 1, DEFAULT_REPEAT_STRIDE, DEFAULT_REPEAT_STRIDE, DEFAULT_REPEAT_STRIDE});
  }
  SetMaskNorm();
  ResetMask();
...
__aicore__ inline void SoftmaxFlashV2NZNoUpdateImplPFA(const LocalTensor<T> &dstTensor, const LocalTensor<T> &sumTensor,
  ...
  for (uint32_t i = 0; i < tiling.rangeM; i++) {
    ...
    SoftMaxGenericNZImplPFA<true>(dstTensor, sumTensor, maxTensor, srcTensor, workLocal, tiling, mask, offset1, offset2,
  ...
  if constexpr (isDataFormatNZ) {
    if (unlikely(srcNDinfo.k != tiling.srcK || srcNDinfo.m != tiling.srcM)) {
      ...
      SoftMaxFlashV2NZImplPFA<T, isUpdate, isBasicBlock>(dstTensor, expSumTensor, maxTensor, srcTensor, expMaxTensor,
                                                         inExpSumTensor, inMaxTensor, tempBuffer, originalSrcShape,
                                                         newTiling);
    } else {
      SoftMaxFlashV2NZImplPFA<T, isUpdate, isBasicBlock>(dstTensor, expSumTensor, maxTensor, srcTensor, expMaxTensor,
                                                         inExpSumTensor, inMaxTensor, tempBuffer, originalSrcShape,
