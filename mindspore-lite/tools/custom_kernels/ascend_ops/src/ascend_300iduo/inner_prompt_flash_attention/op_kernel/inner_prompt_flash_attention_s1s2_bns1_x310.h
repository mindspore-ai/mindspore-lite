 ...
 * \brief
 */
#ifndef INNER_PROMPT_FLASH_ATTENTION_S1S2_BNS1_X310_H
#define INNER_PROMPT_FLASH_ATTENTION_S1S2_BNS1_X310_H
...
class InnerPromptFlashAttentionS1s2Bns1X310 : public InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT> {
 public:
  ...
  __aicore__ inline InnerPromptFlashAttentionS1s2Bns1X310() {}
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310<PFAT>::Process() {
  ComputeEachCore(this->tmp_block_idx);
}

template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310<PFAT>::SInnerLoopFunc(int32_t startIndex,
                                                                                   int32_t endIndex) {
  if (startIndex < 0) {
    startIndex = 0;
  }
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310<PFAT>::ComputeSoftmax(LocalTensor<mm1OutputType> &mmResUb,
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310<PFAT>::ComputeEachCoreSInnerLoop(uint32_t startIndex,
                                                                                              uint32_t endIndex) {
  ...
  for (int64_t sInnerLoopIdx = startIndex; sInnerLoopIdx < endIndex; sInnerLoopIdx++) {
    ...
    if (this->isInnerLoopLast_ && !this->isOuterLoopLast_) {
      ...
      this->CopyND2NZOnTheFly(this->a1Local_, this->queryGm[this->tensorAOffset], outerSize,
                              this->tilingData->promptAttentionBaseParams.headSize, this->queryStride, true);
      this->tensorBOffset = this->tensorBCoreOffset + startIndex * this->keyValueStride * this->singleProcessSInnerSize;
    }
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310<PFAT>::ComputeEachCore(uint32_t coreIdx) {
  ...
  for (uint32_t loopNIdx = nLoopStart; loopNIdx < nLoopEnd; loopNIdx++) {
    ...
    for (int sIdx = sIdStart; sIdx < sLoopEndIdx; sIdx++) {
      ...
      int32_t queryHeadEndOffset =
        this->multiSeqOffset + (loopNIdx + 1) * this->tilingData->promptAttentionBaseParams.seqSize * this->queryStride;
      ...
      for (uint32_t sOuterLoopIdx = outerLoopStart; sOuterLoopIdx < tmpOuterLoopEnd; sOuterLoopIdx++) {
        ...
        // start outer loop calculate in one core
        ...
        // [S1!=S2 change ②] The band mapping below divides a Q-row position
        // (S1 domain) by the S2 block length to derive a KV column index. That
        // implicitly assumes "Q row r aligns to KV col r" (true only when S1==S2).
        // With S1<S2 and nextTokens==0 it would compute end_idx < maxInnerLoopTimes
        // and SILENTLY skip KV tail blocks (wrong result, no error). Under the
        // full-attention window (preTokens>=S1 and nextTokens==0 or >=S2) the loop
        // must cover every KV block, so short-circuit to the full range and keep the
        // band arithmetic only for the genuine sparse/window case.
        int32_t start_idx;
        int32_t end_idx;
        bool fullAttenWindow =
          (preTokens >= (int32_t)actualSeqLengthsIdx) &&
          (nextTokens == 0 || nextTokens >= (int32_t)this->tilingData->promptAttentionBaseParams.seqInnerSize);
        if (fullAttenWindow) {
          start_idx = 0;
          end_idx = this->maxInnerLoopTimes;
        } else {
          start_idx = (this->sOuterOffset - preTokens) / (int32_t)(this->singleProcessSInnerSize);
          end_idx = (this->sOuterOffset + nextTokens + this->singleProcessSOuterSize +
