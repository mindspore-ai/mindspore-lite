 ...
 * \brief
 */
#ifndef INNER_PROMPT_FLASH_ATTENTION_S1S2_BNS1_X310_BASE_H
#define INNER_PROMPT_FLASH_ATTENTION_S1S2_BNS1_X310_BASE_H
...
struct InnerPromptFlashAttentionTypeTraitsNZ {
  using mmInputType = T;
  using mmBiasType = T;
...
class InnerPromptFlashAttentionS1s2Bns1X310Base {
 public:
  __aicore__ inline InnerPromptFlashAttentionS1s2Bns1X310Base() {}
  __aicore__ inline void Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value,
                              ...
                              const InnerPromptFlashAttentionTilingData *__restrict tiling, __gm__ uint8_t *gmTiling,
                              TPipe *tPipe);
  ...
  // define datatype
  using mmInputType = typename InnerPromptFlashAttentionTypeTraitsNZ<T, PFAT::calcMode>::mmInputType;
  using mmBiasType = typename InnerPromptFlashAttentionTypeTraitsNZ<T, PFAT::calcMode>::mmBiasType;
  using mmOutputType = typename InnerPromptFlashAttentionTypeTraitsNZ<T, PFAT::calcMode>::mmOutputType;
  using mm1OutputType = typename InnerPromptFlashAttentionTypeTraitsNZ<T, PFAT::calcMode>::mm1OutputType;
  using softmaxType = typename InnerPromptFlashAttentionTypeTraitsNZ<T, PFAT::calcMode>::softmaxType;
  // define matmul
 ...
 protected:
  const InnerPromptFlashAttentionTilingData *__restrict tilingData;
  TPipe *pipe;
  // define the que
  TBuf<> attenMaskUb_;
  ...
  __aicore__ inline void SoftmaxBasicComputeTail(LocalTensor<mm1OutputType> &mmResUb, LocalTensor<float> &softmaxMaxUb,
                                                 LocalTensor<float> &softmaxSumUb,
                                                 LocalTensor<softmaxType> &softmaxExpUb,
                                                 LocalTensor<uint8_t> &sharedTmpUb,
                                                 const SoftMaxShapeInfo &softmaxShapeInfo);
  ...
  __aicore__ inline void LoopSOuterOffsetInitWithBSH(uint64_t seqListOffsetSize, int sIdx);

  __aicore__ inline void LoopSOuterOffsetInitWithBNSD(uint64_t seqListOffsetSize, int sIdx);

  __aicore__ inline void LoopSOuterOffsetInit(uint64_t seqListOffsetSize, int sIdx);
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::Init(
  ...
  const InnerPromptFlashAttentionTilingData *__restrict tiling, __gm__ uint8_t *gmTiling, TPipe *tPipe) {
  ...
  if ((tilingData->promptAttentionTensorSizeRect.tmpSoftMaxV2Size) != 0 &&
    ...
    pipe->InitBuffer(tmpSoftmaxFlashV2Ub_, tilingData->promptAttentionTensorSizeRect.tmpSoftMaxV2Size / UB_ALIGN_NZ *
                                             UB_ALIGN_NZ * sizeof(uint8_t));
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::InitMsd(
  __gm__ uint8_t *key_antiquant_scale, __gm__ uint8_t *key_antiquant_offset, __gm__ uint8_t *value_antiquant_scale,
  __gm__ uint8_t *value_antiquant_offset) {
  return;
}

template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::InitOutputSingleCore() {
...
InnerPromptFlashAttentionS1s2Bns1X310Base<PFATypeNZ<PFALayoutNZ::BNSD, int8_t, bool, int8_t>>::InitOutputSingleCore() {}
template <>
__aicore__ inline void
InnerPromptFlashAttentionS1s2Bns1X310Base<PFATypeNZ<PFALayoutNZ::BSH, int8_t, bool, int8_t>>::InitOutputSingleCore() {}

template <>
__aicore__ inline void
InnerPromptFlashAttentionS1s2Bns1X310Base<PFATypeNZ<PFALayoutNZ::BNSD, int8_t, half, int8_t>>::InitOutputSingleCore() {}
template <>
__aicore__ inline void
InnerPromptFlashAttentionS1s2Bns1X310Base<PFATypeNZ<PFALayoutNZ::BSH, int8_t, half, int8_t>>::InitOutputSingleCore() {}

template <>
__aicore__ inline void
InnerPromptFlashAttentionS1s2Bns1X310Base<PFATypeNZ<PFALayoutNZ::BNSD, int8_t, float, int8_t>>::InitOutputSingleCore() {
}
template <>
__aicore__ inline void
InnerPromptFlashAttentionS1s2Bns1X310Base<PFATypeNZ<PFALayoutNZ::BSH, int8_t, float, int8_t>>::InitOutputSingleCore() {}

template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::initOffset() {
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::InitTensorSize(
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::AttenMaskCopyIn(uint64_t offset,
                                                                                        uint32_t sinnerSize,
  ...
  // [S1!=S2 change ③] tail-aware Q-row count: on the last outer block only copy the
  // real (non-padded) rows, mirroring DataCopyTransposeOut. Avoids reading past the
  // mask tensor when S1 % singleProcessSOuterSize != 0.
  intriParams.blockCount = this->isOuterTail_ ? this->singleProcessSOuterSizeTail : this->singleProcessSOuterSize;
  intriParams.blockLen = sinnerSize / this->maskTypeByteNum;
  // [S1!=S2 change ①] the mask GM row stride is the mask tensor's own last dim
  // (maskKVsSize), NOT the KV sequence length. These are equal for an exact [S1,S2]
  // mask but differ for a padded mask (maskKVsSize > S2); maskKVsSize is the correct
  // general form and matches all the offset arithmetic (attenMaskCoreOffset etc.).
  intriParams.srcStride =
    (this->tilingData->promptAttentionBaseParams.maskKVsSize - sinnerSize) / this->maskTypeByteNum;
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::AttenMaskTransND2NZ(uint32_t SInnerSize,
                                                                                            uint32_t SOuterSize) {
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::ElewiseCompute310P(
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::SoftmaxBasicComputeFirstTail(
  ...
  if (this->isSoftmaxResNeedUpdate) {
    SoftMaxShapeInfo softmaxShapeInfo{
      ...
      static_cast<uint32_t>(tilingData->promptAttentionSingleCoreParams.singleProcessSOuterSize),
      static_cast<uint32_t>(tilingData->promptAttentionSingleCoreParams.singleProcessSInnerSize)};
    this->isSoftmaxResNeedUpdate =
      AdjustSoftMaxRes<mm1OutputType, float, true>(mmResUb, softmaxMaxUb, this->negativeScalar, 0.0, softmaxShapeInfo);
  }
}

template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::SoftmaxBasicComputeTail(
  LocalTensor<mm1OutputType> &mmResUb, LocalTensor<float> &softmaxMaxUb, LocalTensor<float> &softmaxSumUb,
  LocalTensor<softmaxType> &softmaxExpUb, LocalTensor<uint8_t> &sharedTmpUb, const SoftMaxShapeInfo &softmaxShapeInfo) {
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::SoftmaxBasicComputeFirstTailTmp(
  LocalTensor<mmOutputType> &mmResUb, LocalTensor<mmOutputType> &softmaxMaxUb, LocalTensor<mmOutputType> &softmaxSumUb,
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::SoftmaxBasicComputeTailTmp(
  LocalTensor<mmOutputType> &mmResUb, LocalTensor<mmOutputType> &softmaxMaxUb, LocalTensor<mmOutputType> &softmaxSumUb,
  LocalTensor<mmOutputType> &softmaxExpUb, LocalTensor<uint8_t> &sharedTmpUb,
  const SoftMaxShapeInfo &softmaxShapeInfo) {
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::Bmm2UpdateDivNoTail310P(
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::Bmm2UpdateDivNoTail310PTmp(
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::Bmm2Compute(
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::UpdateVmul(
  LocalTensor<softmaxType> &softmaxExpUb) {
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::ComputeAttenMaskOffset(uint32_t sInnerLoopIdx,
                                                                                               bool isLast) {
  ...
  if (!isLast) {
    ...
    attenMaskOffset =
      attenMaskCoreOffset + this->singleProcessSOuterSize * (uint64_t)tilingData->promptAttentionBaseParams.maskKVsSize;
  }
}

template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::ComputeOffset(uint32_t sInnerLoopIdx,
                                                                                      bool isLast) {
  if constexpr (PFAT::layout == PFALayoutNZ::BSH) {
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::DataCopyTransposeOut(
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::Bmm1Compute(LocalTensor<mmInputType> &a1Local,
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::LoopSOuterOffsetInitWithBSH(
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::LoopSOuterOffsetInitWithBNSD(
  uint64_t seqListOffsetSize, int sIdx) {
  ...
  uint64_t seqInnerOffsetSize =
    tilingData->promptAttentionBaseParams.seqSize == tilingData->promptAttentionBaseParams.seqInnerSize
      ? seqListOffsetSize / headNumRatio
      : static_cast<uint64_t>(sIdx) * head_stride_kv * tilingData->promptAttentionBaseParams.headNumSize / headNumRatio;
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::LoopSOuterOffsetInit(uint64_t seqListOffsetSize,
                                                                                             int sIdx) {
  if constexpr (PFAT::layout == PFALayoutNZ::BSH) {
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::Bmm2UpdateAdd(
  LocalTensor<mmOutputType> &bmm2ResUb) {
  LocalTensor<mmOutputType> bmm2ResPreUb = tempBmm2Ub.Get<mmOutputType>(bmm2ResUbSize);
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::GetSingleCoreParam(int sIdx) {
  ...
  actualSeqLengthKVPerBatch =
    isActualLenDimsKVNull ? tilingData->promptAttentionBaseParams.seqInnerSize : actualSeqLengthsKVGm.GetValue(sIdx);
...
template <typename PFAT>
__aicore__ inline void InnerPromptFlashAttentionS1s2Bns1X310Base<PFAT>::CopyND2NZOnTheFly(
