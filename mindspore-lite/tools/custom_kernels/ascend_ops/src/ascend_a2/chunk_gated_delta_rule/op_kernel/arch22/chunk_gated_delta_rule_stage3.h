...
using aT3 = MatmulType<TPosition::GM, CubeFormat::ND, lowType, true>;
template <typename lowType>
using bT3 = MatmulType<TPosition::GM, CubeFormat::ND, lowType, true>;
template <typename lowType>
using cT3 = MatmulType<TPosition::GM, CubeFormat::ND, lowType>;
template <typename lowType>
using StageThreeMT = matmul::MatmulImpl<aT3<lowType>, bT3<lowType>, cT3<lowType>>;

template <typename lowType>
struct StageThreeParams {
  GlobalTensor<lowType> qkt;    // (Nv, Sp, Dk)
  GlobalTensor<float> gCumExp;  // (Nv, Sp)
  GlobalTensor<lowType> vInner;
  GlobalTensor<float> maskTensor;
  GM_ADDR ws;
  GlobalTensor<lowType> attnOut;
  StageThreeMT<lowType> *mm3;
  TPipe *pipe;
  ChunkGroup *cg;
  float scale;
  int64_t Nv;
...
class Stage3 {
 public:
  __aicore__ inline void Init(StageThreeParams<lowType> *initParams, int32_t coreNum) {
    coreId_ = GetBlockIdx();
    sTP_ = initParams;
    pipe_ = sTP_->pipe;
    ...
    Dv_ = sTP_->Dv;
    Dk_ = sTP_->Dk;
    paddedDv_ = Ceil(Dv_, BLOCK_SIZE / sizeof(lowType)) * (BLOCK_SIZE / sizeof(lowType));
    gOptional_ = sTP_->gOptional;
    uint64_t workSpaceOffset = 0;
    tmpGM_.SetGlobalBuffer(reinterpret_cast<__gm__ lowType *>(initParams->ws + workSpaceOffset +
    ...
    uint64_t inQueueSize =
      static_cast<uint64_t>(chunkSize_) * AscendC::Std::max((int64_t)chunkSize_, paddedDv_) * sizeof(lowType);
    pipe_->InitBuffer(inQueue_, BUFFER_NUM_ONE, inQueueSize);
    pipe_->InitBuffer(
      outQueue_, BUFFER_NUM_ONE,
      chunkSize_ > paddedDv_ ? chunkSize_ * chunkSize_ * sizeof(float) : chunkSize_ * paddedDv_ * sizeof(lowType));
    ...
    // copy mask in
    DataCopyExtParams inParams{static_cast<uint16_t>(chunkSize_), static_cast<uint32_t>(chunkSize_ * sizeof(float)), 0,
                               0, 0};
  ...
  __aicore__ inline void Process() {
    int64_t totalChunks = Nv_ * chunkNum_;  // Nv Nc fusion
    ...
    for (int64_t idx = startChunk; idx < endChunk; idx++) {
      ...
      int64_t chunkPos = chunkId * chunkSize_;                                  // start position of current chunk
      curChunkSize_ = (chunkId == chunkNum_ - 1) ? lastChunkSize : chunkSize_;  // tail chunk
      if ASCEND_IS_AIV {
        if (GetSubBlockIdx() == 0) {
  ...
  __aicore__ inline void CalMaskedQKT(GlobalTensor<lowType> outGM, int nvId, int chunkPos) {
    // auto-pad to chunkSize
    if (gOptional_) {
      AlignedCopyIn(sTP_->gCumExp[nvId * Sp_ + chunkPos], 1, curChunkSize_);  // auto-pad
    ...
    auto qkt = inQueue_.DeQue<lowType>();
    auto scale_qkt = outQueue_.AllocTensor<lowType>();
  ...
  __aicore__ inline void AICProcess(GlobalTensor<lowType> tmpGM, GlobalTensor<lowType> vInner,
                                    GlobalTensor<lowType> attnInter) {
    // masked_qkt @ v_inner
  ...
  __aicore__ inline void AlignedCopyIn(GlobalTensor<inType> tmpGM, int32_t row, int32_t col) {
    ...
    // unaligned copy-in is auto-aligned, then strided copy into UB
