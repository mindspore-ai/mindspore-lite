...
class CGDR {
 public:
  ...
  __aicore__ inline void InitMatmul() {
    if ASCEND_IS_AIC {
      // Initialize with matmul tiling data from tiling
  ...
  __aicore__ inline void InitMask() {
    if ASCEND_IS_AIV {
      ...
      SetFlag<HardEvent::MTE3_MTE2>(eventID);  // Sync is mandatory here, otherwise data may be copied out prematurely
  ...
  __aicore__ inline void Process() {
    ...
    for (int64_t bid = 0; bid < tiling_->b; bid++) {
      ...
      for (int64_t pos = seqStart; pos < seqEnd; pos += tiling_->maxGroupLength) {
        ...
        RunStage2(cg, curState, finalState_[bid * tiling_->nv * tiling_->dv * tiling_->dk]);
        SyncAll<false>();
 ...
 private:
  __aicore__ inline void RunStage1(const ChunkGroup &cg) {
    GDRStageOneInitParams<lowType> initStageOneParams{query_, key_,         value_,        beta_,   g_,
  ...
  __aicore__ inline void RunStage2(ChunkGroup &cg, GlobalTensor<lowType> stateIn, GlobalTensor<lowType> stateOut) {
    Stage2<lowType> stageTwoOp;
    StageTwoParams<lowType> initStageTwoParams{
  ...
  __aicore__ inline void RunStage3(ChunkGroup &cg) {
    Stage3<lowType> stageThreeOp;
    StageThreeParams<lowType> initStageThreeParams{
      qkt_,         gCum_,
 ...
 private:
  ...
  TBuf<TPosition::VECCALC> tmpBuff_;  // buffer for constructing mask matrix

  // Matmul objects
  StageOneMT<lowType> stage1MT_;
  StageTwoMT<lowType> stage2MT_;
  StageThreeMT<lowType> stage3MT_;

  // Stage operators
  Stage1<lowType> stageOneOp_;
  bool gFlag_ = false;
};
