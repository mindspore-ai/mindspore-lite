...
void IrExportBuilder::BuildModelInfo() {
  ...
  model_->set_ir_version(kCurrentMindIRVersion);
...
bool IrExportBuilder::BuildParameters(const FuncGraphPtr &func_graph, mind_ir::GraphProto *const graph_proto) {
  ...
  for (size_t param_counter = 0; param_counter < param_size; ++param_counter) {
    ...
    param->set_user_data<std::string>(lite::kUniqueCacheName, std::make_shared<std::string>(param_name));
    if (top_graph &&
...
bool IrExportBuilder::BuildIsolatedCNode(const AnfNodePtr &node, std::set<AnfNodePtr> *visited) {
  ...
  for (size_t i = 1; i < input_size; i++) {
    ...
    input->set_user_data<std::string>(lite::kUniqueCacheName, std::make_shared<std::string>(node_name));
    input_names.push_back(node_name);
  }
  // build cnode
  auto output_name = GetUniqueNodeName(cnode);
  cnode->set_user_data<std::string>(lite::kUniqueCacheName, std::make_shared<std::string>(output_name));
...
bool IrExportBuilder::BuildOutput(const CNodePtr &node, mind_ir::GraphProto *const graph_proto) {
  ...
  node->set_user_data<std::string>(lite::kUniqueCacheName,
                                   std::make_shared<std::string>(graph_name + lite::kReturnNode));
  AnfNodePtr arg = node->input(1);
  auto node_name = BuildInputNode(arg, graph_proto);
  arg->set_user_data<std::string>(lite::kUniqueCacheName, std::make_shared<std::string>(node_name));
  if (node_name.empty()) {
  ...
  op->set_user_data<std::string>(lite::kUniqueCacheName,
                                 std::make_shared<std::string>(graph_name + lite::kReturnPrimNode));
...
std::string IrExportBuilder::GetOpTypeName(const AnfNodePtr &node) {
  ...
  if (IsValueNode<Primitive>(node)) {
    ...
    if (do_sign_prim != nullptr && do_sign_prim->function() != nullptr &&
      ...
      node->set_user_data<std::string>(lite::kUniqueCacheName, std::make_shared<std::string>(unique_name));
    }
...
bool IrExportBuilder::BuildCNode(const CNodePtr &node, mind_ir::GraphProto *const graph_proto) {
  ...
  node->set_user_data<std::string>(lite::kUniqueCacheName, std::make_shared<std::string>(output_name));
  if (nodeName_.count(output_name) > 0) {
    MS_LOG(INFO) << "There is a duplicate name: " << output_name;
    return true;
  }
  ...
  if (IsValueNode<FuncGraph>(node->input(0))) {
    node->input(0)->set_user_data<std::string>(lite::kUniqueCacheName,
...
std::string IrExportBuilder::GetUniqueNodeName(const AnfNodePtr &node) {
  ...
  if (IsValueNode<FuncGraph>(node)) {
    ...
    node->set_user_data<std::string>(lite::kUniqueCacheName, std::make_shared<std::string>(name));
    return name;
  }
...
bool IrExportBuilder::SetScalarToAttributeProto_ir(const ValuePtr &value,
  ...
  if (value->isa<StringImm>()) {
    ...
    attr_proto->set_type(mind_ir::AttributeProto_AttributeType_FLOAT16);
    attr_proto->set_f(static_cast<float>(value->cast<FP16ImmPtr>()->value()));
  } else if (value->isa<BF16Imm>()) {
    attr_proto->set_type(mind_ir::AttributeProto_AttributeType_BFLOAT16);
    attr_proto->set_f(static_cast<float>(value->cast<BF16ImmPtr>()->value()));
  } else {
