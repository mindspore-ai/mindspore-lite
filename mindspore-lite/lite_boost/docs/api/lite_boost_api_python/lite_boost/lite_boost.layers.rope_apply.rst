...
.. py:function:: lite_boost.layers.rope_apply(x, grid_sizes, freqs)
    ...
    cos/sin 表以 LRU 方式进程内缓存，条目数与估算字节数均有上限
    （``_ROPE_CACHE_MAX_ENTRIES=32``、``_ROPE_CACHE_MAX_BYTES=2 GiB``），缓存内存有界；被淘汰的表再次命中
    时按 key 重建，结果不变。长时间运行显存持续增长时，可在业务空闲点定期调用 ``torch.npu.empty_cache()``。
