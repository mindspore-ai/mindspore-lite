...
# ============================================================================
"""Qwen2 transformers monkey-patches (custom Ms* operator ONNX exports).

Replaces the transformers 4.57 Qwen2 forwards (attention / decoder layer /
model / causal LM / RMSNorm / RoPE helpers) with custom-operator
implementations so ``torch.onnx.export`` produces the mslite-llm NNRT graph.

The Ms* operators (``MsRotaryPosEmb`` / ``MsScatterND`` / ``MsRmsNorm`` /
``MsGroupMatmul`` / ``MsAddSoftmax``) are imported from the custom_ops
torch_custom layer (vendored under custom_ops/) — the single source of truth
for the operator ONNX contracts (eager reference + ``custom::`` symbolic).

Example:
    from models.qwen2_5.qwen2_5_patch import apply_qwen2_patch

    apply_qwen2_patch()  # requires transformers 4.57.x
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# Part 1: monkey-patches — replace transformers Qwen2 forward methods with
...
def repeat_kv_4_40(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
    ...
    batch, _, slen, head_dim = hidden_states.shape
    if n_rep == 1:
        return hidden_states
    hidden_states = (
        torch.cat([hidden_states.reshape(-1, 1, slen, head_dim)] * n_rep, 1)
...
def apply_rotary_pos_emb_4_40(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):  # pylint: disable=unused-argument
...
# The Ms* custom operators (MsRotaryPosEmb / MsScatterND / MsRmsNorm /
# MsGroupMatmul / MsAddSoftmax) are imported from the custom_ops torch_custom
# layer above; only the model-side adapters remain below.
...
def qwen2_attention_forward_4_40(  # pylint: disable=unused-argument
    self,
    ...
    **kwargs,
    ...
    """Qwen2 attention forward with RoPE lifted out (rope_cos/rope_sin inputs)."""
    ...
    query_states = query_states.view(bsz, -1, self.config.num_attention_heads, self.head_dim).transpose(1, 2)
    key_states = key_states.view(bsz, -1, self.config.num_key_value_heads, self.head_dim).transpose(1, 2)
    value_states = value_states.view(bsz, -1, self.config.num_key_value_heads, self.head_dim).transpose(1, 2)
...
def qwen2_decoder_layer_forward_4_40(  # pylint: disable=unused-argument
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
    ...
    **kwargs,
    ...
    """Qwen2 decoder-layer forward patched to consume GGUF-side rope tables."""
    if "padding_mask" in kwargs:
        warnings.warn(
            "Passing `padding_mask` is deprecated and will be removed in v4.37. "
            "Please make sure use `attention_mask` instead.`"
...
def qwen2_model_forward_4_40(  # pylint: disable=protected-access
    self,
    ...
    """Qwen2 model forward patched for rope-table injection."""
...
def qwen2_causal_model_forward_4_40(
    ...
    """Qwen2 causal-LM forward patched for GGUF-side weights and rope tables."""
    output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
    output_hidden_states = (
...
def apply_qwen2_patch():
    """Monkey-patch transformers qwen2 modules with the custom-op forwards.

    Requires transformers 4.57.x: the patched forwards match the 4.57 Qwen2
    module attributes (incl. the real q/k/v attention biases that older lines
    dropped), and GGUF skeleton loading uses the 4.57 ``gguf_file=`` kwarg /
    ``modeling_gguf_pytorch_utils``.
    """
    transformers_version = check_version()
    if not transformers_version.startswith("4.57"):
        raise RuntimeError(
            f"Transformers version {transformers_version} is NOT compatible with the qwen2.5 exporter. "
    ...
    logger.info("Applied qwen2 patches (transformers %s).", transformers_version)
