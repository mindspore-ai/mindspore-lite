...
* head_dim may differ from ``hidden // heads`` (MiniMind-3: 96), handled by
  the GGUF config mapping patch in ``qwen3_patch``.
"""
...
from .qwen3_patch import apply_qwen3_patch
...
class Qwen3Onnx:
    ...
    def load(self, model_path, layers=8):
        ...
        the real Q4_0 weights are injected afterwards by ``qwen3_gguf_loader``.
        """
    ...
    def rope_sin_cos_save(self, cos_path, sin_path, seq_len):
        ...
        transformers 4.57: RoPE lives on ``Qwen3Model.rotary_emb``; forward
        takes (hidden_states, position_ids) and returns [batch, seq_len, head_dim].
        """
