...
# ============================================================================
"""Inject GGUF Q4_0 weights into a MiniCPM-2B ONNX skeleton.

llama.cpp quantizes MiniCPM with the standard LLaMA-family tensor names
(``blk.{i}.attn_q.weight`` / ``ffn_gate.weight`` / ``attn_norm.weight`` /
``output_norm.weight``), which is exactly the Qwen2.5 map — only the
reference layer count differs (40).  MiniCPM carries no q/k/v biases
(``attention_bias=False``), so those optional entries simply miss in the
GGUF file and are skipped by the existing ``None`` guard.

Example:
    from models.minicpm.minicpm_gguf_loader import gguf_loader
...
# ONNX node name -> GGUF tensor name (fp16 biases + layer norms).
FP16_WEIGHT_MAP = {
    "/model/layers.{}/self_attn/q_proj/Add": "blk.{}.attn_q.bias",
    "/model/layers.{}/self_attn/k_proj/Add": "blk.{}.attn_k.bias",
    "/model/layers.{}/self_attn/v_proj/Add": "blk.{}.attn_v.bias",
...
def load_q4_weight(model, weights, layers=40):
...
def load_decode_fp16_weight(model, weights, layers=40):
    """Inject fp16 biases and layer-norm weights into the decoder."""
    nodes = {node.name: node for node in model.graph.node}
    ...
    for i in range(layers):
        for onnx_layer_name, gguf_weight_name in FP16_WEIGHT_MAP.items():
            ...
            if gguf_weight is None:
                # MiniCPM has no q/k/v biases — expected miss, not an error.
                logger.debug("Missing GGUF tensor %s (optional)", gguf_weight_name)
                continue
            new_initializers.append(create_new_initializer(onnx_weight_name, gguf_weight))
            logger.debug("Load %s -> %s", gguf_weight_name, onnx_weight_name)
    return new_initializers
