...
Status Model::Build(const void *model_data, size_t data_size, const void *weight_data, size_t weight_size,
  ...
  if (weight_data == nullptr || weight_size == 0) {
    constexpr const char kMsg[] =
      "The pointer to the model's independent weight file provided by the current user is null, and the "
      "model weight size is zero. Please check whether the relevant parameters are correctly passed. If "
      "the user's model architecture and weights are in a single MindIR file, please use other Build "
      "interfaces; this interface is not recommended.";
    MS_LOG(ERROR) << kMsg;
    return Status(kLiteParamInvalid, kMsg);
  }
