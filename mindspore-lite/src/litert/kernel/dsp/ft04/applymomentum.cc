...
int ApplyMomentumDSPKernel::ApplyMomentumRunFp16() {
  kernel_name_ = "hp_applymomentum_s";
  core_mask_ = 0xf;
  return dsp_runtime_->RunKernel(kernel_name_, kernel_args_, core_mask_);
}
...
int ApplyMomentumDSPKernel::Run() {
  ...
  if (data_type == kNumberTypeFloat32) {
    float_param_bytes = sizeof(float) * kApplyMomentumFloatParamSize;
  } else if (data_type == kNumberTypeFloat16) {
    float_param_bytes = sizeof(uint16_t) * kApplyMomentumFloatParamSize;
  } else {
  ...
  int ret = RET_ERROR;
  if (data_type == kNumberTypeFloat32) {
    ret = ApplyMomentumRunFp32();
  } else if (data_type == kNumberTypeFloat16) {
    ret = ApplyMomentumRunFp16();
  } else {
    free_all_buffers();
    MS_LOG(ERROR) << "Unsupported data type: " << static_cast<int>(data_type);
    return RET_ERROR;
