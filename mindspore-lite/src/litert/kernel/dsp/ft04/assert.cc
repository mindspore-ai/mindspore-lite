...
#include "src/litert/kernel/dsp/ft04/assert.h"
#include <algorithm>
#include <string>
...
int AssertDSPKernel::Run() {
  ...
  if (in_dt != kNumberTypeInt32) {
    MS_LOG(ERROR) << "Assert DSP ft04 only supports int32 input for now. got: " << static_cast<int>(in_dt);
    return RET_ERROR;
  }
  if (out_tensors_[0]->data_type() != kNumberTypeInt32) {
    MS_LOG(ERROR) << "Assert DSP ft04 expects int32 output. got: " << static_cast<int>(out_tensors_[0]->data_type());
    return RET_ERROR;
  }
