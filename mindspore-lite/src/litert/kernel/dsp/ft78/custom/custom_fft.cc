...
#include "src/litert/kernel/dsp/ft78/custom/custom_fft.h"
#include <vector>
...
constexpr int kFFTMaxSize = 64 * 1024;
}  // namespace
...
int CustomFFTKernel::Prepare() {
  ...
  SetKernelArg({w_device_ptr_, static_cast<uint64_t>(length_)});
...
int CustomFFTKernel::Run() {
  ...
  SetKernelArg({input_device_ptr, w_device_ptr_, output_device_ptr, static_cast<uint64_t>(length_)});
  ret = dsp_runtime_->RunKernel(kernel_name_, kernel_args_, core_mask_);
  if (ret != kSuccess) {
