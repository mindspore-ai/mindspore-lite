...
#include "src/litert/kernel/dsp/kernel/sub.h"
#include <algorithm>
#include <map>
#include <string>
#include <set>
...
int SubDSPKernel::Run() {
  ...
  if (x_device_ptr == 0) {
    MS_LOG(ERROR) << "SubDSPKernel x device ptr is null.";
    return RET_ERROR;
  }
  uint64_t y_device_ptr = allocator->GetDeviceMemPtr(input_y->data());
  if (y_device_ptr == 0) {
    MS_LOG(ERROR) << "SubDSPKernel y device ptr is null.";
    return RET_ERROR;
  }
  uint64_t out_device_ptr = allocator->GetDeviceMemPtr(output->data());
  if (out_device_ptr == 0) {
    MS_LOG(ERROR) << "SubDSPKernel out device ptr is null.";
    return RET_ERROR;
  }
  ...
  int ret =
    sub_func(x_device_ptr, y_device_ptr, out_device_ptr, input0_shape.data(), static_cast<int>(input0_shape.size()),
  ...
  if (ret != kSuccess) {
    MS_LOG(ERROR) << "SubDSPKernel run failed.";
    return RET_ERROR;
  }
  return RET_OK;
}
