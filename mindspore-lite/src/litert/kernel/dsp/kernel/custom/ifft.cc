...
#include "src/litert/kernel/dsp/kernel/custom/ifft.h"
#include <vector>
#include <string>
...
int IFFTKernel::Prepare() {
  ...
  // Initialize w using twiddle_factor_func for IFFT (direction_flag = 1)
  ...
  int ret = twiddle_factor_func(w_device_ptr_, static_cast<uint64_t>(length_),
                                static_cast<uint64_t>(kFFTDirectionInverse), core_mask_, dtype, mtype);
  if (ret != kSuccess) {
...
int IFFTKernel::CheckSpecs(const std::vector<MSTensor> &inputs, const std::vector<MSTensor> &outputs) {
  ...
  if (shape_size == kMinDimension) {
    ...
    MS_LOG(ERROR) << "IFFT input shape is not supported!";
    return kLiteError;
  }
  if (!IsPowerOfTwo(length_)) {
    MS_LOG(ERROR) << "IFFT input size is not power of 2!";
    return kLiteError;
  }
  return kSuccess;
}
...
int IFFTKernel::Run() {
  ...
  int ret = ifft_func(input_device_ptr, w_device_ptr_, output_device_ptr, batch_count, length_, temp_fft_device_ptr_,
...
REGISTER_CUSTOM_KERNEL(DSP, FTMatrix, kComplex64, Custom_FT_IFFT, CustomKernelCreator<IFFTKernel>)
}  // namespace mindspore::lite
