...
#include "src/litert/kernel/cpu/fp32_grad/binary_cross_entropy_grad.h"
...
kernel::LiteKernel *CpuBinaryCrossEntropyGradFp32KernelCreator(const std::vector<lite::Tensor *> &inputs,
  ...
  auto *kernel = new (std::nothrow) BinaryCrossEntropyGradCPUKernel(opParameter, inputs, outputs, ctx);
  if (kernel == nullptr) {
