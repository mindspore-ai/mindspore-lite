...
int ElementMaximumInt8(int8_t *input0, int8_t *input1, int8_t *output, int element_size,
                       ArithmeticQuantArg *quant_arg) {
  float in0_bias = -quant_arg->in0_args_.zp_ * quant_arg->in0_args_.scale_;
  float in1_bias = -quant_arg->in1_args_.zp_ * quant_arg->in1_args_.scale_;
  float out_scale = quant_arg->out_args_.scale_;
  int out_zp = quant_arg->out_args_.zp_;
  for (int index = 0; index < element_size; ++index) {
    float in0_real = input0[index] * quant_arg->in0_args_.scale_ + in0_bias;
    float in1_real = input1[index] * quant_arg->in1_args_.scale_ + in1_bias;
    float max_val = (in0_real >= in1_real) ? in0_real : in1_real;
    int32_t q_val = roundf(max_val / out_scale) + out_zp;
    output[index] = (int8_t)MSMAX(INT8_MIN, MSMIN(INT8_MAX, q_val));
  }
  return NNACL_OK;
}

int ElementMinimumInt8(int8_t *input0, int8_t *input1, int8_t *output, int element_size,
                       ArithmeticQuantArg *quant_arg) {
  float in0_bias = -quant_arg->in0_args_.zp_ * quant_arg->in0_args_.scale_;
  float in1_bias = -quant_arg->in1_args_.zp_ * quant_arg->in1_args_.scale_;
  float out_scale = quant_arg->out_args_.scale_;
  ...
  for (int index = 0; index < element_size; ++index) {
    float in0_real = input0[index] * quant_arg->in0_args_.scale_ + in0_bias;
    float in1_real = input1[index] * quant_arg->in1_args_.scale_ + in1_bias;
    float min_val = (in0_real <= in1_real) ? in0_real : in1_real;
    int32_t q_val = roundf(min_val / out_scale) + out_zp;
    output[index] = (int8_t)MSMAX(INT8_MIN, MSMIN(INT8_MAX, q_val));
  }
  return NNACL_OK;
}
