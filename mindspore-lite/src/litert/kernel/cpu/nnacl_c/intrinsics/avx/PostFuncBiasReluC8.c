...
#include "nnacl_c/intrinsics/avx/common_utils.h"
...
void PostFuncBiasReluC8(float *dst, const float *src, const float *bias, size_t oc8div, size_t oc8mod,
  ...
  stride /= sizeof(float);
  int loop_c8 = 0;
  size_t src_stride = plane_size * C8NUM;
  for (; loop_c8 <= (int)(oc8div)-C32NUM; loop_c8 += C32NUM) {
    ...
    for (; plane_size_tmp >= C4NUM; plane_size_tmp -= C4NUM) {
      ...
      _mm256_storeu_ps(dst_c8, src1);
      _mm256_storeu_ps(dst_c8 + C8NUM, src5);
      _mm256_storeu_ps(dst_c8 + C16NUM, src9);
      _mm256_storeu_ps(dst_c8 + C24NUM, src13);
      dst_c8 += stride;
      _mm256_storeu_ps(dst_c8, src2);
      _mm256_storeu_ps(dst_c8 + C8NUM, src6);
      _mm256_storeu_ps(dst_c8 + C16NUM, src10);
      _mm256_storeu_ps(dst_c8 + C24NUM, src14);
      dst_c8 += stride;
      ...
      _mm256_storeu_ps(dst_c8, src3);
      _mm256_storeu_ps(dst_c8 + C8NUM, src7);
      _mm256_storeu_ps(dst_c8 + C16NUM, src11);
      _mm256_storeu_ps(dst_c8 + C24NUM, src15);
      dst_c8 += stride;
      _mm256_storeu_ps(dst_c8, src4);
      _mm256_storeu_ps(dst_c8 + C8NUM, src8);
      _mm256_storeu_ps(dst_c8 + C16NUM, src12);
      _mm256_storeu_ps(dst_c8 + C24NUM, src16);
      dst_c8 += stride;
    }
    for (; plane_size_tmp > 0; plane_size_tmp -= 1) {
      ...
      _mm256_storeu_ps(dst_c8, src1);
      _mm256_storeu_ps(dst_c8 + C8NUM, src2);
      _mm256_storeu_ps(dst_c8 + C16NUM, src3);
      _mm256_storeu_ps(dst_c8 + C24NUM, src4);
      dst_c8 += stride;
      src += C8NUM;
    }
