...
int L2NormInt8CPUKernel::DoExecute(int task_id) {
  ...
  // Input tensor shape cannot be empty
  MS_CHECK_GT(input_shape.size(), 0, RET_ERROR);
  // Input tensor last dim size must be greater than 0.
  MS_CHECK_GT(input_shape.back(), 0, RET_ERROR);
  MS_CHECK_GT(input_tensor->ElementsNum(), 0, RET_ERROR);
  int outer_size = input_tensor->ElementsNum() / input_shape.back();
