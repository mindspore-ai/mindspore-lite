...
bool ParseShapeStr(const std::string &shape_str, std::vector<int64_t> *shape_ptr) {
  if (shape_ptr == nullptr) {
    MS_LOG(ERROR) << "Input argument shape_ptr is nullptr";
    return false;
  }
  auto str_dims = lite::StrSplit(shape_str, ",");
  if (str_dims.empty()) {
    MS_LOG(ERROR) << "Invalid input shape dim, dims number cannot be 0";
    return false;
  }
...
bool ParseShapeStr(const std::string &shape_str, std::vector<ShapeDim> *shape_ptr) {
  ...
  auto str_dims = lite::StrSplit(shape_str, ",");
  if (str_dims.empty()) {
    MS_LOG(ERROR) << "Invalid input shape dim, dims number cannot be 0: " << shape_str;
    return false;
  }
  auto &shape = *shape_ptr;
  shape.resize(str_dims.size());
