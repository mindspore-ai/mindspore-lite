...
abstract::ShapePtr ReduceFusionInferShape(const PrimitivePtr &primitive,
  ...
  bool axis_is_dynamic = lite::CheckAndGetAxisValue(input_args, &axis_value, &axis_shape, primitive);
  ...
  lite::ReduceFuncCheckAxisInferImpl(primitive, &axis_value, x_shape.size());

  if (axis_is_dynamic) {
    out_shape = lite::ReduceFuncCalShapeAxisDyn(x_shape, keep_dims);
    return std::make_shared<abstract::Shape>(out_shape);
  }
  out_shape = lite::ReduceFuncCalShapeInferImpl(primitive, x_shape, axis_value, keep_dims);
  return std::make_shared<abstract::Shape>(out_shape);
}
}  // namespace
