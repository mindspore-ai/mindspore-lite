...
OpParameter *PopulateGluParameter(const void *prim) {
  ...
  auto *param = reinterpret_cast<GluParameter *>(malloc(sizeof(GluParameter)));
  if (param == nullptr) {
    MS_LOG(ERROR) << "malloc ExpParameter failed.";
    return nullptr;
  }
  memset(param, 0, sizeof(GluParameter));
...
REG_POPULATE(PrimitiveType_GLU, PopulateGluParameter, SCHEMA_CUR)
}  // namespace lite
}  // namespace mindspore
