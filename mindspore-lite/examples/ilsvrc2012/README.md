...
>It should be noted that `mindspore_lite` can obtain the `mindspore_lite-{version}-*.whl` file through [Build MindSpore Lite](https://www.mindspore.cn/lite/cloud_docs/en/master/use/build.html) , then install via `pip install mindspore_lite-{version}-*.whl`
