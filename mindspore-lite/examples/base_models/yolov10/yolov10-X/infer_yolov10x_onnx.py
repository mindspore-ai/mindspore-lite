...
# ============================================================================
"""YOLOv10 ONNX inference script using ONNXRuntime."""
...
class YoloV10OnnxInferencer:
    """YOLOv10 ONNX inference class using ONNXRuntime."""
    ...
    def __init__(
        ...
        self._input_names = [inp.name for inp in self.sess.get_inputs()]
        self._output_names = [out.name for out in self.sess.get_outputs()]
    ...
    def preprocess(self, image_paths: List[str]) -> Tuple[np.ndarray, List[Tuple[int, int]]]:
        ...
        Returns:
            images: Preprocessed image tensor of shape (batch, 3, H, W)
            orig_shapes: Original image shapes for post-processing
        """
        images = []
        orig_shapes = []
    ...
    def forward(self, images: np.ndarray) -> np.ndarray:
        ...
        outputs = self.sess.run(self._output_names, {self._input_names[0]: images})
        return outputs[0]
    ...
    def postprocess(
        ...
        Args:
            ...
            orig_shapes: Original image shapes
...
def _parse_args():
    """parse args"""
    parser = argparse.ArgumentParser(description="YOLOv10 ONNX inference")
    parser.add_argument("--model", type=str, required=True, help="Path to YOLOv10 ONNX model")
...
def main():
    ...
    inferencer = YoloV10OnnxInferencer(
