#!/usr/bin/env python3
"""STDF MindSpore Lite 推理脚本(MindIR)。
...
class StdfMsliteInferencer:
    """STDF MindSpore Lite 推理封装。"""
...
def _parse_args():
    p = argparse.ArgumentParser(description="STDF MindSpore Lite 推理(MindIR)")
    p.add_argument("--mindir", type=str, required=True)
    p.add_argument("--input", type=str, required=True, help="低质量帧图像")
    p.add_argument("--output", type=str, default="./stdf_enhanced_mslite.png")
...
def main():
    ...
    infer = StdfMsliteInferencer(args.mindir, device=args.device, device_id=args.device_id)
