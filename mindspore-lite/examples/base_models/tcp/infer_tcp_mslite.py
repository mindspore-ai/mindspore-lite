...
# ============================================================================
"""TCP MindSpore Lite inference (numpy-only, no torch)."""
...
class TCPMsLite:
    ...
    def forward(self, front_img, speed, command):
        """run forward."""
        inputs = [mslite.Tensor(np.ascontiguousarray(front_img)),
                  mslite.Tensor(np.ascontiguousarray(speed)),
                  mslite.Tensor(np.ascontiguousarray(command))]
...
def parse_args():
    """parse args"""
    parser = argparse.ArgumentParser(description="TCP MindSpore Lite inference")
...
def main():
    ...
    net = TCPMsLite(args.model, args.device, args.device_id)
