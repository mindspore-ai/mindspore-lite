...
# ============================================================================
"""HiVT MindSpore Lite inference (numpy-only, no torch)."""
...
class HiVTMsLite:
    ...
    def forward(self, actor_history, map_lanes):
        ...
        inputs = [mslite.Tensor(np.ascontiguousarray(actor_history)),
                  mslite.Tensor(np.ascontiguousarray(map_lanes))]
...
def parse_args():
    """parse args"""
    parser = argparse.ArgumentParser(description="HiVT MindSpore Lite inference")
...
def main():
    ...
    net = HiVTMsLite(args.model, args.device, args.device_id)
