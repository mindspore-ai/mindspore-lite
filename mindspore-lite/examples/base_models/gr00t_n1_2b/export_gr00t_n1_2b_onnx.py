#!/usr/bin/env python3
"""Export a GR00T-N1-style Flow Matching action policy (velocity net) to ONNX.

GR00T N1 (NVIDIA) pairs an Eagle VLM backbone with a Flow Matching action
expert: the action expert is a velocity network v(x_t, t | cond) integrated via
an ODE (Euler steps) from t=0 (noise) to t=1 (action chunk). This differs from
diffusion (DDPM): continuous time in [0,1] and velocity (not noise) prediction.

This script exports the **single velocity step** (vision-conditioned):

  - inputs : image   [1, 3, 224, 224]         float32
             x_t     [1, horizon, action_dim]  float32   (current flow state)
             t       [1]                       float32   (flow time in [0,1])
  - output : velocity [1, horizon, action_dim] float32

The host side runs the Euler ODE loop (numpy, see infer_*). ``--random-init``
runs the pipeline; load real GR00T weights via the official package in task-2.
"""
...
class GR00TVelocityNet(nn.Module):
    """GR00T-N1 Flow Matching velocity net: (image, x_t, t) -> velocity."""
...
def _parse_args():
    p = argparse.ArgumentParser(description="Export a GR00T-N1 Flow Matching velocity step to ONNX.")
    p.add_argument("--output-dir", type=str, default="./gr00t_n1_2b_onnx")
    p.add_argument("--device", type=str, default="cpu", choices=["cpu", "cuda"])
    p.add_argument("--opset", type=int, default=17)
    p.add_argument("--checkpoint", type=str, default="", help="GR00T state_dict (task-2: official package).")
...
def main():
    ...
    model = GR00TVelocityNet(args.img_size, args.patch, args.dim, args.depth, args.heads,
    ...
    if args.checkpoint:
        ...
        print(f"random-init demo GR00T-N1 velocity net (seed={args.seed}). "
              f"Real GR00T = Eagle VLM + Flow Matching action expert (task-2).")
    ...
    onnx_path = out / "gr00t_n1_velocity.onnx"
    with torch.no_grad():
        torch.onnx.export(
