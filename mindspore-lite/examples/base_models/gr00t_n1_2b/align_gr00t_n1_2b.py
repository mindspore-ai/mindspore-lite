#!/usr/bin/env python3
"""Accuracy alignment for the GR00T-N1 Flow Matching velocity net (single step)."""
...
def _torch_baseline(args, image, x_t, t):
    ...
    from export_gr00t_n1_2b_onnx import GR00TVelocityNet  # noqa: E402
...
def _mslite_infer(mindir_path, image, x_t, t, device, device_id):
    ...
    if str(here) not in sys.path:
        sys.path.insert(0, str(here))
    from infer_gr00t_n1_2b_mslite import _build_model, _run_model  # noqa: E402
...
def main():
    p = argparse.ArgumentParser(description="GR00T-N1 alignment (PyTorch vs ONNX vs MSLite)")
    p.add_argument("--onnx", type=str, default="./gr00t_n1_2b_onnx/gr00t_n1_velocity.onnx")
    p.add_argument("--mindir", type=str, default="./gr00t_n1_2b_onnx/gr00t_n1_velocity_graph.mindir")
    ...
    print("=" * 78)
    print("GR00T-N1 alignment: PyTorch vs ONNX vs MindSpore Lite (Flow Matching velocity)")
    print("=" * 78)
    print(f"input: image {image.shape}, x_t {x_t.shape}, t {t}")

    print("\n[PyTorch] GR00TVelocityNet on CPU (fp32) ...")
