...
"""
Infer Qwen3-VL-2B-Thinking on Ascend with MindSpore Lite.

This script supports the Thinking variant of Qwen3-VL-2B which generates
...
def _get_vision_position_ids(start_position, grid_thw, spatial_merge_size, device):
    """
    Get vision position ids for Qwen3-VL-2B-Thinking.
...
def _get_rope_index(
    ...
    """
    Get rope index for Qwen3-VL-2B-Thinking.
...
def _build_position_ids(
    ...
    """
    Build position ids for Qwen3-VL-2B-Thinking.

    Args:
        cfg (Qwen3VLConfig): Qwen3-VL-2B-Thinking configuration.
        input_ids (torch.Tensor): Input ids.
    ...
    Returns:
        tuple: (position_ids_4, rope_deltas)
    """
    import torch
...
def _build_mslite_inputs(model: mslite.Model, feed_dict, preferred_order=None):
    """
    Build MindSpore Lite inputs for Qwen3-VL-2B-Thinking.
...
class Qwen3VLThinkingInferencer:
    """
    Qwen3-VL-2B-Thinking inferencer with MindSpore Lite.
    ...
    def __init__(
        ...
        """
        Initialize Qwen3-VL-2B-Thinking inferencer.
        """
    ...
    def _prepare_inputs(self, image_path_or_url: str, prompt: str):
        """
        Prepare inputs for Qwen3-VL-2B-Thinking.
    ...
    def infer(
        ...
        """
        Infer Qwen3-VL-2B-Thinking on MindSpore Lite.
        ...
        full_response = self.processor.tokenizer.decode(generated, skip_special_tokens=False)

        # Parse thinking content and answer content
        thinking_content = ""
        content = full_response

        # Try to extract thinking and answer parts
        # The Thinking model uses special tokens: <think_start> (151667) and <think_end> (151668)
...
def main():
    """
    Main function for Qwen3-VL-2B-Thinking inference on Ascend with MindSpore Lite.
    """
    parser = argparse.ArgumentParser(
        description="Qwen3-VL-2B-Thinking MindSpore Lite inference (vision + prefill + decode)"
    )
    parser.add_argument(
