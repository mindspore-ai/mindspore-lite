...
"""
Inference with Jina Reranker V3 (Listwise + Pointwise) using MindSpore Lite.
...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Inference with Jina Reranker V3 (Listwise) using MindSpore Lite"
    )
    parser.add_argument(
        "--model-path",
        type=str,
...
def _build_mslite_inputs(model, input_ids_np, attention_mask_np,
    ...
    Args:
        ...
        input_ids_np: numpy array for input_ids
        attention_mask_np: numpy array for attention_mask
        doc_token_indices_np: numpy array for doc_token_indices
        query_token_index_np: numpy array for query_token_index

    Returns:
        list of mslite.Tensor inputs
    """
...
def rerank_listwise(model, tokenizer, query, documents, max_length=8192,
...
def rerank_pointwise(model, tokenizer, query, documents, max_length=8192):
    """
    ...
    All prompts are tokenized together with padding so that every input has
    the same sequence length. This is required because MSLite with
    ``--optimize=ascend_oriented`` compiles the GE graph for a fixed input
    shape; varying the sequence length between calls produces incorrect
    results.
    ...
    for i in range(len(documents)):
        input_ids_i = encoded["input_ids"][i : i + 1]
        attention_mask_i = encoded["attention_mask"][i : i + 1]
        block_scores = _compute_block_scores(
            model, input_ids_i, attention_mask_i
        )
