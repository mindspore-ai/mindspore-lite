...
本教程介绍如何将 **BasicVSR**(CVPR2021 视频超分)导出为单 ONNX,经 ONNX Runtime 验证后转换为 MindIR,在 **Atlas 300I Duo(310P3)** 上推理测速。BasicVSR 用双向循环传播 + SpyNet 光流对齐,4x 超分。固定 N 帧输入,trace 时展开循环(无显式 recurrent 状态 I/O)。
...
```bash
cd mindspore-lite/examples/base_models/basicvsr
python export_basicvsr_onnx.py \
  --repo-dir ./mmagic_src \
  --model-file mmagic.models.basicvsr_net.BasicVSRNet \
  --ckpt ./basicvsr_x4.pth \
  --output-dir ./basicvsr_onnx --device cpu \
...
| `--model-file` | 架构类 dotted 路径 | `mmagic.models.basicvsr_net.BasicVSRNet` |
| `--ckpt` | 权重 | `./basicvsr_x4.pth` |
| `--num-frames` | 固定帧数(展开循环) | `10` |
| `--lr-height/--lr-width` | 低清尺寸(须被 4 整除) | `64` / `64` |

```text
./basicvsr_onnx/
└── basicvsr.onnx   # 输入 lr_seq [1,10,3,64,64],输出 sr_seq [1,10,3,256,256]
```
...
```bash
python infer_basicvsr_onnx.py --onnx ./basicvsr_onnx/basicvsr.onnx \
...
```bash
converter_lite --fmk=ONNX --modelFile=./basicvsr_onnx/basicvsr.onnx \
  --outputFile=./basicvsr_onnx/basicvsr --optimize=ascend_oriented \
...
```bash
python infer_basicvsr_mslite.py --mindir ./basicvsr_onnx/basicvsr.mindir \
...
1. 无法导入 BasicVSRNet → 确认 mmagic 已装或 `--repo-dir` 正确,`--model-file` 路径匹配版本。
