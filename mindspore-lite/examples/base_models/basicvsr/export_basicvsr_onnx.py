#!/usr/bin/env python3
"""Export BasicVSR (CVPR2021 视频超分) to ONNX。

BasicVSR 用双向循环传播 + SpyNet 光流对齐,4x 超分。固定 N 帧输入,trace 时展开循环。
...
class BasicVsrSR(nn.Module):
    """BasicVSR 超分封装:N 帧低清 -> N 帧 4x 超分(双向循环在 trace 时展开)。"""
...
def main():
    ...
    parser.add_argument("--model-file", type=str, default="mmagic.models.basicvsr_net.BasicVSRNet")
    parser.add_argument("--ckpt", type=str, default="./basicvsr_x4.pth")
    parser.add_argument("--output-dir", type=str, default="./basicvsr_onnx")
    ...
    wrapper = BasicVsrSR(net).to(args.device).eval()
    lr = torch.randn(1, args.num_frames, 3, args.lr_height, args.lr_width,
                     dtype=torch.float32, device=args.device)
    _export_one(wrapper, output_dir / "basicvsr.onnx", (lr,),
