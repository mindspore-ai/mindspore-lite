#!/usr/bin/env python3
"""ViDeNN MindSpore Lite 推理脚本(MindIR)。
...
def _build_seq(frame_hw3):
    fr = np.transpose(frame_hw3, (2, 0, 1))[None, ...]
    return np.repeat(fr, 2, axis=0)[None, ...]


class VidennMsliteInferencer:
    """ViDeNN MindSpore Lite 推理封装。"""
...
def _parse_args():
    p = argparse.ArgumentParser(description="ViDeNN MindSpore Lite 推理(MindIR)")
    p.add_argument("--mindir", type=str, required=True)
    p.add_argument("--input", type=str, required=True, help="含噪帧图像")
    p.add_argument("--output", type=str, default="./videnn_denoised_mslite.png")
...
def main():
    ...
    infer = VidennMsliteInferencer(args.mindir, device=args.device, device_id=args.device_id)
    seq = _build_seq(_load_frame(args.input, args.height, args.width))
