#!/usr/bin/env python3
"""ViDeNN ONNXRuntime 推理脚本(精度基准 ground truth)。

demo: 读单张含噪帧复制成 2 帧序列, 输出去噪帧。
"""
...
def _build_seq(frame_hw3):
    ...
    return np.repeat(fr, 2, axis=0)[None, ...]  # [1,2,3,H,W]
...
class VidennOnnxInferencer:
    """ViDeNN ONNXRuntime 推理封装。"""
...
def _parse_args():
    p = argparse.ArgumentParser(description="ViDeNN ONNXRuntime 推理")
    p.add_argument("--onnx", type=str, required=True)
    p.add_argument("--input", type=str, required=True, help="含噪帧图像")
    p.add_argument("--output", type=str, default="./videnn_denoised_onnx.png")
...
def main():
    ...
    infer = VidennOnnxInferencer(args.onnx, device=args.device)
