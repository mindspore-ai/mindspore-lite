...
# ============================================================================
"""PointPillars MindSpore Lite inference (numpy-only, no torch)."""
...
def _random_input(c, h, w):
    """fixed-seed random pseudo-image."""
    rng = np.random.RandomState(SEED)
    return np.ascontiguousarray(rng.rand(1, c, h, w).astype(np.float32))


class PointPillarsMsLite:
    """PointPillars MindSpore Lite inference."""
...
def parse_args():
    """parse args"""
    parser = argparse.ArgumentParser(description="PointPillars MindSpore Lite inference")
...
def main():
    ...
    net = PointPillarsMsLite(args.model, args.device, args.device_id)
    images = _random_input(args.in_channels, args.feat_h, args.feat_w)
    print(f"Using random pseudo-image, shape={images.shape}, seed={SEED}")
