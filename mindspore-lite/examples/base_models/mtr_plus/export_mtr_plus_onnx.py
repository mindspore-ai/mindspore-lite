...
# ============================================================================
"""Export MTR++ (Motion Transformer with ego-actor interaction) to ONNX.

MTR++ extends MTR with scene-level interaction among agents. IO is identical to
MTR (actor history + map polylines -> trajectory + scores).
"""
...
def parse_args():
    ...
    parser.add_argument("--output", type=str, default="mtr_plus_onnx/mtr_plus.onnx")
...
class MTRPlusWrapper(nn.Module):
    """MTR++ wrapper: actor history + map polylines -> trajectory + scores."""
...
def main():
    ...
    try:
        model = ModelCls(obs_len=args.obs_len, num_modes=args.num_modes)
    except TypeError:
        model = ModelCls()
    ...
    wrapper = MTRPlusWrapper(model).to(args.device).eval()
