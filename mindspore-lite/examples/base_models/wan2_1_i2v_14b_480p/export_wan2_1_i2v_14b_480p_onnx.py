"""Export Wan2.1-I2V-14B-480P-Diffusers submodules to ONNX for MindSpore Lite.

Wan2.1-I2V-14B is image-to-video. Compared with the Wan2.1-T2V pipeline it adds
CLIP image conditioning on top of the UMT5 text path and keeps the Wan2.1
*scalar* timestep schedule (NOT the Wan2.2 ``expand_timesteps`` per-token
schedule). First-frame image conditioning is applied by channel-wise
concatenation: the pipeline feeds the transformer a 36-channel
``hidden_states`` = ``cat([latents(16), mask_lat_size(4), latent_condition(16)],
dim=1)`` where ``latent_condition`` is the VAE-encoded image-as-video latent
(denormalised) and ``mask_lat_size`` is the per-frame first-frame mask broadcast
to 4 channels. The pipeline is split into four fixed-shape sub-models:
  ...
  * transformer        : WanTransformer3DModel (DiT, 14B, in_channels=36). Inputs:
                           hidden_states[1,36,F',H',W']   (latents cat condition)
                           timestep[1]                    (scalar, Wan2.1)
...
to encode the conditioning image into ``latent_condition``).
"""
...
# CLIP ViT-H/14 image encoder (Wan2.1 I2V / Wan2.2 TI2V): 224x224 input,
# 257 output tokens (256 patches + CLS), 1280 hidden dim.
_CLIP_IMAGE_SIZE = 224
_CLIP_NUM_TOKENS = 257
_CLIP_HIDDEN_DIM = 1280

# Wan2.1 I2V channel-wise conditioning: the pipeline concatenates
# [latents(16), mask_lat_size(4), latent_condition(16)] along dim=1 before
# feeding the transformer, so the DiT in_channels = 36 (mask_lat_size carries
# vae_scale_factor_temporal=4 channels; see pipeline_wan_i2v.prepare_latents).
_MASK_CHANNELS = _VAE_SCALE_FACTOR_TEMPORAL
_DIT_IN_CHANNELS = _LATENT_CHANNELS + _MASK_CHANNELS + _LATENT_CHANNELS
...
def _dit_in_shape(num_frames, height, width):
    """Compute the transformer input shape (B, in_channels=36, F', H', W').

    The Wan2.1 I2V pipeline feeds the DiT a 36-channel tensor =
    ``cat([latents(16), mask_lat_size(4), latent_condition(16)], dim=1)``.
    """
    num_latent_frames = (num_frames - 1) // _VAE_SCALE_FACTOR_TEMPORAL + 1
    latent_h = height // _VAE_SCALE_FACTOR_SPATIAL
    latent_w = width // _VAE_SCALE_FACTOR_SPATIAL
    return (1, _DIT_IN_CHANNELS, num_latent_frames, latent_h, latent_w)
...
class _TransformerWrapper(nn.Module):
    ...
    def forward(self, hidden_states, timestep, encoder_hidden_states,
        ...
        ``hidden_states`` is the 36-channel ``cat([latents, mask, condition])``
        built by the pipeline (NOT the bare 16-channel latent); ``timestep`` is
        the Wan2.1 scalar timestep tensor [1] (NOT the Wan2.2 per-token tensor);
        ``encoder_hidden_states_image`` is the CLIP image embedding
        [1, 257, 1280].
        """
        return self.model(
...
def export_transformer(model_dir, output_dir, height, width, num_frames,
    ...
    dit_in_shape = _dit_in_shape(num_frames, height, width)
    wrapper = _TransformerWrapper(model)
    hidden_states = torch.zeros(dit_in_shape, dtype=dtype)
    # Wan2.1 I2V: scalar timestep [1] (NOT per-token; that is Wan2.2 TI2V).
...
def main():
    """Parse arguments and export the requested Wan2.1-I2V-14B sub-modules."""
    parser = argparse.ArgumentParser(description="Export Wan2.1-I2V-14B-480P submodules to ONNX")
    parser.add_argument("--model-dir", required=True, help="Wan2.1-I2V-14B-480P-Diffusers weight dir")
    parser.add_argument("--output-dir", default="./wan2_1_i2v_14b_480p_onnx", help="ONNX output dir")
    ...
    if "transformer" in parts:
        print("[export] Wan transformer (DiT, 14B) ...")
