#!/usr/bin/env python3
"""HPT policy MindSpore Lite inference (MindIR): observation -> action chunk."""
...
def _load_obs(args):
    ...
    return np.random.default_rng(args.seed).standard_normal((1, args.obs_dim)).astype(np.float32)


def _parse_args():
    p = argparse.ArgumentParser(description="HPT policy MindSpore Lite inference (MindIR)")
...
def main():
    ...
    if obs.shape[1] != args.obs_dim:
        raise RuntimeError(f"obs dim {obs.shape[1]} != --obs-dim {args.obs_dim}")
    ...
    print("Perf:")
    print(f"  obs_dim: {obs.shape[1]}  device: {args.device}")
