#!/usr/bin/env python3
"""HPT policy ONNXRuntime inference: observation -> action chunk (single forward)."""
...
def _load_obs(args):
    if args.obs_npy:
        arr = np.load(args.obs_npy).astype(np.float32).reshape(1, -1)
        return arr
    if args.obs:
...
def _parse_args():
    p = argparse.ArgumentParser(description="HPT policy ONNXRuntime inference")
...
def main():
    ...
    print("Perf:")
    print(f"  obs_dim: {obs.shape[1]}  horizon/action from model; device: {args.device}")
