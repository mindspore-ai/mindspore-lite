...
# ============================================================================
"""End-to-end image-to-video inference for SVD-XT on MindSpore Lite (Ascend).
...
_DTYPE_MAP = {
    "FLOAT16": np.float16, "FLOAT32": np.float32, "INT64": np.int64, "INT32": np.int32,
}


def _np_to_input(tensor_info, array):
    """Cast a numpy array to the dtype expected by a model input tensor."""
    name = getattr(tensor_info.dtype, "name", str(tensor_info.dtype))
    target = _DTYPE_MAP.get(name)
...
def _build_model(mindir_path, device_id):
    ...
    # precision_mode is set by the converter config (force_fp16); not re-set here.
    model = mslite.Model()
...
class SvdInferencer:
    """SVD-XT image-to-video on MindSpore Lite (image_encoder / unet+vae)."""
    ...
    def infer(self, image_pil, vae_torch, seed=0, num_inference_steps=25,
        ...
        """Run the full SVD-XT pipeline. Returns (frames_uint8, timing_dict).
...
def main():
    """Parse arguments and run SVD-XT image-to-video inference."""
    parser = argparse.ArgumentParser(
        description="SVD-XT MindSpore Lite inference")
    parser.add_argument("--mindir-dir", required=True,
                        help="dir with the 3 *_graph.mindir")
    parser.add_argument("--model-dir", required=True,
                        help="stable-video-diffusion-img2vid-xt diffusers "
