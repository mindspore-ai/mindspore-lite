> ⚠️ **验证阻塞（环境-npu 算子）**：SVD UNet 前向调用了 `npu::npu_geglu`（torch_npu 2.9.0 注入的 NPU 专用融合 GEGLU），CPU trace 报 `NotImplementedError: Could not run 'npu::npu_geglu' from the 'CPU' backend`，导出阶段即失败（image_encoder 可导出，unet 失败）。纯 `F.gelu` 的 GEGLU 在 CPU 正常，说明该 npu 算子为 SVD 特定代码路径触发，非全局。**解除阻塞需**：在不含 torch_npu/ATB 注入的纯净 torch 环境中导出，或对 SVD UNet 的 GEGLU 做 CPU 回退 monkeypatch。image_encoder/VAE 结构与 MSLite 骨架已就绪。

# Stable-Video-Diffusion-Img2Vid-XT ONNX 导出与 MindSpore Lite 推理部署教程

本教程介绍如何将 `stabilityai/stable-video-diffusion-img2vid-xt`（SVD-XT，图生视频扩散模型）导出为 ONNX，转换为 MindSpore Lite MindIR，并在昇腾（Atlas 300I Duo / 310P3）上完成端到端图生视频推理与精度对齐。

SVD-XT 由三部分组成：CLIP-ViT-H 图像编码器（`image_encoder`，projection_dim=1024，输出 `image_embeds` 作为 UNet 交叉注意力条件）、时空 UNet（`UNetSpatioTemporalConditionModel`，去噪器，`in_channels=8`、`cross_attention_dim=1024`）、4 通道时序 VAE 解码器（`AutoencoderKLTemporalDecoder`，含沿时间轴的 Conv3d）。本教程将这三部分**全部导出为 MindIR** 并在昇腾上推理（CLIP 在 dev1，UNet+VAE 在 dev0）。VAE **编码器**仅在 CPU torch 上运行一次（对条件帧编码得到 `image_latents`），不导出；`EulerDiscreteScheduler` 也在 CPU 上运行。

固定配置：576x1024 分辨率（16:9），25 帧（SVD-XT 默认 `unet.config.num_frames`），latent 尺寸 72x128。
...
python -c "from modelscope import snapshot_download; \
  print(snapshot_download('AI-ModelScope/stable-video-diffusion-img2vid-xt', \
    cache_dir='/home/yf/modelscope_cache'))"
```

将其软链到 `./stable-video-diffusion-img2vid-xt`：

```bash
ln -sfn /home/yf/modelscope_cache/AI-ModelScope/stable-video-diffusion-img2vid-xt \
...
```bash
cd ./mindspore-lite/examples/base_models/svd_img2vid_xt

python export_svd_img2vid_xt_onnx.py \
  --model-id ./stable-video-diffusion-img2vid-xt \
  --output-dir ./svd_img2vid_xt_onnx \
  --resolution 576 1024 \
  --num-frames 25 \
  --dtype fp32
```
...
| 参数 | 说明 | 默认值 |
|---|---|---|
| `--model-id` | 本地 diffusers 权重目录 | `./stable-video-diffusion-img2vid-xt` |
| `--output-dir` | ONNX 输出目录 | `./svd_img2vid_xt_onnx` |
...
$CONV --fmk=ONNX --modelFile=./svd_img2vid_xt_onnx/svd_image_encoder.onnx \
  --outputFile=./svd_img2vid_xt_onnx/svd_image_encoder \
  --optimize=ascend_oriented --saveType=MINDIR \
  --configFile=./configs/svd_image_encoder.config

# unet
$CONV --fmk=ONNX --modelFile=./svd_img2vid_xt_onnx/svd_unet.onnx \
  --outputFile=./svd_img2vid_xt_onnx/svd_unet \
  --optimize=ascend_oriented --saveType=MINDIR \
  --configFile=./configs/svd_unet.config

# vae_decoder
$CONV --fmk=ONNX --modelFile=./svd_img2vid_xt_onnx/svd_vae_decoder.onnx \
  --outputFile=./svd_img2vid_xt_onnx/svd_vae_decoder \
  --optimize=ascend_oriented --saveType=MINDIR \
...
python infer_svd_img2vid_xt_mslite.py \
  --mindir-dir   ./svd_img2vid_xt_onnx \
  --model-dir    ./stable-video-diffusion-img2vid-xt \
...
提供 `align_svd_img2vid_xt.py`，在固定条件图像/seed/latent 下对 HF(CPU fp32) 与 MindIR(Ascend fp16) 做逐帧端到端比对：用 seeded torch generator 生成共享初始 latents [1,25,4,72,128]，分别喂给 HF `StableVideoDiffusionPipeline`（CPU）与 MSLite 推理，比较输出视频的逐帧与聚合 `max_abs`、`mean_abs`、`PSNR`。

```bash
python align_svd_img2vid_xt.py \
  --mindir-dir ./svd_img2vid_xt_onnx \
  --model-dir  ./stable-video-diffusion-img2vid-xt \
  --image      ./conditioning.jpg \
  --seed 0 --steps 25 --fps 7 \
...
- [SVD-XT 模型页（HuggingFace）](https://huggingface.co/stabilityai/stable-video-diffusion-img2vid-xt)
...
SVD-XT 遵循 [Stability AI Community License](https://huggingface.co/stabilityai/stable-video-diffusion-img2vid-xt/blob/main/LICENSE.md)。本教程遵循相应依赖的许可证要求。
