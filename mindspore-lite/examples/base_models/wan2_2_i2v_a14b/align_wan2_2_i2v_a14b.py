"""End-to-end accuracy alignment for Wan2.2-I2V-A14B: HF diffusers vs MindSpore Lite.

Generates the SAME video twice from identical inputs (prompt, conditioning
image, fixed initial latents, scheduler settings, MoE boundary_ratio):

  1. HF ``WanImageToVideoPipeline`` on CPU (ground truth, float32). The pipeline
     is constructed with ``expand_timesteps=True`` AND ``boundary_ratio`` set so
     it follows the Wan2.2-A14B schedule (per-token timesteps, first-frame
     conditioning, two-expert MoE switching at boundary_ratio).
  2. MindSpore Lite pipeline on Ascend (infer_wan2_2_i2v_a14b_mslite).
...
from infer_wan2_2_i2v_a14b_mslite import WanI2VMoEInferencer, _DEFAULT_BOUNDARY_RATIO
...
def _hf_baseline(model_dir, image, prompt, negative_prompt, latents, height, width,
                 ...
                 boundary_ratio):
    """Run the HF WanImageToVideoPipeline (expand_timesteps + MoE) on CPU.

    Returns video frames in [0,1] with shape (F, H, W, 3). ``boundary_ratio``
    activates the two-expert MoE path (``transformer`` for high-noise steps,
    ``transformer_2`` for low-noise steps), matching the Wan2.2-A14B schedule.
    """
    pipe = WanImageToVideoPipeline.from_pretrained(model_dir, torch_dtype=torch.float32)
    # Wan2.2 I2V-A14B uses the expand_timesteps schedule AND the two-expert MoE
    # (boundary_ratio). Both are normally set from the pipeline config.json; we
    # set them explicitly for clarity and CLI override.
    pipe.config.expand_timesteps = True
    pipe.config.boundary_ratio = boundary_ratio
    out = pipe(
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Wan2.2-I2V-A14B HF vs MSLite alignment")
    ...
    parser.add_argument("--boundary-ratio", type=float, default=_DEFAULT_BOUNDARY_RATIO,
                        help="MoE expert switching threshold (must match inferencer)")
    ...
    print("[align] running HF baseline (WanImageToVideoPipeline, expand_timesteps + MoE) on CPU ...")
    hf = _hf_baseline(args.model_dir, image, args.prompt, args.negative_prompt, latents,
                      ...
                      args.guidance_scale, args.max_seq_len, args.boundary_ratio)
