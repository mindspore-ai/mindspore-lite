"""Wan2.2-I2V-A14B image-to-video MoE inference with MindSpore Lite (Ascend).

Loads the five converted MindIR sub-models (UMT5 text encoder, CLIP image
encoder, two Wan DiT experts, Wan VAE decoder), encodes the prompt + the
...
Ascend) with **Wan2.2-A14B two-expert MoE switching**, and decodes the latent
to a video (VAE on Ascend).

Model inference (text / CLIP / both transformers / VAE) is pure
``mindspore_lite`` + ``numpy``. ``torch`` is imported ONLY for the
(numpy-backed) diffusers UniPC scheduler that runs on CPU, exactly as in the
wan2_1_t2v example; no torch tensor ever touches the Ascend models.

Wan2.2-I2V-A14B MoE = two dense experts switched at a fixed timestep threshold
(``boundary_ratio * num_train_timesteps``), matching diffusers'
``pipeline_wan_i2v.py`` with ``boundary_ratio`` set:

  * ``t >= boundary_timestep``  -> high-noise expert (``wan_transformer_high_noise``)
  * ``t <  boundary_timestep``  -> low-noise expert  (``wan_transformer_low_noise``)

The switching is a Python-level branch over the scheduler timesteps; both
experts are dense ``WanTransformer3DModel`` graphs (no per-token routing inside
the traced graph).

Wan2.2 expand_timesteps / mask-mix 16-channel conditioning details handled in
numpy (mirrors diffusers' ``pipeline_wan_i2v.py`` with ``expand_timesteps=True``):
...
text/clip encoders -> dev1, both transformer experts + VAE -> dev0 (experts
share one device since they run at different timesteps, never concurrently).
"""
...
# Wan2.2-A14B MoE boundary: the SNR-based timestep threshold below which the
# low-noise expert takes over. diffusers' WanImageToVideoPipeline reads this
# from the pipeline config.json; 0.25 is the value shipped with the
# Wan2.2-I2V-A14B-Diffusers repo (matches the Wan2.2 paper ``t_moe`` point at
# half of SNR_min). It can be overridden via --boundary-ratio.
_DEFAULT_BOUNDARY_RATIO = 0.25
...
class WanI2VMoEInferencer:
    """End-to-end Wan2.2-I2V-A14B image-to-video MoE inferencer over MindSpore Lite."""
    ...
    def __init__(self, mindir_dir, model_dir, text_device=1, clip_device=1,
                 ...
                 guidance_scale=5.0, boundary_ratio=_DEFAULT_BOUNDARY_RATIO):
        """Load sub-models, tokenizers, scheduler, VAE stats and MoE boundary.

        ``boundary_ratio`` is the Wan2.2-A14B two-expert switching threshold as
        a fraction of ``num_train_timesteps``; timesteps ``t >= boundary`` use
        the high-noise expert, ``t < boundary`` use the low-noise expert.
        """
        ...
        # Two dense MoE experts (high-noise / low-noise); both live on the same
        # Ascend device because they run at disjoint timestep ranges.
        self.transformer_high = _build_model(
            mindir_dir / "wan_transformer_high_noise_graph.mindir", transformer_device)
        self.transformer_low = _build_model(
            mindir_dir / "wan_transformer_low_noise_graph.mindir", transformer_device)
    ...
    def _select_expert(self, t):
        """Return the active MoE expert model for timestep ``t``.

        Wan2.2-A14B MoE switching: ``t >= boundary_timestep`` -> high-noise
        expert; ``t < boundary_timestep`` -> low-noise expert. This is a dense
        Python branch (whole-expert activation per timestep), not per-token
        routing, so it does not touch the traced graph.
        """
        boundary_timestep = self.boundary_ratio * self.scheduler.config.num_train_timesteps
        return self.transformer_high if float(t) >= boundary_timestep else self.transformer_low

    def _denoise(self, latents, condition, prompt_embeds, negative_embeds,
                 image_embeds):
        """Run the CFG flow-matching denoising loop with MoE expert switching.

        Wan2.2 I2V keeps the CLIP image embedding fixed for both the cond and
        uncond branches (only the text embed differs), matching the HF pipeline.
        The active expert is chosen per-timestep from ``boundary_ratio``; within
        a step both the cond and uncond passes use the SAME expert (the
        timestep determines the expert, not the CFG branch).
        """
        ...
        for t in timesteps:
            ...
            expert = self._select_expert(t)
            noise_cond = _run_model(expert,
            ...
            noise_uncond = _run_model(expert,
                                      {"hidden_states": latent_model_input,
                                       "timestep": timestep,
    ...
    def generate(self, prompt, image, negative_prompt, output_path, seed=42,
                 latents_npy=None):
        """Run the full I2V-MoE pipeline with stage timing and save the video.
...
def main():
    """Parse arguments and run Wan2.2-I2V-A14B image-to-video MoE inference."""
    parser = argparse.ArgumentParser(description="Wan2.2-I2V-A14B MindSpore Lite inference")
    parser.add_argument("--mindir-dir", required=True, help="dir with the 5 *_graph.mindir")
    parser.add_argument("--model-dir", required=True, help="Wan2.2-I2V-A14B weights dir")
    ...
    parser.add_argument("--boundary-ratio", type=float, default=_DEFAULT_BOUNDARY_RATIO,
                        help="MoE expert switching threshold (fraction of num_train_timesteps)")
    ...
    inferencer = WanI2VMoEInferencer(
        ...
        guidance_scale=args.guidance_scale, boundary_ratio=args.boundary_ratio)
    ...
    print(f"  Transformer avg/step:         {timing['transformer_avg_step_ms']:.2f} ms "
          f"({args.num_inference_steps} steps, CFG x2, MoE two-expert)")
