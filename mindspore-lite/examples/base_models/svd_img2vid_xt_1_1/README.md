# Stable-Video-Diffusion-Img2Vid-XT-1-1 ONNX 导出与 MindSpore Lite 推理部署教程

本教程介绍如何将 `stabilityai/stable-video-diffusion-img2vid-xt-1-1`（SVD-XT-1-1，图生视频扩散模型）导出为 ONNX，转换为 MindSpore Lite MindIR，并在昇腾（Atlas 300I Duo / 310P3）上完成端到端图生视频推理与精度对齐。

SVD-XT-1-1 是 SVD-XT 的**继续训练精修版本**（continued-training refinement），与 SVD-XT **架构完全相同**（相同的 `UNetSpatioTemporalConditionModel` + `AutoencoderKLTemporalDecoder` + CLIP-ViT-H 图像编码器），**pipeline 超参数也完全一致**（经 diffusers 0.38.0 `StableVideoDiffusionPipeline.__call__` 源码核对），仅**检查点权重不同**（运动平滑度与画面质量改进）。因此本教程的导出切分、I/O shape、默认条件（576x1024 / 25 帧 / fps=7→内部 fps-1=6 / motion_bucket_id=127 / noise_aug=0.02 / min-max guidance 1.0→3.0）与 SVD-XT 教程完全一致，唯一差别是指向 `img2vid-xt-1-1` 权重目录。

SVD-XT-1-1 由三部分组成：CLIP-ViT-H 图像编码器（`image_encoder`，projection_dim=1024，输出 `image_embeds` 作为 UNet 交叉注意力条件）、时空 UNet（`UNetSpatioTemporalConditionModel`，去噪器，`in_channels=8`、`cross_attention_dim=1024`）、4 通道时序 VAE 解码器（`AutoencoderKLTemporalDecoder`，含沿时间轴的 Conv3d）。本教程将这三部分**全部导出为 MindIR** 并在昇腾上推理（CLIP 在 dev1，UNet+VAE 在 dev0）。VAE **编码器**仅在 CPU torch 上运行一次（对条件帧编码得到 `image_latents`），不导出；`EulerDiscreteScheduler` 也在 CPU 上运行。

固定配置：576x1024 分辨率（16:9），25 帧（SVD-XT-1-1 默认 `unet.config.num_frames`），latent 尺寸 72x128。
...
python -c "from modelscope import snapshot_download; \
  print(snapshot_download('AI-ModelScope/stable-video-diffusion-img2vid-xt-1-1', \
    cache_dir='/home/yf/modelscope_cache'))"
```

将其软链到 `./stable-video-diffusion-img2vid-xt-1-1`：

```bash
ln -sfn /home/yf/modelscope_cache/AI-ModelScope/stable-video-diffusion-img2vid-xt-1-1 \
...
导出脚本将 SVD-XT-1-1 拆为三个 ONNX 子图（与 SVD-XT 相同）：
...
- **fps 处理**：SVD 训练时按 `fps-1` 微调（参见 diffusers 源码注释），故 pipeline 在构造 `added_time_ids` 前先 `fps = fps - 1`（默认 `fps=7` → 用 6）。`added_time_ids = [fps-1, motion_bucket_id, noise_aug_strength]`，经 `add_time_proj`（256 维正弦编码 ×3 = 768）+ `add_embedding` 线性投影，与时间嵌入相加。SVD-XT-1-1 的 fps 默认值（7）与 SVD-XT 相同。
...
SVD-XT-1-1 与 SVD-XT 在**架构、导出切分、I/O shape、默认超参数上完全一致**，唯一差别是**检查点权重**（继续训练得到的精修权重，运动更平滑、画面质量更高）。`added_time_ids` 默认值（fps-1=6, motion_bucket_id=127, noise_aug=0.02）与 SVD-XT 相同——这一点已在 diffusers 0.38.0 `StableVideoDiffusionPipeline.__call__` 的签名默认值中核实（`fps=7, motion_bucket_id=127, noise_aug_strength=0.02, min/max_guidance_scale=1.0/3.0, num_inference_steps=25`）。如需更高运动量，可调高 `--motion-bucket-id`（如 127→180）；如需更长视频，可调 `--fps`，但导出/推理两端的 fps 必须一致。

### 自定义算子策略

SVD-XT-1-1 UNet 的 `AttnProcessor2_0` 将 q/k/v 投影后 reshape 为 `(batch, num_heads, seq, head_dim)`（BNSD 布局），再调用 `torch.nn.functional.scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False)`。脚本 monkeypatch 该函数，将 SDPA 调用替换为 CANN `PromptFlashAttention` Custom 节点（q/k/v **已是 BNSD，无需转置**、`sparse_mode=0`、不传 atten_mask，对时空自注意力和单 token 交叉注意力均成立——两者都是全双向无 mask）。其余（q/k/v 投影、group-norm、resnet、时空 transformer block 的 time embedding 注入、残差、输出投影）均走标准 diffusers 算子 trace。

### 导出命令

```bash
cd ./mindspore-lite/examples/base_models/svd_img2vid_xt_1_1

python export_svd_img2vid_xt_1_1_onnx.py \
  --model-id ./stable-video-diffusion-img2vid-xt-1-1 \
  --output-dir ./svd_img2vid_xt_1_1_onnx \
  --resolution 576 1024 \
  --num-frames 25 \
  --dtype fp32
```
...
| 参数 | 说明 | 默认值 |
|---|---|---|
| `--model-id` | 本地 diffusers 权重目录 | `./stable-video-diffusion-img2vid-xt-1-1` |
| `--output-dir` | ONNX 输出目录 | `./svd_img2vid_xt_1_1_onnx` |
...
| num_frames | 25（SVD-XT-1-1 默认，= unet.config.num_frames） |
...
| added_time_ids | 3：(fps-1=6, motion_bucket_id=127, noise_aug_strength=0.02) |
...
> 每个 transformer block 含 1 个自注意力（沿帧 token）+ 1 个交叉注意力（对 1 个 CLIP image token），数量取决于 `transformer_layers_per_block`。SVD-XT-1-1 与 SVD-XT 的 transformer block 配置相同。
...
$CONV --fmk=ONNX --modelFile=./svd_img2vid_xt_1_1_onnx/svd_image_encoder.onnx \
  --outputFile=./svd_img2vid_xt_1_1_onnx/svd_image_encoder \
  --optimize=ascend_oriented --saveType=MINDIR \
  --configFile=./configs/svd_image_encoder.config

# unet
$CONV --fmk=ONNX --modelFile=./svd_img2vid_xt_1_1_onnx/svd_unet.onnx \
  --outputFile=./svd_img2vid_xt_1_1_onnx/svd_unet \
  --optimize=ascend_oriented --saveType=MINDIR \
  --configFile=./configs/svd_unet.config

# vae_decoder
$CONV --fmk=ONNX --modelFile=./svd_img2vid_xt_1_1_onnx/svd_vae_decoder.onnx \
  --outputFile=./svd_img2vid_xt_1_1_onnx/svd_vae_decoder \
  --optimize=ascend_oriented --saveType=MINDIR \
...
python infer_svd_img2vid_xt_1_1_mslite.py \
  --mindir-dir   ./svd_img2vid_xt_1_1_onnx \
  --model-dir    ./stable-video-diffusion-img2vid-xt-1-1 \
...
提供 `align_svd_img2vid_xt_1_1.py`，在固定条件图像/seed/latent 下对 HF(CPU fp32) 与 MindIR(Ascend fp16) 做逐帧端到端比对：用 seeded torch generator 生成共享初始 latents [1,25,4,72,128]，分别喂给 HF `StableVideoDiffusionPipeline`（CPU）与 MSLite 推理，比较输出视频的逐帧与聚合 `max_abs`、`mean_abs`、`PSNR`。

```bash
python align_svd_img2vid_xt_1_1.py \
  --mindir-dir ./svd_img2vid_xt_1_1_onnx \
  --model-dir  ./stable-video-diffusion-img2vid-xt-1-1 \
  --image      ./conditioning.jpg \
  --seed 0 --steps 25 --fps 7 \
...
- **权重误用 SVD-XT**：SVD-XT-1-1 与 SVD-XT 架构相同但权重不同，务必确认 `--model-dir` 指向 `stable-video-diffusion-img2vid-xt-1-1`（而非 `img2vid-xt`），否则视频画面与运动风格会与预期不符（但仍能正常推理，因 shape/算子完全兼容）。
...
SVD-XT-1-1 UNet 含 4 个 down/mid/up 时空 transformer block，固定 [2,25,8,72,128] shape 在 310P3 上的图编译可能需要数十分钟，属正常现象；固定 shape 已是最快路径。
...
SVD-XT-1-1 UNet（batch=2 × 25 帧 × 8 通道）激活较大；若同时加载 CLIP 导致 OOM，确保 CLIP 在 `--image-device` 另一张卡（默认 dev1）。VAE 逐帧解码（num_frames=1）已最小化显存。

### 5) sample 通道数不对（非 8）

SVD-XT-1-1 UNet 的 `in_channels=8` = 4 噪声 + 4 VAE image_latent。pipeline 在送入 UNet 前将 `image_latents`（repeat 到 25 帧）与噪声沿**通道维（dim=2）**拼接。若未拼接或通道数错，UNet 报维度错误。推理脚本已在 `_denoise` 内完成拼接。
...
SVD-XT-1-1 与 SVD-XT 的差异主要体现在运动平滑度与小幅画面质量上；用相同 seed/条件对比两端视频可观察到 1-1 版本运动更自然。务必确认权重目录指向 `img2vid-xt-1-1`。
...
- [SVD-XT-1-1 模型页（HuggingFace）](https://huggingface.co/stabilityai/stable-video-diffusion-img2vid-xt-1-1)
...
SVD-XT-1-1 遵循 [Stability AI Community License](https://huggingface.co/stabilityai/stable-video-diffusion-img2vid-xt-1-1/blob/main/LICENSE.md)。本教程遵循相应依赖的许可证要求。
