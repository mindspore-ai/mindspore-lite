...
# ============================================================================
"""Export stabilityai/stable-video-diffusion-img2vid-xt-1-1 to ONNX for MindSpore Lite.

SVD-XT-1-1 is the refined image-to-video diffusion model (fixed 1024x576, 25
frames), a continued-training refinement of SVD-XT with the SAME architecture
and pipeline hyperparameters. The pipeline is exported as three ONNX sub-graphs
...
(the SVD-XT-1-1 default, ``unet.config.num_frames``).

Defaults are identical to SVD-XT: height/width 576/1024, num_frames 25,
fps=7 (internal fps-1=6), motion_bucket_id=127, noise_aug_strength=0.02,
min/max guidance 1.0/3.0 (verified against diffusers 0.38.0
``StableVideoDiffusionPipeline.__call__``). The 1-1 release only differs in
checkpoint weights (refined motion/quality).
"""
...
def export_unet(model_dir, output_dir, height, width, num_frames, dtype,
    ...
    # Same defaults for SVD-XT-1-1 and SVD-XT.
...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Export stable-video-diffusion-img2vid-xt-1-1 components "
                    "to ONNX")
    parser.add_argument("--model-id", type=str,
                        default="./stable-video-diffusion-img2vid-xt-1-1",
                        help="Local diffusers weights directory (or HF id).")
    parser.add_argument("--output-dir", type=str,
                        default="./svd_img2vid_xt_1_1_onnx",
    ...
    parser.add_argument("--resolution", type=int, nargs=2,
                        ...
                        help="Output video frame resolution (fixed for SVD). "
                             "SVD-XT-1-1 default is 576 1024 (16:9).")
    parser.add_argument("--num-frames", type=int, default=25,
                        help="Number of video frames (SVD-XT-1-1 default 25).")
...
def main():
    ...
    print(f"SVD-XT-1-1 ONNX export: resolution={height}x{width}, "
