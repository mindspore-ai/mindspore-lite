...
# ============================================================================
"""End-to-end accuracy alignment for SVD-XT-1-1: HF diffusers vs MindSpore Lite.

SVD-XT-1-1 shares SVD-XT's architecture and pipeline hyperparameters; only the
checkpoint differs (refined motion/quality). This script thus mirrors the
SVD-XT alignment flow.
...
initial latents, scheduler/guidance settings):
  ...
  2. MindSpore Lite pipeline on Ascend (the internals of
     ``infer_svd_img2vid_xt_1_1_mslite.SvdInferencer``, driven here so the shared
...
from infer_svd_img2vid_xt_1_1_mslite import SvdInferencer
...
def main():
    """Run HF and MSLite SVD-XT-1-1 pipelines on shared latents, report gap."""
    parser = argparse.ArgumentParser(
        description="SVD-XT-1-1 HF vs MSLite alignment")
