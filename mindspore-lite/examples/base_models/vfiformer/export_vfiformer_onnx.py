#!/usr/bin/env python3
"""Export VFIformer (Transformer 视频插帧) to ONNX。

封装 ``forward(img0, img1) -> mid_frame``。上游 dstoero/VFIformer,Transformer+注意力,
forward 签名需 Phase 2 用实际源码核对。
"""
...
class VfiFormerInterp(nn.Module):
    """VFIformer 插帧封装。"""
...
def main():
    parser = argparse.ArgumentParser(description="Export VFIformer to ONNX.")
    parser.add_argument("--repo-dir", type=str, default="./vfiformer_src")
    parser.add_argument("--model-file", type=str, default="models.VFIformer")
    parser.add_argument("--ckpt", type=str, default="./vfiformer.pth")
    parser.add_argument("--output-dir", type=str, default="./vfiformer_onnx")
    ...
    wrapper = VfiFormerInterp(net).to(args.device).eval()
    ...
    _export_one(wrapper, output_dir / "vfiformer.onnx", (d0, d1),
