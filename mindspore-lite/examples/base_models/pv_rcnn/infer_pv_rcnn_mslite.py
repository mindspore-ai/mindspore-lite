...
# distributed under the License is the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
# ============================================================================
"""PV-RCNN MindSpore Lite inference (numpy-only, no torch)."""
...
def _random_input(c, h, w):
    """fixed-seed random dense BEV feature."""
    rng = np.random.RandomState(SEED)
    return np.ascontiguousarray(rng.rand(1, c, h, w).astype(np.float32))


class PVRcnnMsLite:
    """PV-RCNN MindSpore Lite inference."""
    ...
    def forward(self, bev_feat):
        """run forward."""
        inputs = [mslite.Tensor(np.ascontiguousarray(bev_feat))]
...
def parse_args():
    """parse args"""
    parser = argparse.ArgumentParser(description="PV-RCNN MindSpore Lite inference")
...
def main():
    ...
    net = PVRcnnMsLite(args.model, args.device, args.device_id)
    bev = _random_input(args.in_channels, args.feat_h, args.feat_w)
    print(f"Using random dense BEV, shape={bev.shape}, seed={SEED}")
    ...
    outs = net.forward(bev)
    for name, out in zip(["cls_scores", "bbox_preds", "dir_cls"], outs):
