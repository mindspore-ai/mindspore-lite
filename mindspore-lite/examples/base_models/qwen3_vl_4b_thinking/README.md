# Qwen3-VL-4B-Thinking ONNX 导出与 MindSpore Lite 推理

本目录提供 Qwen3-VL-4B-Thinking 导出为 ONNX 以及基于 MindSpore Lite 在昇腾 Ascend 上端到端推理的完整脚本。

## 概览

Qwen3-VL-4B-Thinking 是一个具备**思维链推理能力**的多模态大模型（Vision-Language Model），在生成回答前会先进行逐步推理（Chain-of-Thought），从而提升复杂视觉理解任务的准确度。
...
- **精度验证**：PyTorch 与 MindSpore Lite 推理结果对比验证
...
Qwen3-VL-4B-Thinking 与 Instruct 版本具有相同的模型架构，但增加了思维链推理模式：

- 模型在回答前自动生成推理过程
- 使用特殊 token 标记推理内容：`<|think_start|>`（151667）、`<|think_end|>`（151668）
- 输出结构为：`<think_start> reasoning_content <think_end> answer_content`
...
```bash
python export_qwen3_vl_4b_thinking_onnx.py \
    --model-id ./Qwen/Qwen3-VL-4B-Thinking \
    --output-dir ./qwen3_vl_4b_thinking_onnx \
    --device cpu \
...
| 参数 | 说明 | 默认值 |
|---|---|---|
| `--model-id` | 模型路径或 ModelScope ID | `./Qwen/Qwen3-VL-4B-Thinking` |
| `--output-dir` | 导出输出目录 | `./qwen3_vl_4b_thinking_onnx` |
...
| `--no-fuse-weights` | 禁用权重融合（QKV/GateUp）以避免大模型 ONNX 外部数据问题 | `False` |
...
- `image_embeds`：`float32`，形状 `(num_image_tokens, 2560)`
- `deepstack_embeds`：`float32`，形状 `(3, num_image_tokens, 2560)`
...
- `position_ids`：`int64`，形状 `(4, batch, seq_len)`（MRoPE 4D 位置编码）
...
- `present_key_values`：`float16`，形状 `(2*num_layers, batch, num_kv_heads, seq_len, head_dim)`
  以 Qwen3-VL-4B-Thinking 为例：`2*num_layers=72`、`num_kv_heads=8`、`head_dim=80`。
...
- `past_key_values`：`float16`，形状 `(72, 1, 8, 512, 80)`（固定 cache）
- `cache_pos`：`int64`，形状 `(1,)`（当前有效 cache 长度/位置）

**输出：**

- `logits`：`float16`，形状 `(1, 1, vocab_size)`
- `present_key_values`：`float16`，形状 `(72, 1, 8, 512, 80)`
...
converter_lite \
    --fmk=ONNX \
    --modelFile=./qwen3_vl_4b_thinking_onnx/qwen3_vl_vision.onnx \
    --outputFile=./qwen3_vl_4b_thinking_onnx/qwen3_vl_vision \
...
converter_lite \
    --fmk=ONNX \
    --modelFile=./qwen3_vl_4b_thinking_onnx/qwen3_vl_llm_prefill.onnx \
    --outputFile=./qwen3_vl_4b_thinking_onnx/qwen3_vl_llm_prefill \
    --optimize=ascend_oriented \
    --saveType=MINDIR \
    --configFile=./configs/config.ini
...
converter_lite \
    --fmk=ONNX \
    --modelFile=./qwen3_vl_4b_thinking_onnx/qwen3_vl_llm_decode.onnx \
    --outputFile=./qwen3_vl_4b_thinking_onnx/qwen3_vl_llm_decode \
    --optimize=ascend_oriented \
    --saveType=MINDIR \
    --configFile=./configs/config.ini
```
...
| `--processor` | Processor 模型路径或目录 | `./Qwen/Qwen3-VL-4B-Thinking` |
...
测试模型：Qwen3-VL-4B-Thinking
测试条件：输入图片（128×128）+ 文本 prompt，输出 374 token
...
| Vision | 12.595 |
| Prefill | 193.599 |
| Total Decode | 22911.685 |
| Decode 步数 | 374 |
| **Avg decode step** | **61.261** |
| E2E | 24264.704 |
| **吞吐量 (tok/s)** | **15.42** |
...
------------------------------------------------------------
Thinking: So, let's describe this image. First, there are two people sitting on a
beach at sunset. The background has the ocean and the sky with warm colors, like
orange and pink, which is typical of a sunset...
------------------------------------------------------------
Response: This image captures a serene moment at sunset on a beach. Two people are
seated on the wet sand, facing each other, with the ocean stretching out behind
them. The sky is painted with warm hues of orange and pink...
```
...
- 推理内容通过 `<|think_start|>` 和 `<|think_end|>` 特殊 token 分隔
...
Qwen3-VL 使用多模态旋转位置编码（M-RoPE），通过 4 维 position_ids 实现空间-时间联合位置表示：

- 维度 0：文本序列位置
- 维度 1-3：多模态（时间、高度、宽度）位置
...
- 使用 `--no-fuse-weights` 禁用权重融合以减少外部数据
- 关闭其它占用内存的程序
...
### 精度验证报错 mm_token_type_ids 缺失

请确保使用 transformers >= 5.9.0 版本，该版本支持 `mm_token_type_ids` 参数。

## 参考链接

- [MindSpore Lite Ascend 推理](https://www.mindspore.cn/lite/)
- [Qwen3-VL GitHub](https://github.com/QwenLM/Qwen3-VL)
- [Qwen3-VL-4B-Thinking ModelScope](https://modelscope.cn/models/Qwen/Qwen3-VL-4B-Thinking)
- [Qwen3-VL-4B-Thinking HuggingFace](https://huggingface.co/Qwen/Qwen3-VL-4B-Thinking)
- [ONNX Runtime 文档](https://onnxruntime.ai/docs/)

## 许可证

本工具遵循 Qwen3-VL 模型的许可证要求，详见 [Qwen3-VL license](https://huggingface.co/Qwen/Qwen3-VL-4B-Thinking)。
