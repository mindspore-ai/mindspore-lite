...
"""
Infer Qwen2.5-VL-3B-Instruct on Ascend with MindSpore Lite.
"""
...
def _get_vision_position_ids(start_position, grid_thw, spatial_merge_size, device):
    """
    Get vision position ids for Qwen2.5-VL-3B-Instruct.
...
def _get_rope_index(
    ...
    """
    Get rope index for Qwen2.5-VL-3B-Instruct.
...
def _build_position_ids(
    ...
    """
    Build position ids for Qwen2.5-VL-3B-Instruct.

    Args:
        cfg (Qwen2_5_VLConfig): Qwen2.5-VL-3B-Instruct configuration.
        input_ids (torch.Tensor): Input ids.
...
def _build_mslite_inputs(model: mslite.Model, feed_dict, preferred_order=None):
    """
    Build MindSpore Lite inputs for Qwen2.5-VL-3B-Instruct.
...
class Qwen2_5VLInferencer:
    """
    Qwen2.5-VL-3B-Instruct inferencer.
    """
    ...
    def __init__(
        ...
        """
        Initialize Qwen2.5-VL-3B-Instruct inferencer.
        """
    ...
    def _ascend_device_str(self):
        return f"ascend:{int(self.device_id)}"
    ...
    def _prepare_inputs(self, image_path_or_url: str, prompt: str):
        """
        Prepare inputs for Qwen2.5-VL-3B-Instruct.
    ...
    def _run_vision_encoder(self, pixel_values_np, image_grid_thw_np):
        """Run vision model and return (image_embeds, elapsed_ms).

        For Qwen2.5-VL, the vision model only outputs image_embeds (1 output).
        deepstack_embeds is created as empty tensor since num_deepstack=0.
        """
        ...
        # Qwen2.5-VL vision model only outputs image_embeds.
        # deepstack_embeds is empty (num_deepstack=0).
        num_img_tokens = image_embeds.shape[0]
        hidden_size = image_embeds.shape[1]
        deepstack_embeds = np.zeros((0, num_img_tokens, hidden_size), dtype=np.float16)
    ...
    def infer(
        ...
        """
        Infer Qwen2.5-VL-3B-Instruct on MindSpore Lite.
        """
        ...
        # Prepare prefill inputs and pad to alignment boundary for PromptFlashAttention
        input_ids_np = input_ids.cpu().numpy().astype(np.int32)
        attn_mask_np = attention_mask.cpu().numpy().astype(np.int32)
        pos_ids_np = position_ids_4.cpu().numpy().astype(np.int32)
        orig_seq_len = input_ids_np.shape[1]
        # Align to 128 tokens for Ascend PromptFlashAttention compatibility
        PREFILL_ALIGN = 128
        padded_len = ((orig_seq_len + PREFILL_ALIGN - 1) // PREFILL_ALIGN) * PREFILL_ALIGN
        if padded_len > orig_seq_len:
            pad_len = padded_len - orig_seq_len
            input_ids_pad = np.pad(input_ids_np, ((0, 0), (0, pad_len)), constant_values=0)
            attn_mask_pad = np.pad(attn_mask_np, ((0, 0), (0, pad_len)), constant_values=0)
            pos_ids_pad = np.pad(pos_ids_np, ((0, 0), (0, 0), (0, pad_len)), constant_values=0)
        else:
            input_ids_pad = input_ids_np
            attn_mask_pad = attn_mask_np
            pos_ids_pad = pos_ids_np

        prefill_feed = {
            "input_ids": input_ids_pad,
            "attention_mask": attn_mask_pad,
            "position_ids": pos_ids_pad,
        ...
        print(
            ...
            f"decode_total={t_decode_ms:.3f} decode_steps={decode_steps} decode_avg={avg_decode_ms:.3f} "
            f"e2e={t_e2e_ms:.3f}"
        )
...
def main():
    """
    Main function for Qwen2.5-VL-3B-Instruct inference on Ascend with MindSpore Lite.
    """
    parser = argparse.ArgumentParser(
        description="Qwen2.5-VL-3B-Instruct MindSpore Lite inference (vision + prefill + decode)"
    )
    parser.add_argument(
        "--vision-model", type=str, required=True, help="Path to qwen2_5_vl_vision.mindir"
    )
    parser.add_argument(
        "--prefill-model",
        type=str,
        required=True,
        help="Path to qwen2_5_vl_llm_prefill.mindir",
    )
    parser.add_argument(
        "--decode-model",
        type=str,
        required=True,
        help="Path to qwen2_5_vl_llm_decode.mindir",
    )
    parser.add_argument(
        "--processor",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="128",
        help="Force processor image size (must match exported vision model). Example: 128 or 512x320 or 512,320",
    )
    parser.add_argument(
        "--no-pad-to-square",
        action="store_true",
    ...
    inferencer = Qwen2_5VLInferencer(
