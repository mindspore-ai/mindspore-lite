...
"""
Export Qwen2.5-VL-3B-Instruct to ONNX.
"""
...
try:
    from transformers.models.qwen2_5_vl.modeling_qwen2_5_vl import apply_rotary_pos_emb
except Exception:
...
def _merge_mrope(cos, sin, mrope_section):
    """Merge 3D mRoPE cos/sin into single tensors for Qwen2.5-VL.

    Args:
        cos: (3, batch, seq_len, head_dim) - multimodal rotary cos
        sin: (3, batch, seq_len, head_dim) - multimodal rotary sin
        mrope_section: [16, 24, 24] - frequencies per spatial dimension

    Returns:
        cos_merged: (batch, seq_len, head_dim)
        sin_merged: (batch, seq_len, head_dim)
    """
...
class VisionTowerWrapper(torch.nn.Module):
    """
    Wrapper for Qwen2.5-VL Vision Tower to cache position embeddings.
    """

    def __init__(self, vision_tower, dummy_grid_thw):
        """
        Initialize Qwen2.5-VL Vision Tower wrapper.
        """
        super().__init__()
        self.vision_tower = vision_tower
        self.dummy_grid_thw = dummy_grid_thw
        with torch.no_grad():
            # Qwen2.5-VL uses rotary_pos_emb for position encoding
            from transformers.models.qwen2_5_vl.modeling_qwen2_5_vl import get_vision_position_ids
            position_ids = get_vision_position_ids(dummy_grid_thw, vision_tower.spatial_merge_size)
            cached_rotary_pos_emb = vision_tower.rotary_pos_emb(position_ids)
        # Cache the rotary position embeddings - use object.__setattr__ to bypass
        # PyTorch's __setattr__ guard which rejects non-Module assignments
        vision_tower._cached_rotary_pos_emb = cached_rotary_pos_emb
        object.__setattr__(
            vision_tower, 'rotary_pos_emb',
            lambda x: vision_tower._cached_rotary_pos_emb
        )

    def forward(self, pixel_values):
        """
        Forward pass for Qwen2.5-VL Vision Tower wrapper.
        """
        outputs = self.vision_tower(
            pixel_values, grid_thw=self.dummy_grid_thw, return_dict=True
        )
        # Qwen2.5-VL outputs pooler_output (image embeddings)
...
def export_vision_tower(model, output_path, device="cpu", vision_image_size=128):
    """
    Export Qwen2.5-VL Vision Tower to ONNX.
    """
    print(f"Exporting Vision Tower to {output_path}...")

    # Qwen2.5-VL uses a visual encoder (usually model.model.visual)
    ...
    raw_grid_h = int(vision_image_size) // int(patch_size)
    raw_grid_w = int(vision_image_size) // int(patch_size)
    # Ensure grid is divisible by spatial_merge_size for valid position ids
    grid_h = (raw_grid_h // spatial_merge_size) * spatial_merge_size
    grid_w = (raw_grid_w // spatial_merge_size) * spatial_merge_size
    if grid_h != raw_grid_h or grid_w != raw_grid_w:
        print(f"Adjusted grid from ({raw_grid_h},{raw_grid_w}) to ({grid_h},{grid_w}) "
              f"to be divisible by spatial_merge_size={spatial_merge_size}")
    ...
    dummy_pixel_values = torch.randn(
        dummy_seq_len, 1176, device=device, dtype=torch.float16
    )
...
class LLMWrapper(torch.nn.Module):
    """
    Wrapper for Qwen2.5-VL-3B-Instruct LLM to cache position embeddings.
    """

    def __init__(self, llm):
        """
        Initialize Qwen2.5-VL-3B-Instruct LLM wrapper.
        """
        super().__init__()
        self.llm = llm
    ...
    def forward(self, input_ids, attention_mask, position_ids):
        """
        Forward pass for Qwen2.5-VL-3B-Instruct LLM decode.
        """
...
def _make_additive_causal_mask(attention_mask, q_len, k_len, past_len, dtype):
    """
    Create additive causal mask for Qwen2.5-VL-3B-Instruct text model.
    """
...
def _text_position_ids(position_ids, batch, seq_len, device):
    """
    Process position ids for Qwen2.5-VL-3B-Instruct text model.
    """
...
def _text_attn_forward(
    ...
    """
    Forward pass for Qwen2.5-VL-3B-Instruct text attention (prefill).
    """
    ...
    if qkv is None:
        ...
        q, k, v = qkv
    # Qwen2.5-VL does not have q_norm/k_norm (those are Qwen3-VL specific)
    has_qk_norm = hasattr(attn_mod, 'q_norm') and hasattr(attn_mod, 'k_norm')
    if has_qk_norm:
    ...
    if _USE_CUSTOM_OP:
        ...
        if past_key is None:
            # Use PromptFlashAttention for prefill (handles GQA internally)
            attn_output = _CustomPromptFlashAttention.apply(
                query_states,
                key_states,
                value_states,
                atten_mask,
            ...
            # Decode step: expand GQA KV and use IncreFlashAttention
        ...
        if int(num_kv_heads) != int(num_heads):
            repeat = int(num_heads) // int(num_kv_heads)
            key_exp = key_states[:, :, None, :, :].expand(
                key_states.shape[0], int(num_kv_heads), repeat, key_states.shape[2], head_dim
            ).reshape(key_states.shape[0], int(num_heads), key_states.shape[2], head_dim)
            val_exp = value_states[:, :, None, :, :].expand(
                value_states.shape[0], int(num_kv_heads), repeat, value_states.shape[2], head_dim
            ).reshape(value_states.shape[0], int(num_heads), value_states.shape[2], head_dim)
        else:
            key_exp = key_states
            val_exp = value_states
        scores = torch.matmul(query_states, key_exp.transpose(-2, -1)) * float(scaling)
        if attention_mask is not None:
            scores = scores.masked_fill(attention_mask.to(torch.bool), torch.finfo(scores.dtype).min)
        probs = torch.softmax(scores, dim=-1, dtype=torch.float32).to(query_states.dtype)
        attn_output = torch.matmul(probs, val_exp)
...
def _text_attn_forward_decode_fixed(
    ...
    if _USE_CUSTOM_OP:
        ...
        if int(num_kv_heads) != int(num_heads):
            repeat = int(num_heads) // int(num_kv_heads)
            key_exp = key_cache[:, :, None, :, :].expand(
                key_cache.shape[0], int(num_kv_heads), repeat, key_cache.shape[2], head_dim
            ).reshape(key_cache.shape[0], int(num_heads), key_cache.shape[2], head_dim)
            val_exp = value_cache[:, :, None, :, :].expand(
                value_cache.shape[0], int(num_kv_heads), repeat, value_cache.shape[2], head_dim
            ).reshape(value_cache.shape[0], int(num_heads), value_cache.shape[2], head_dim)
        else:
            key_exp = key_cache
            val_exp = value_cache
        scores = torch.matmul(query_states, key_exp.transpose(-2, -1)) * float(scaling)
...
class Qwen2_5VLLlmPrefill(torch.nn.Module):
    """
    Qwen2.5-VL-3B-Instruct LLM prefill model.
    """

    def __init__(self, text_model, lm_head, image_token_id: int, num_deepstack: int):
        """
        Initialize Qwen2.5-VL-3B-Instruct LLM prefill model.
        """
        super().__init__()
    ...
    def forward(
        ...
        """
        Forward pass for Qwen2.5-VL-3B-Instruct LLM prefill.
        """
        ...
        if _USE_CUSTOM_OP:
            # Merge 3D mRoPE cos/sin into single tensor for Qwen2.5-VL
            mrope_section = self.text_model.config.rope_parameters.get(
                "mrope_section", [16, 24, 24]
            )
            cos, sin = _merge_mrope(cos, sin, mrope_section)
...
class Qwen2_5VLLlmDecode(torch.nn.Module):
    """
    Qwen2.5-VL-3B-Instruct LLM decode model.
    """
...
def _get_llm_export_meta(model):
    """
    Get metadata for Qwen2.5-VL-3B-Instruct LLM export.
    """
...
def _prepare_llm_modules(model, device):
    """
    Prepare Qwen2.5-VL-3B-Instruct LLM modules for export.
    """
...
def _build_llm_wrappers(
    ...
    """
    Build Qwen2.5-VL-3B-Instruct LLM wrappers for export.
    """
    prefill = (
        Qwen2_5VLLlmPrefill(
            text_model,
    ...
    decode = (
        Qwen2_5VLLlmDecode(text_model, lm_head, max_seq_len=int(max_seq_len))
        .to(device)
        .eval()
    )
...
def _make_prefill_dummy_inputs(
    ...
    """
    Create dummy inputs for Qwen2.5-VL-3B-Instruct LLM prefill.
    """
...
def _make_decode_dummy_inputs(
    ...
    """
    Create dummy inputs for Qwen2.5-VL-3B-Instruct LLM decode.
    """
...
def _export_onnx(
    ...
    """
    Export Qwen2.5-VL-3B-Instruct LLM prefill or decode model to ONNX.
    """
...
def export_llm_prefill_decode(model, output_dir, device="cpu", kv_cache_len: int = 512):
    """
    Export Qwen2.5-VL-3B-Instruct LLM prefill and decode models to ONNX.
    """
    (
        text_model,
        lm_head,
    ...
    prefill_path = Path(output_dir) / "qwen2_5_vl_llm_prefill.onnx"
    decode_path = Path(output_dir) / "qwen2_5_vl_llm_decode.onnx"
...
def _export_vision_step(model_id, output_dir, device, vision_image_size):
    """
    Export Qwen2.5-VL-3B-Instruct vision tower model to ONNX.
    """
    ...
    try:
        ...
        pass
    _clear_torch_cache()
    vision_path = Path(output_dir) / "qwen2_5_vl_vision.onnx"
...
def main():
    """
    Main function for Qwen2.5-VL-3B-Instruct export to ONNX.
    """
    parser = argparse.ArgumentParser(description="Export Qwen2.5-VL-3B-Instruct to ONNX")
    parser.add_argument(
        "--model-id",
        type=str,
    ...
    parser.add_argument(
        "--output-dir", type=str, default="./qwen2_5_vl_3b_instruct_onnx", help="Output directory"
    )
    parser.add_argument(
