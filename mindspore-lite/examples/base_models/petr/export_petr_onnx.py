...
# ============================================================================
"""Export PETR (position embedding transformation 3D detection) to ONNX via mmdetection3d."""
...
def parse_args():
    """parse args"""
    parser = argparse.ArgumentParser(description="Export PETR to ONNX.")
    parser.add_argument("--config", type=str, required=True)
    parser.add_argument("--checkpoint", type=str, required=True)
    parser.add_argument("--output", type=str, default="petr_onnx/petr.onnx")
...
class PETRWrapper(nn.Module):
    """PETR wrapper: multi-view images -> query-level cls + box predictions."""
...
def build_petr(cfg_path, ckpt_path, device):
    """build PETR model from mmdet3d config."""
...
def main():
    ...
    model = build_petr(args.config, args.checkpoint, args.device)
    wrapper = PETRWrapper(model).to(args.device).eval()

    dummy = torch.randn(1, args.ncams, 3, args.img_h, args.img_w, device=args.device,
                        dtype=torch.float32)
    print(f"Exporting PETR ONNX, input shape={tuple(dummy.shape)}")
    with torch.no_grad():
        torch.onnx.export(
