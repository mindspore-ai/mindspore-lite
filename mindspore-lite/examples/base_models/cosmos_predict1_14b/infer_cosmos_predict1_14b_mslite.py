#!/usr/bin/env python3
"""Cosmos-Predict1 MindSpore Lite inference (MindIR): cond -> video latent via DDPM."""
...
def _parse_args():
    p = argparse.ArgumentParser(description="Cosmos-Predict1 MindSpore Lite inference (DDPM)")
...
def main():
    ...
    def step(x, t, c):
        outs = _run_model(model, inputs, {"noisy_latent": x, "timestep": t, "cond": c})
    ...
    print("Perf:")
    print(f"  ddpm_steps: {args.num_steps}  num_tokens: {args.num_tokens}  latent_dim: {args.latent_dim}  device: {args.device}")
