...
# ============================================================================
"""LaneGCN MindSpore Lite inference (numpy-only, no torch)."""
...
def _random_inputs(obs_len, agent_feat, lane_num, lane_feat):
    """fixed-seed random (actor_hist, lane_nodes)."""
    rng = np.random.RandomState(SEED)
    actor = np.ascontiguousarray(rng.rand(1, obs_len, agent_feat).astype(np.float32))
    lanes = np.ascontiguousarray(rng.rand(1, lane_num, lane_feat).astype(np.float32))
    return actor, lanes


class LaneGCNMsLite:
    """LaneGCN MindSpore Lite inference."""
    ...
    def forward(self, actor_hist, lane_nodes):
        """run forward."""
        inputs = [mslite.Tensor(np.ascontiguousarray(actor_hist)),
                  mslite.Tensor(np.ascontiguousarray(lane_nodes))]
...
def parse_args():
    """parse args"""
    parser = argparse.ArgumentParser(description="LaneGCN MindSpore Lite inference")
...
def main():
    ...
    net = LaneGCNMsLite(args.model, args.device, args.device_id)
