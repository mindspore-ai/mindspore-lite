# DUF-28L ONNX 模型导出与 MindSpore Lite 推理部署教程

本教程介绍如何将 **DUF-28L**(Deep Video Super-Resolution,3D 卷积 + 动态上采样滤波)导出为单 ONNX,经 ONNX Runtime 验证后转换为 MindIR,在 **Atlas 300I Duo(310P3)** 上推理测速。输入 7 帧低清,输出 4x 超分中心帧。
...
```bash
cd mindspore-lite/examples/base_models/duf_28l
python export_duf_28l_onnx.py \
  --repo-dir ./duf_src --model-file model.DUF --ckpt ./DUF-28L.pth \
  --output-dir ./duf_28l_onnx --device cpu \
  --num-frames 7 --lr-height 64 --lr-width 64 --depth 28
```
...
```text
./duf_28l_onnx/
└── duf_28l.onnx   # 输入 lr_seq [1,7,3,64,64],输出 sr_frame [1,3,256,256]
```
...
```bash
python infer_duf_28l_onnx.py --onnx ./duf_28l_onnx/duf_28l.onnx \
...
```bash
converter_lite --fmk=ONNX --modelFile=./duf_28l_onnx/duf_28l.onnx \
  --outputFile=./duf_28l_onnx/duf_28l --optimize=ascend_oriented \
...
```bash
python infer_duf_28l_mslite.py --mindir ./duf_28l_onnx/duf_28l.mindir \
