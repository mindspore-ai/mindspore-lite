#!/usr/bin/env python3
"""TOFlow ONNXRuntime 推理脚本(精度基准 ground truth): 多帧低清 -> 4x 超分。"""
...
class Duf28lOnnxInferencer:
    """TOFlow ONNXRuntime 推理封装。"""
...
def _parse_args():
    p = argparse.ArgumentParser(description="TOFlow ONNXRuntime 推理")
    p.add_argument("--onnx", type=str, required=True)
    p.add_argument("--input", type=str, required=True, help="低清帧图像")
    p.add_argument("--output", type=str, default="./duf_28l_sr_onnx.png")
...
def main():
    ...
    infer = Duf28lOnnxInferencer(args.onnx, device=args.device)
