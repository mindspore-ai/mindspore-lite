#!/usr/bin/env python3
"""Export IconVSR (CVPR2021 视频超分) to ONNX。

IconVSR 用双向循环传播 + SpyNet 光流对齐,4x 超分。固定 N 帧输入,trace 时展开循环。
...
class IconVsrSR(nn.Module):
    """IconVSR 超分封装:N 帧低清 -> N 帧 4x 超分(双向循环在 trace 时展开)。"""
...
def main():
    parser = argparse.ArgumentParser(description="Export IconVSR to ONNX.")
    parser.add_argument("--repo-dir", type=str, default="./mmagic_src",
                        help="mmagic 或上游源码目录(用于 import 架构类)")
    parser.add_argument("--model-file", type=str, default="mmagic.models.iconvsr_net.IconVSRNet")
    parser.add_argument("--ckpt", type=str, default="./iconvsr_x4.pth")
    parser.add_argument("--output-dir", type=str, default="./iconvsr_onnx")
    ...
    wrapper = IconVsrSR(net).to(args.device).eval()
    lr = torch.randn(1, args.num_frames, 3, args.lr_height, args.lr_width,
                     dtype=torch.float32, device=args.device)
    _export_one(wrapper, output_dir / "iconvsr.onnx", (lr,),
