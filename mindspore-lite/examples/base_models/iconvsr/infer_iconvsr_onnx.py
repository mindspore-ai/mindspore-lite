#!/usr/bin/env python3
"""IconVSR ONNXRuntime 推理脚本(精度基准): N 帧低清 -> N 帧 4x 超分(取中心帧保存)。"""
...
class IconVsrOnnxInferencer:
    """IconVSR ONNXRuntime 推理封装。"""
...
def _parse_args():
    p = argparse.ArgumentParser(description="IconVSR ONNXRuntime 推理")
    p.add_argument("--onnx", type=str, required=True)
    p.add_argument("--input", type=str, required=True, help="低清帧图像")
    p.add_argument("--output", type=str, default="./iconvsr_sr_onnx.png")
...
def main():
    ...
    infer = IconVsrOnnxInferencer(args.onnx, device=args.device)
