# IconVSR ONNX 模型导出与 MindSpore Lite 推理部署教程

本教程介绍如何将 **IconVSR**(CVPR2021 视频超分)导出为单 ONNX,经 ONNX Runtime 验证后转换为 MindIR,在 **Atlas 300I Duo(310P3)** 上推理测速。IconVSR 用双向循环传播 + SpyNet 光流对齐,4x 超分。固定 N 帧输入,trace 时展开循环(无显式 recurrent 状态 I/O)。
...
```bash
cd mindspore-lite/examples/base_models/iconvsr
python export_iconvsr_onnx.py \
  --repo-dir ./mmagic_src \
  --model-file mmagic.models.iconvsr_net.IconVSRNet \
  --ckpt ./iconvsr_x4.pth \
  --output-dir ./iconvsr_onnx --device cpu \
...
| `--model-file` | 架构类 dotted 路径 | `mmagic.models.iconvsr_net.IconVSRNet` |
| `--ckpt` | 权重 | `./iconvsr_x4.pth` |
| `--num-frames` | 固定帧数(展开循环) | `10` |
| `--lr-height/--lr-width` | 低清尺寸(须被 4 整除) | `64` / `64` |

```text
./iconvsr_onnx/
└── iconvsr.onnx   # 输入 lr_seq [1,10,3,64,64],输出 sr_seq [1,10,3,256,256]
```
...
```bash
python infer_iconvsr_onnx.py --onnx ./iconvsr_onnx/iconvsr.onnx \
...
```bash
converter_lite --fmk=ONNX --modelFile=./iconvsr_onnx/iconvsr.onnx \
  --outputFile=./iconvsr_onnx/iconvsr --optimize=ascend_oriented \
...
```bash
python infer_iconvsr_mslite.py --mindir ./iconvsr_onnx/iconvsr.mindir \
...
1. 无法导入 IconVSRNet → 确认 mmagic 已装或 `--repo-dir` 正确,`--model-file` 路径匹配版本。
