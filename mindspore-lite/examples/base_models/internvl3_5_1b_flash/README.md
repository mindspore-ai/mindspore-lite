# InternVL3.5-1B-Flash ONNX 模型导出与 MindSpore Lite 推理部署教程

本教程介绍如何将 [OpenGVLab/InternVL3_5-1B-Flash](https://www.modelscope.cn/models/OpenGVLab/InternVL3_5-1B-Flash) 视觉语言模型按结构拆分导出为 ONNX，转换为 MindSpore Lite MindIR，并在 **Ascend Atlas 300I Duo** 上完成图文推理与精度对齐。
...
> InternVL3.5-1B-Flash 与标准 1B 架构完全一致（Qwen3-0.6B 主干：hidden=1024/28层/16头/8KV/head_dim=128），脚本/配置/转换流程与 `internvl3_5_1b` 相同，仅模型 id 与权重不同；故本教程沿用 1B 说明，性能数据为 Flash 实测。
---
...
```bash
python -c "from modelscope import snapshot_download as s; print(s('OpenGVLab/InternVL3_5-1B-Flash', cache_dir='/home/yf/modelscope_cache'))"
ln -sfn /home/yf/modelscope_cache/OpenGVLab/InternVL3_5-1B-Flash ./InternVL3_5-1B-Flash
```
...
```bash
cd mindspore-lite/examples/base_models/internvl3_5_1b_flash
HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 python export_internvl3_5_1b_flash_onnx.py \
  --model-id ./InternVL3_5-1B-Flash --output-dir ./internvl3_5_1b_flash_onnx \
...
| 参数 | 说明 | 默认值 |
| --- | --- | --- |
| `--model-id` | 本地权重目录或 ModelScope id | `./InternVL3_5-1B-Flash` |
| `--output-dir` | ONNX / embed 输出目录 | `./internvl3_5_1b_flash_onnx` |
...
[export] saved internvl3_5_1b_flash_onnx/internvl_vision.onnx
[export] llm prefill ...
...
[export] embed_weights (151936, 1024) -> internvl3_5_1b_flash_onnx/embed_weights.npy
```
...
$CONV --fmk=ONNX --modelFile=./internvl3_5_1b_flash_onnx/internvl_vision.onnx      --outputFile=./internvl3_5_1b_flash_onnx/internvl_vision      --optimize=ascend_oriented --saveType=MINDIR --configFile=./configs/internvl_vision.config
$CONV --fmk=ONNX --modelFile=./internvl3_5_1b_flash_onnx/internvl_llm_prefill.onnx --outputFile=./internvl3_5_1b_flash_onnx/internvl_llm_prefill --optimize=ascend_oriented --saveType=MINDIR --configFile=./configs/internvl_llm_prefill.config
$CONV --fmk=ONNX --modelFile=./internvl3_5_1b_flash_onnx/internvl_llm_decode.onnx  --outputFile=./internvl3_5_1b_flash_onnx/internvl_llm_decode  --optimize=ascend_oriented --saveType=MINDIR --configFile=./configs/internvl_llm_decode.config
```
...
```bash
HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 python infer_internvl3_5_1b_flash_mslite.py \
  --mindir-dir ./internvl3_5_1b_flash_onnx --model-dir ./InternVL3_5-1B-Flash \
...
--- Performance ---
  Vision encode:   90.58 ms
  LLM prefill:     493.86 ms (seq=320)
  LLM decode:      18360.19 ms (94 steps)
  End-to-end:      18944.64 ms
  Avg decode step: 195.32 ms
  Throughput:      5.12 tok/s (decode)
```
...
| **端到端（128 tok 上限）** | **~18.9 s** |
| **decode 吞吐** | **5.12 tok/s** |
...
HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 python align_internvl3_5_1b_flash.py \
  --model-dir ./InternVL3_5-1B-Flash --image ./test.jpg --dump-dir ./align_dump --max-new-tokens 32
# 推理环境（mindspore-lite）：MSLite prefill 首 token 余弦对齐
python align_internvl3_5_1b_flash.py --mindir-dir ./internvl3_5_1b_flash_onnx --dump-dir ./align_dump --check-mslite
```
...
- 上游：<https://github.com/OpenGVLab/InternVL>、ModelScope `OpenGVLab/InternVL3_5-1B-Flash`
