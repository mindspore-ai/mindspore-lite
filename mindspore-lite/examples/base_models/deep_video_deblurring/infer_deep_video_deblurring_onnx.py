#!/usr/bin/env python3
"""DEEP_VIDEO_DEBLURRING ONNXRuntime 推理脚本(精度基准 ground truth)。

demo: 读单帧复制成 7 帧序列, 输出增强帧。
"""
...
class DeepVideoDeblurringOnnxInferencer:
    """DEEP_VIDEO_DEBLURRING ONNXRuntime 推理封装。"""
    ...
    def forward(self, seq):
        return self.sess.run(None, {"seq": seq})[0]


def _parse_args():
    p = argparse.ArgumentParser(description="DEEP_VIDEO_DEBLURRING ONNXRuntime 推理")
    p.add_argument("--onnx", type=str, required=True)
    p.add_argument("--input", type=str, required=True, help="低质量帧图像")
    p.add_argument("--output", type=str, default="./deep_video_deblurring_enhanced_onnx.png")
...
def main():
    ...
    infer = DeepVideoDeblurringOnnxInferencer(args.onnx, device=args.device)
