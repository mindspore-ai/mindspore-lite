#!/usr/bin/env python3
"""DEEP_VIDEO_DEBLURRING MindSpore Lite 推理脚本(MindIR)。
...
def _check_memory_budget(device_id, limit_pct=80):
    ...
    if over:
        print("[memory-budget] !!! 超出 80% 预算: " + "; ".join(over) + "。请释放内存或降帧/降分辨率。")
        return False
    return True
...
class DeepVideoDeblurringMsliteInferencer:
    """DEEP_VIDEO_DEBLURRING MindSpore Lite 推理封装。"""
    ...
    def forward(self, seq):
        return self.model.predict([mslite.Tensor(seq)])[0].get_data_to_numpy()


def _parse_args():
    p = argparse.ArgumentParser(description="DEEP_VIDEO_DEBLURRING MindSpore Lite 推理(MindIR)")
    p.add_argument("--mindir", type=str, required=True)
    p.add_argument("--input", type=str, required=True, help="低质量帧图像")
    p.add_argument("--output", type=str, default="./deep_video_deblurring_enhanced_mslite.png")
...
def main():
    ...
    infer = DeepVideoDeblurringMsliteInferencer(args.mindir, device=args.device, device_id=args.device_id)
