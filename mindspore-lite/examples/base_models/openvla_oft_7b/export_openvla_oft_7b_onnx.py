#!/usr/bin/env python3
"""Export an OpenVLA-OFT-style policy (vision+language -> action chunk) to ONNX.

OpenVLA-OFT / OFT+ (Stanford) is an optimized fine-tune of OpenVLA emitting
*streaming action tokens* (an action-flow head) for higher accuracy and longer
action chunks. The full model is a VLM (Llama-2 backbone) with prefill/decode.

This script provides the same self-contained PyTorch regression skeleton as the
OpenVLA-7B example (image + task tokens -> action chunk, single forward) so the
migration pipeline runs end-to-end. Wire the real OFT weights via the official
prismatic/transformers path with a prefill/decode split in task-2.
"""
...
class OpenVLAOFTPolicy(nn.Module):
    """Simplified OpenVLA-OFT regression policy (streaming action chunk)."""
    ...
    def __init__(self, img_size=224, patch=16, dim=384, depth=4, heads=6,
        ...
        # OFT action-flow head: linear projection over readout tokens.
...
def _parse_args():
    p = argparse.ArgumentParser(description="Export an OpenVLA-OFT-style policy to ONNX.")
    p.add_argument("--output-dir", type=str, default="./openvla_oft_7b_onnx")
    p.add_argument("--device", type=str, default="cpu", choices=["cpu", "cuda"])
    p.add_argument("--opset", type=int, default=17)
    p.add_argument("--checkpoint", type=str, default="", help="OFT state_dict (task-2: use prismatic/transformers).")
...
def main():
    ...
    if args.checkpoint:
        ...
        print(f"random-init demo OpenVLA-OFT skeleton (seed={args.seed}). "
              f"Real OFT = prismatic VLM + streaming action-flow head (task-2).")
    ...
    onnx_path = out / "openvla_oft_7b_policy.onnx"
    with torch.no_grad():
        torch.onnx.export(
