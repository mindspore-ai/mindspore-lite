...
def _build_model(mindir_path, device_id):
    ...
    ``mindir_path`` may point at either ``<name>_graph.mindir`` (weights
    externalised into a sibling ``<name>_variables/`` dir, e.g. the transformer)
    or a single ``<name>.mindir`` (weights inlined, e.g. the VAE). Resolve to
    whichever exists.
    """
    mindir_path = Path(mindir_path)
    candidates = [mindir_path, mindir_path.with_name(mindir_path.stem.replace("_graph", "") + ".mindir")]
    chosen = next((p for p in candidates if p.exists()), mindir_path)
    context = mslite.Context()
    context.target = ["ascend"]
    context.ascend.device_id = int(device_id)
    model = mslite.Model()
    model.build_from_file(str(chosen), mslite.ModelType.MINDIR, context)
    return model
...
def _np_to_input(tensor_info, array):
    ...
    ``tensor_info.dtype`` is a ``mindspore_lite.DataType`` enum (the converter casts
    the exported int64 input_ids/attention_mask to int32), so map via the enum
    rather than ``np.dtype``.
    """
    dtype_map = {mslite.DataType.FLOAT16: np.float16, mslite.DataType.FLOAT32: np.float32,
                 mslite.DataType.INT32: np.int32, mslite.DataType.INT64: np.int64,
                 mslite.DataType.BOOL: np.bool_}
    target = dtype_map.get(tensor_info.dtype)
...
class WanT2VInferencer:
    ...
    def __init__(self, mindir_dir, model_dir, text_device=1, transformer_device=0,
                 ...
                 max_seq_len=512, num_inference_steps=50, guidance_scale=5.0, vae_clip=0.0):
    ...
    def _encode_prompt(self, prompt):
        """Tokenize + run the UMT5 text encoder -> [1, max_seq_len, 4096] embeds.

        Matches diffusers' WanPipeline._get_t5_prompt_embeds: after encoding, the
        padding positions (beyond the actual sequence length) are ZEROED. The UMT5
        encoder produces non-zero values even for padded tokens; without zeroing, these
        garbage values corrupt the transformer's cross-attention and the denoise loop
        diverges (latent std blows up 5x, VAE decodes to a flat grid).
        """
        ...
        # Zero out padding positions (match WanPipeline._get_t5_prompt_embeds).
    ...
    def _vae_decode(self, latents):
        ...
        diffusers WanPipeline normalises as ``latents * std + mean`` (it stores the
        reciprocal, ``latents_std = 1.0 / config.latents_std``, then divides). Using
        ``latents / std`` (the naive guess) is wrong and decodes to a meaningless
        grid texture.

        The denoised latents are also clamped to ``[-4, 4]`` before denormalisation:
        on Atlas 300I Duo (310P3) the transformer runs at fp16-effective precision
        even with ``force_fp32`` (edge chip), so the CFG-amplified denoise loop
        drifts the latents to a larger std (~2.3) than the fp32 HF baseline (~1.2).
        The out-of-range tail drives the VAE out of distribution and it collapses to
        a flat, single-colour-per-channel image. Clamping to the VAE's expected
        latent range restores a structured (beach) decode. Pass ``--vae-clip 0`` to
        disable.
        """
        if self.vae_clip > 0:
            latents = np.clip(latents, -self.vae_clip, self.vae_clip)
...
def main():
    ...
    parser.add_argument("--vae-clip", type=float, default=0.0,
                        help="clamp denoised latents to [-vae_clip, vae_clip] before VAE decode "
                             "(mitigates 310P3 fp16 latent drift); 0 disables.")
