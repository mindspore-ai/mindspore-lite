#!/usr/bin/env python3
"""Export AMT-S (All-Pairs Multi-Field Transforms, Small) to ONNX.
...
def _load_model(repo_dir, ckpt, model_file, device):
    ...
    try:
        import importlib
        mod = importlib.import_module(pkg)  # noqa: WPS433 (支持连字符模块名 AMT-S)
...
def main():
    parser = argparse.ArgumentParser(description="Export AMT-S to ONNX.")
    parser.add_argument("--repo-dir", type=str, default="./amt_src")
    parser.add_argument("--model-file", type=str, default="networks.AMT-S.Model")
    parser.add_argument("--ckpt", type=str, default="./amt-s.pth")
    parser.add_argument("--output-dir", type=str, default="./amt_s_onnx")
    ...
    wrapper = AmtSInterp(net).to(args.device).eval()
    ...
    _export_one(wrapper, output_dir / "amt_s.onnx", (d0, d1),
