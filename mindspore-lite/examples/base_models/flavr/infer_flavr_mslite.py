#!/usr/bin/env python3
"""FLAVR MindSpore Lite 推理脚本(MindIR)。
...
class FlavrMsliteInferencer:
    """FLAVR MindSpore Lite 推理封装。"""
...
def _parse_args():
    p = argparse.ArgumentParser(description="FLAVR MindSpore Lite 推理(MindIR)")
    p.add_argument("--mindir", type=str, required=True)
    p.add_argument("--input", type=str, required=True, help="低质量帧图像")
    p.add_argument("--output", type=str, default="./flavr_interp_mslite.png")
...
def main():
    ...
    infer = FlavrMsliteInferencer(args.mindir, device=args.device, device_id=args.device_id)
