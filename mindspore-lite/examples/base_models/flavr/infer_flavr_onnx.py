#!/usr/bin/env python3
"""FLAVR ONNXRuntime 推理脚本(精度基准 ground truth)。
...
class FlavrOnnxInferencer:
    """FLAVR ONNXRuntime 推理封装。"""
...
def _parse_args():
    p = argparse.ArgumentParser(description="FLAVR ONNXRuntime 推理")
    p.add_argument("--onnx", type=str, required=True)
    p.add_argument("--input", type=str, required=True, help="低质量帧图像")
    p.add_argument("--output", type=str, default="./flavr_interp_onnx.png")
...
def main():
    ...
    infer = FlavrOnnxInferencer(args.onnx, device=args.device)
