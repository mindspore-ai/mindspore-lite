...
"""
Export Qwen3-VL-8B-Instruct to ONNX.
...
Qwen3-VL-8B-Instruct is the instruction-tuned variant of the Qwen3-VL-8B model,
designed for general-purpose multimodal understanding tasks.
"""
...
class LLMWrapper(torch.nn.Module):
    """
    Wrapper for Qwen3-VL-8B-Instruct LLM to cache position embeddings.
    """

    def __init__(self, llm):
        """
        Initialize Qwen3-VL-8B-Instruct LLM wrapper.
        """
        super().__init__()
        self.llm = llm
    ...
    def forward(self, input_ids, attention_mask, position_ids):
        """
        Forward pass for Qwen3-VL-8B-Instruct LLM decode.
        """
        outputs = self.llm(
            input_ids=input_ids,
...
def _make_additive_causal_mask(attention_mask, q_len, k_len, past_len, dtype):
    """
    Create additive causal mask for Qwen3-VL-8B-Instruct text model.
    """
...
def _text_position_ids(position_ids, batch, seq_len, device):
    """
    Process position ids for Qwen3-VL-8B-Instruct text model.
    """
...
def _text_attn_forward(
    ...
    """
    Forward pass for Qwen3-VL-8B-Instruct text attention.
    """
...
class Qwen3VLLlmPrefill(torch.nn.Module):
    """
    Qwen3-VL-8B-Instruct LLM prefill model.
    """

    def __init__(self, text_model, lm_head, image_token_id: int, num_deepstack: int):
        """
        Initialize Qwen3-VL-8B-Instruct LLM prefill model.
        """
        super().__init__()
    ...
    def forward(
        ...
        """
        Forward pass for Qwen3-VL-8B-Instruct LLM prefill.
        """
...
class Qwen3VLLlmDecode(torch.nn.Module):
    """
    Qwen3-VL-8B-Instruct LLM decode model.
    """
...
def _get_llm_export_meta(model):
    """
    Get metadata for Qwen3-VL-8B-Instruct LLM export.
    """
...
def _prepare_llm_modules(model, device):
    """
    Prepare Qwen3-VL-8B-Instruct LLM modules for export.
    """
...
def _build_llm_wrappers(
    ...
    """
    Build Qwen3-VL-8B-Instruct LLM wrappers for export.
    """
    prefill = (
        Qwen3VLLlmPrefill(
            text_model,
...
def _make_prefill_dummy_inputs(
    ...
    """
    Create dummy inputs for Qwen3-VL-8B-Instruct LLM prefill.
    """
...
def _make_decode_dummy_inputs(
    ...
    """
    Create dummy inputs for Qwen3-VL-8B-Instruct LLM decode.
    """
...
def _export_onnx(
    ...
    """
    Export Qwen3-VL-8B-Instruct LLM prefill or decode model to ONNX.
    """
...
def export_llm_prefill_decode(model, output_dir, device="cpu", kv_cache_len: int = 512):
    """
    Export Qwen3-VL-8B-Instruct LLM prefill and decode models to ONNX.
    """
    (
        text_model,
        lm_head,
    ...
    # Save each subgraph in its own subfolder so external-data files
    # (qwen3_vl_llm_prefill.data, qwen3_vl_llm_decode.data) do not collide
    # and conversions stay isolated.
    prefill_dir = Path(output_dir) / "prefill"
    decode_dir = Path(output_dir) / "decode"
    prefill_dir.mkdir(parents=True, exist_ok=True)
    decode_dir.mkdir(parents=True, exist_ok=True)
    prefill_path = prefill_dir / "qwen3_vl_llm_prefill.onnx"
    decode_path = decode_dir / "qwen3_vl_llm_decode.onnx"
    ...
    # Consolidate external data into a single file to avoid per-file naming issues
...
def _export_vision_step(model_id, output_dir, device, vision_image_size):
    """
    Export Qwen3-VL-8B-Instruct vision tower model to ONNX.
    """
    ...
    vision_path = vision_dir / "qwen3_vl_vision.onnx"
...
def main():
    """
    Main function for Qwen3-VL-8B-Instruct export to ONNX.
    """
    parser = argparse.ArgumentParser(description="Export Qwen3-VL-8B-Instruct to ONNX")
    parser.add_argument(
        "--model-id",
        type=str,
    ...
    parser.add_argument(
        "--output-dir", type=str, default="./qwen3_vl_8b_instruct_onnx", help="Output directory"
    )
    parser.add_argument(
