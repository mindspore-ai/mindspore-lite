#!/usr/bin/env python3
"""Export an LLaVA-1.5-style vision-language model skeleton to ONNX.

LLaVA-1.5 (CLIP-ViT vision encoder + MLP projector + Vicuna LLM) is the canonical
open VLM. The full model is autoregressive (prefill/decode); the repo already
has that split pattern (see internvl3_5_1b).

This script provides a self-contained PyTorch skeleton of one forward pass:

  - inputs : image      [1, 3, 224, 224]   float32
             input_ids  [1, seq_len]        int64     (tokenized prompt)
  - output : logits     [1, num_vis+seq_len, vocab_size]  float32

``--random-init`` runs the pipeline. Real LLaVA should be loaded via transformers
(``LlavaForConditionalGeneration``) with a prefill/decode split in task-2.
"""
...
class TransformerBlock(nn.Module):
    """Pre-norm transformer block (bidirectional skeleton; real LLM is causal)."""
...
class VisionEncoder(nn.Module):
    """CLIP-ViT-like patch encoder -> image tokens."""
...
class LlavaSkel(nn.Module):
    """LLaVA skeleton: vision encoder + projector + LLM head (single forward)."""
...
def _parse_args():
    p = argparse.ArgumentParser(description="Export an LLaVA-1.5-style VLM skeleton to ONNX.")
    p.add_argument("--output-dir", type=str, default="./llava_1_5_7b_onnx")
    p.add_argument("--device", type=str, default="cpu", choices=["cpu", "cuda"])
    p.add_argument("--opset", type=int, default=17)
    p.add_argument("--checkpoint", type=str, default="", help="LLaVA state_dict (task-2: transformers LlavaForConditionalGeneration).")
...
def main():
    ...
    if args.checkpoint:
        ...
        print(f"random-init demo LLaVA skeleton (seed={args.seed}). "
              f"Real LLaVA-1.5 = CLIP-ViT + projector + Vicuna-7B (task-2: transformers + prefill/decode).")
    ...
    onnx_path = out / "llava_1_5_7b.onnx"
    with torch.no_grad():
        torch.onnx.export(
