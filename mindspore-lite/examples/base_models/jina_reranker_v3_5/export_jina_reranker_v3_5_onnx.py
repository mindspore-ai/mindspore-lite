...
"""
Export Jina Reranker V3.5 to ONNX format (Listwise only).

This script exports the JinaForRanking model (built on Qwen3-0.6B with hybrid
sliding/full attention) with its native "last but not late interaction"
architecture. The ONNX model supports listwise (multiple docs per forward)
scoring via ``doc_token_indices`` / ``query_token_index`` extra inputs and
``torch.gather`` lookups (avoiding non-exportable boolean indexing).

V3.5 differences vs V3 (driving this implementation):
  - Qwen3 backbone with **hybrid attention**: 28 layers, 16 sliding_attention
    (sliding_window=1024) + 12 full_attention, mixed per ``layer_types``
    config.
  - Default ``torch_dtype=bfloat16`` (V3 was bfloat16 too but weights end up
    different). We force float32 at load to match V3 export discipline
    (MindSpore Lite's Clip parser does not accept FLOAT16).
  - Same prompt template, same special tokens, same projector output dim.
...
The fused Custom operators include:
  ...
  - PromptFlashAttention: QK^T+softmax+V        -> Custom(PromptFlashAttention)
    (full_attention layers only; sliding layers fall back to SDPA so the
    sliding window constraint is honored)
...
class _CannPromptFlashAttention(torch.autograd.Function):
    ...
    Supports three input layouts: BSH (3D), BSND (4D), BNSD (4D). The forward
    (eager compute used for tracing) and symbolic both honour ``input_layout``
    so that the runtime tensor shape and the Custom op's ``input_layout_s``
    attribute agree.
    """

    @staticmethod
    def forward(ctx, query, key, value, atten_mask, num_heads, num_key_value_heads, scale_value,
                sparse_mode=0, pre_tokens=0, next_tokens=0, input_layout="BSH"):
        """Run PromptFlashAttention reference implementation for tracing.

        ``input_layout`` selects how the q/k/v tensors are laid out:
          - ``"BSH"``  -> q/k/v shape ``[B, S, H_q/H_kv]`` (3D, H embeds N*D).
          - ``"BSND"`` -> q/k/v shape ``[B, S, N, D]`` (4D).
          - ``"BNSD"`` -> q/k/v shape ``[B, N, S, D]`` (4D).
        Output is returned in the same layout as ``query``.
        """
        del ctx
        _ = (sparse_mode, pre_tokens, next_tokens)
        nh = int(num_heads)
        nkv = int(num_key_value_heads)
        if input_layout == "BSH":
            B, S, H_q = query.shape
            head_dim = H_q // nh
            q = query.view(B, S, nh, head_dim).transpose(1, 2)
            k = key.view(B, S, nkv, head_dim).transpose(1, 2)
            v = value.view(B, S, nkv, head_dim).transpose(1, 2)
            ...
            raise ValueError(f"_CannPromptFlashAttention.forward: unsupported input_layout={input_layout!r}")
    ...
    def symbolic(g, query, key, value, atten_mask, num_heads, num_key_value_heads, scale_value,
        ...
        if int(sparse_mode) != 0 or int(pre_tokens) != 0 or int(next_tokens) != 0:
            kwargs["sparse_mode_i"] = int(sparse_mode)
            kwargs["pre_tokens_i"] = int(pre_tokens)
            kwargs["next_tokens_i"] = int(next_tokens)
        y = g.op("Custom", *inputs, **kwargs)
...
def _linear_2d(linear_mod, x):
    ...
    The weight is cast to the input dtype so the 3D MatMulV2 reference runs
    without dtype mismatch (attention inputs are fp16, MLP inputs are fp32).
    """
    w = linear_mod.weight.t().to(x.dtype)
    if x.dim() == 2:
        out = _CannMatMulV2.apply(x, w)
        if linear_mod.bias is not None:
            out = out + linear_mod.bias.to(x.dtype)
        return out

    orig_shape = x.shape
    x2d = x.reshape(-1, orig_shape[-1])
    out2d = _CannMatMulV2.apply(x2d, w)
    if linear_mod.bias is not None:
        out2d = out2d + linear_mod.bias.to(x.dtype)
...
def _make_bool_sliding_window_mask_dynamic(attention_mask, sliding_window):
    """
    Build a boolean sliding-window causal mask (True=masked) of shape (B,1,S,S).

    Each query position ``i`` may only attend to keys ``k`` in
    ``(i - sliding_window + 1, i]`` (i.e. the ``sliding_window`` most recent
    keys before and including itself). Padding tokens are masked everywhere.
    """
    batch_size = attention_mask.shape[0]
    ...
    ar = ar[0]
    causal = ar[None, :] <= ar[:, None]
    deltas = ar[:, None] - ar[None, :]
    window = (deltas >= 0) & (deltas < int(sliding_window))
    keep = causal & window
    full_keep = keep.unsqueeze(0).unsqueeze(0).expand(batch_size, 1, seq_len, seq_len)
    k_valid = attention_mask[:, None, None, :].to(torch.bool)
    q_valid = attention_mask[:, None, :, None].to(torch.bool)
    return ~(full_keep & k_valid & q_valid)
...
def _cann_attn_forward(
    ...
    ``input_layout`` selects the q/k/v layout that the PFA Custom op sees:
      - ``"BSH"``  -> 3D ``[B, S, H_q]`` (free reshape from the 4D projected tensors).
      - ``"BSND"`` -> 4D ``[B, S, N, D]`` (pass-through, free reshape back for o_proj).
      - ``"BNSD"`` -> 4D ``[B, N, S, D]`` (one contiguous permute in, one out).
    """
    del padded_len
    ...
    if input_layout == "BSH":
        query_for_pfa = query_states.reshape(*input_shape, -1)
        key_for_pfa = key_states.reshape(*input_shape, -1)
        value_for_pfa = value_states.reshape(*input_shape, -1)
    elif input_layout == "BSND":
        query_for_pfa = query_states
        key_for_pfa = key_states
        value_for_pfa = value_states
    elif input_layout == "BNSD":
        query_for_pfa = query_states.permute(0, 2, 1, 3).contiguous()
        key_for_pfa = key_states.permute(0, 2, 1, 3).contiguous()
        value_for_pfa = value_states.permute(0, 2, 1, 3).contiguous()
    else:
        raise ValueError(f"_cann_attn_forward: unsupported input_layout={input_layout!r}")
    ...
    if input_layout == "BSH":
        attn_output_flat = attn_output
    elif input_layout == "BSND":
        attn_output_flat = attn_output.reshape(*input_shape, -1)
    else:  # BNSD -> transpose N/S back to BSND-order, then flatten
        attn_output_flat = attn_output.transpose(1, 2).reshape(*input_shape, -1)

    if not _ONNX_DYNAMIC_EXPORT and int(pad_len) > 0:
        attn_output_flat = attn_output_flat[:, :seq_len, :]
    attn_output_flat = attn_output_flat.to(orig_dtype)
    attn_output_flat = _linear(attn_mod.o_proj, attn_output_flat, enable_bmm2mm_fusion)
    return attn_output_flat


def _build_sliding_window_additive_mask(attention_mask, sliding_window, dtype):
    """
    Build a (B,1,S,S) additive mask that combines causal + sliding-window + padding.

    Masked positions get ``torch.finfo(dtype).min`` (effectively -inf after softmax).
    Unmasked positions are 0. This is the mask that sliding-window attention layers
    must apply - just ``is_causal=True`` would let the query attend to keys beyond
    the window, which is wrong for layers with ``layer_type='sliding_attention'``.
    """
    ...
    window = (deltas >= 0) & (deltas < int(sliding_window))
    keep = causal & window
    keep = keep.unsqueeze(0).unsqueeze(0).expand(batch_size, 1, seq_len, seq_len)
    k_valid = attention_mask[:, None, None, :].to(torch.bool)
    q_valid = attention_mask[:, None, :, None].to(torch.bool)
    valid = keep & k_valid & q_valid
    mask = torch.where(
        valid,
        torch.zeros(1, dtype=dtype, device=attention_mask.device),
        torch.full((), min_val, dtype=dtype, device=attention_mask.device),
    )
    return mask.expand(batch_size, 1, seq_len, seq_len).contiguous()


def _sdpa_sliding_attn_forward(
    attn_mod,
    hidden_states,
    position_embeddings,
    sliding_window,
    ...
    """
    Reference (SDPA) attention forward for sliding-window layers.

    These layers cannot use CANN PromptFlashAttention because PFA does not
    expose the ``sliding_window`` constraint - we need a proper window mask.
    Falls back to ``torch.nn.functional.scaled_dot_product_attention`` which
    handles the mask natively.

    Runs in fp16 (like the PFA layers) so the sliding layers get the same
    mix-precision speedup instead of staying in fp32.
    """
    input_shape = hidden_states.shape[:-1]
    orig_dtype = hidden_states.dtype
    head_dim = attn_mod.head_dim
    ...
    cos, sin = position_embeddings
    cos, sin = _expand_rotary_cos_sin(cos, sin, len(hidden_shape))
    query_states, key_states = _cann_apply_rotary_pos_emb(query_states, key_states, cos, sin)
    ...
    q_4d = query_states.transpose(1, 2)
    k_4d = key_states.transpose(1, 2)
    v_4d = value_states.transpose(1, 2)
    attn_mask = _build_sliding_window_additive_mask(
        attention_mask, int(sliding_window), q_4d.dtype
    )
    attn_output = torch.nn.functional.scaled_dot_product_attention(
        q_4d, k_4d, v_4d, attn_mask=attn_mask, dropout_p=0.0, is_causal=False
    )
...
class JinaRerankerV3_5ListwiseWrapper(torch.nn.Module):
    """
    Wrapper for Jina Reranker V3.5 listwise ONNX export (no fusion).

    Forwards through the inner Qwen3Model so that the official mask machinery
    (full + sliding window, dict-keyed by ``layer_types``) is applied per layer.
    The lm_head weight is intentionally not exported - we only need the last
    hidden state, not the logits.
    """
    ...
    def forward(self, input_ids, attention_mask, doc_token_indices, query_token_index):
        """Compute listwise scores with the official non-fused backbone."""
        ...
        if sliding_window is not None:
            sliding_mask = _make_bool_sliding_window_mask_dynamic(
                attention_mask, int(sliding_window)
            )
            sliding_mask_add = torch.where(
                sliding_mask,
                torch.full_like(attn_mask, torch.finfo(attn_mask.dtype).min),
                torch.zeros_like(attn_mask),
            ).to(attn_mask.dtype)
            causal_mask_mapping["sliding_attention"] = sliding_mask_add
...
class JinaRerankerV3_5FusedWrapper(torch.nn.Module):
    """
    Wrapper for Jina Reranker V3.5 listwise ONNX export with CANN fused ops.

    Manually unrolls the Qwen3Model forward so that each full-attention
    layer goes through PromptFlashAttention Custom op (with a padded causal
    mask) and each sliding-attention layer uses reference SDPA (since PFA
    does not honour ``sliding_window``). RotaryMul and Add+RmsNorm are also
    replaced by their CANN Custom equivalents.

    MLP activation: defaults to direct silu(gate)*up (no cat) which avoids
    FRACTAL_NZ→ND TransData; pass ``enable_swiglu_fusion=True`` for an
    A/B comparison against cat([gate, up]) + Custom(SwiGlu).
    """
    ...
    def __init__(
        ...
        enable_sliding_pfa=False,
        enable_sliding_sparse=False,
        pfa_layout="BSH",
    ):
        super().__init__()
        ...
        self.enable_sliding_pfa = bool(enable_sliding_pfa)
        self.enable_sliding_sparse = bool(enable_sliding_sparse)
        if pfa_layout not in ("BSH", "BSND", "BNSD"):
            raise ValueError(f"pfa_layout must be one of BSH/BSND/BNSD, got {pfa_layout!r}")
        self.pfa_layout = pfa_layout
        layer_types = getattr(model.model.config, "layer_types", None)
        if layer_types is None:
            n = len(self.layers)
            layer_types = ["full_attention"] * n
        self.layer_types = list(layer_types)
        self.sliding_window = int(getattr(model.model.config, "sliding_window", 0) or 0)
    ...
    def forward(self, input_ids, attention_mask, doc_token_indices, query_token_index):
        ...
        sliding_bool_mask = None
        if self.enable_sliding_pfa:
            sliding_bool_mask = _make_bool_sliding_window_mask_dynamic(
                attention_mask, self.sliding_window
            )
        for i, layer in enumerate(self.layers):
            layer_type = self.layer_types[i] if i < len(self.layer_types) else "full_attention"
            if layer_type == "full_attention":
                attn_out = _cann_attn_forward(
                    ...
                    pad_len,
                    padded_len,
                    input_layout=self.pfa_layout,
                )
            elif self.enable_sliding_pfa:
                if self.enable_sliding_sparse:
                    # The kernel band (sparse_mode=1 + pre_tokens) fully encodes
                    # causal+window, so no mask input is exported. This assumes
                    # right-padded inputs (see infer script); padding after the
                    # content is already excluded by causality.
                    sliding_mask = None
                    sliding_kwargs = {
                        "sparse_mode": 1,
                        "pre_tokens": int(self.sliding_window),
                        "next_tokens": 0,
                    }
                else:
                    sliding_mask = sliding_bool_mask
                    sliding_kwargs = {}
                attn_out = _cann_attn_forward(
                    layer.self_attn,
                    hidden_states,
                ...
                attn_out = _sdpa_sliding_attn_forward(
                    layer.self_attn,
                    hidden_states,
                    position_embeddings,
                    self.sliding_window,
...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Export Jina Reranker V3.5 (Listwise only) to ONNX"
    )
    parser.add_argument(
        "--model-id",
        type=str,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Enable BMM->MMv2 optimization: route Linear through 2D MatMulV2 Custom op. "
            "Ablation on 300I DUO (CANN 8.5) shows NO gain (+3.2ms vs baseline fused), "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Enable RmsNorm/AddRmsNorm fusion Custom ops. "
            "Ablation shows a +55ms regression on 300I DUO; experimental only."
        ),
    )
    parser.add_argument(
        "--enable_qk_merge",
        ...
        help=(
            ...
            "Ablation shows no measurable gain on 300I DUO; keep disabled for production."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Enable SwiGlu fusion via cat([gate,up]) + Custom(SwiGlu). "
            "Ablation shows an +8ms regression on 300I DUO; experimental only."
        ),
    )
    parser.add_argument(
        "--enable_sliding_pfa",
        dest="enable_sliding_pfa",
        action="store_true",
        default=True,
        help=(
            "Route sliding-attention layers through the PromptFlashAttention Custom op "
            "with a banded sliding-window bool mask instead of reference SDPA. "
            "Enabled by default; use --disable_sliding_pfa to turn off."
        ),
    )
    parser.add_argument(
        "--disable_sliding_pfa",
        dest="enable_sliding_pfa",
        action="store_false",
        help="Disable sliding-attention PFA routing (fall back to reference SDPA).",
    )
    parser.add_argument(
        "--enable_sliding_sparse",
        action="store_true",
        help=(
            "Additionally set sparse_mode/pre_tokens/next_tokens on sliding PFA layers "
            "so the kernel can skip fully-masked tiles outside the sliding window band."
        ),
    )
    parser.add_argument(
        "--pfa_layout",
        type=str,
        default="BSH",
        choices=("BSH", "BSND", "BNSD"),
        help=(
            "Tensor layout fed to the PromptFlashAttention Custom op. "
            "BSH=3D[B,S,H] (default, free reshape), BSND=4D[B,S,N,D] (free pass-through), "
            "BNSD=4D[B,N,S,D] (one contiguous permute per q/k/v, plus one reshape out)."
        ),
    )
...
def main():
    ...
    if use_fusion:
        ...
        wrapper = JinaRerankerV3_5FusedWrapper(
            model,
            ...
            enable_sliding_pfa=bool(getattr(args, "enable_sliding_pfa", False)),
            enable_sliding_sparse=bool(getattr(args, "enable_sliding_sparse", False)),
            pfa_layout=str(getattr(args, "pfa_layout", "BSH")),
        ).to(args.device).eval()
    else:
        print("Preparing model for export (no fusion)...")
        wrapper = JinaRerankerV3_5ListwiseWrapper(model).to(args.device).eval()
    ...
    output_path = os.path.join(args.output_dir, "jina_reranker_v3_5.onnx")
    ...
    onnx_path = os.path.join(args.output_dir, "jina_reranker_v3_5.onnx")
    ...
    onnx.save_model(
        ...
        location="jina_reranker_v3_5.onnx.data",
        size_threshold=1024,
