#!/usr/bin/env python3
"""Export AMT-L (All-Pairs Multi-Field Transforms, Small) to ONNX.
...
def _load_model(repo_dir, ckpt, model_file, device):
    ...
    try:
        import importlib
        mod = importlib.import_module(pkg)  # noqa: WPS433 (支持连字符模块名 AMT-L)
...
def main():
    parser = argparse.ArgumentParser(description="Export AMT-L to ONNX.")
    parser.add_argument("--repo-dir", type=str, default="./amt_lrc")
    parser.add_argument("--model-file", type=str, default="networks.AMT-L.Model")
    parser.add_argument("--ckpt", type=str, default="./amt_l.pth")
    parser.add_argument("--output-dir", type=str, default="./amt_l_onnx")
    ...
    wrapper = AmtLInterp(net).to(args.device).eval()
    ...
    _export_one(wrapper, output_dir / "amt_l.onnx", (d0, d1),
