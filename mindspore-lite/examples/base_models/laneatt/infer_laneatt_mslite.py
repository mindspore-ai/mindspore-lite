...
# ============================================================================
"""LaneATT MindSpore Lite inference (numpy-only, no torch)."""
...
def _random_input(h, w):
    """fixed-seed random image input."""
    rng = np.random.RandomState(SEED)
    return np.ascontiguousarray(rng.rand(1, 3, h, w).astype(np.float32))


class LaneAttMsLite:
    """LaneATT MindSpore Lite inference."""
...
def parse_args():
    """parse args"""
    parser = argparse.ArgumentParser(description="LaneATT MindSpore Lite inference")
...
def main():
    ...
    net = LaneAttMsLite(args.model, args.device, args.device_id)
    ...
    for name, out in zip(["lane_existence", "lane_offsets"], outs):
