...
# ============================================================================
"""BEVDepth ONNX inference (random multi-view perf + output shape).

Note: the exported ONNX contains a Custom::BEVPoolV3 node, which ONNX Runtime
cannot execute. This script is for output-shape inspection only when the custom
op is registered; otherwise use the MindIR path.
"""
...
class BEVDepthOnnx:
    """BEVDepth ONNXRuntime inference."""
...
def parse_args():
    """parse args"""
    parser = argparse.ArgumentParser(description="BEVDepth ONNX inference")
...
def main():
    ...
    net = BEVDepthOnnx(args.model, args.device)
    ...
    try:
        ...
        print(f"(ONNXRuntime cannot run Custom::BEVPoolV3 op: {e})")
        print("Use the MindIR path via infer_bevdepth_mslite.py instead.")
