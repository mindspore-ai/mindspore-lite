#!/usr/bin/env python3
"""Export EDVR-L (Video Restoration, DCNv1 + 时空注意力, M=中等规模) to ONNX。
...
class EdvrLSR(nn.Module):
    """EDVR-L 超分封装(含 DCNv1 对齐)。"""
...
def main():
    parser = argparse.ArgumentParser(description="Export EDVR-L to ONNX.")
    ...
    parser.add_argument("--ckpt", type=str, default="./edvr_l_x4.pth")
    parser.add_argument("--output-dir", type=str, default="./edvr_l_onnx")
    ...
    wrapper = EdvrLSR(net).to(args.device).eval()
    lr = torch.randn(1, args.num_frames, 3, args.lr_height, args.lr_width,
                     dtype=torch.float32, device=args.device)
    _export_one(wrapper, output_dir / "edvr_l.onnx", (lr,),
