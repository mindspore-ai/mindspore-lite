#!/usr/bin/env python3
"""Diffusion Policy MindSpore Lite inference (MindIR): obs -> action chunk via DDPM.

Loads the converted single-step U-Net MindIR and runs DDPM sampling in pure
...
def _parse_args():
    p = argparse.ArgumentParser(description="Diffusion Policy MindSpore Lite inference (DDPM)")
    p.add_argument("--model", type=str, required=True, help="MindIR path (diffusion_policy_graph.mindir)")
...
def main():
    ...
    def step(x, t, o):
        outs = _run_model(model, inputs, {"noisy_action": x, "timestep": t, "obs": o})
    ...
    for _ in range(int(args.runs)):
        ...
        raw = ddpm_sample(step, obs, args.action_dim, args.horizon, args.num_steps, rng)
        lat.append((time.perf_counter() - t0) * 1000.0)
    mem_after = _memory_snapshot()

    action = np.transpose(raw, (0, 2, 1))
    print("Output:")
