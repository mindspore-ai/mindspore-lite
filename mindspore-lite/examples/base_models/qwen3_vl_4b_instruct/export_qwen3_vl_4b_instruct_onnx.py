...
def export_llm_prefill_decode(model, output_dir, device="cpu", kv_cache_len: int = 512):
    ...
    prefill_path = Path(output_dir) / "prefill" / "qwen3_vl_llm_prefill.onnx"
    decode_path = Path(output_dir) / "decode" / "qwen3_vl_llm_decode.onnx"
...
def _export_vision_step(model_id, output_dir, device, vision_image_size):
    ...
    try:
        ...
        pass
    _clear_torch_cache()
    vision_path = Path(output_dir) / "vision" / "qwen3_vl_vision.onnx"
