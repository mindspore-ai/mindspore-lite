#!/usr/bin/env python3
"""RealBasicVSR MindSpore Lite 推理脚本(MindIR)。
...
class RealBasicVsrMsliteInferencer:
    """RealBasicVSR MindSpore Lite 推理封装。"""
    ...
    def forward(self, lr_seq):
        return self.model.predict([mslite.Tensor(lr_seq)])[0].get_data_to_numpy()


def _parse_args():
    p = argparse.ArgumentParser(description="RealBasicVSR MindSpore Lite 推理(MindIR)")
    p.add_argument("--mindir", type=str, required=True)
    p.add_argument("--input", type=str, required=True, help="低清帧图像")
    p.add_argument("--output", type=str, default="./realbasicvsr_sr_mslite.png")
...
def main():
    ...
    infer = RealBasicVsrMsliteInferencer(args.mindir, device=args.device, device_id=args.device_id)
    ...
    print(f"[mslite] saved center SR -> {args.output}")
