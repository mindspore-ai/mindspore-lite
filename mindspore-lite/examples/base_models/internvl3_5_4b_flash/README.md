# InternVL3.5-4B-Flash ONNX 模型导出与 MindSpore Lite 推理部署教程

本教程介绍如何将 [OpenGVLab/InternVL3_5-4B-Flash](https://www.modelscope.cn/models/OpenGVLab/InternVL3_5-4B-Flash) 视觉语言模型按结构拆分导出为 ONNX，转换为 MindSpore Lite MindIR，并在 **Ascend Atlas 300I Duo** 上完成图文推理与精度对齐。
...
| 子模型 | 输入 | 输出 |
| --- | --- | --- |
| `internvl_vision`（InternViT+mlp1） | pixel_values[1,3,448,448] | image_embeds[1,256,2560] |
| `internvl_llm_prefill` | inputs_embeds[1,320,2560] + attention_mask[1,320] + position_ids[1,320] | logits + present KV[36,1,8,1024,128] |
| `internvl_llm_decode` | inputs_embeds[1,1,1024] + attention_mask[1,1024] + position_ids[1,1] + past KV | logits + updated KV |

多模态融合在推理侧完成：embed 输入 token，把 `<IMG_CONTEXT>`（id=151671）位置替换为视觉 embed。1B 实测配置：hidden=2560，36 层，16 注意力头，8 KV 头，head_dim=128，vocab=151936。**同一套脚本适用于 2B/4B/8B 及 Flash 变体**，仅模型 id 与固定 shape（从 checkpoint 读取）不同。
...
> InternVL3.5-4B-Flash 与标准 4B 架构一致（Qwen3 主干：hidden=2560/36层/8KV/head_dim=128），流程同 `internvl3_5_4b`，仅模型 id 与权重不同；性能数据为 Flash 实测。
---
...
```bash
python -c "from modelscope import snapshot_download as s; print(s('OpenGVLab/InternVL3_5-4B-Flash', cache_dir='/home/yf/modelscope_cache'))"
ln -sfn /home/yf/modelscope_cache/OpenGVLab/InternVL3_5-4B-Flash ./InternVL3_5-4B-Flash
```
...
```bash
cd mindspore-lite/examples/base_models/internvl3_5_4b_flash
HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 python export_internvl3_5_4b_flash_onnx.py \
  --model-id ./InternVL3_5-4B-Flash --output-dir ./internvl3_5_4b_flash_onnx \
...
| 参数 | 说明 | 默认值 |
| --- | --- | --- |
| `--model-id` | 本地权重目录或 ModelScope id | `./InternVL3_5-4B-Flash` |
| `--output-dir` | ONNX / embed 输出目录 | `./internvl3_5_4b_flash_onnx` |
...
[export] saved internvl3_5_4b_flash_onnx/internvl_vision.onnx
[export] llm prefill ...
[export] prefill: hidden=2560 num_layers=36 num_kv_heads=8 head_dim=128 seq=320 kv_len=1024
[export] llm decode ...
[export] decode: num_layers=36 num_kv_heads=8 head_dim=128 kv_len=1024
[export] llm embed weights ...
[export] embed_weights (151936, 2560) -> internvl3_5_4b_flash_onnx/embed_weights.npy
```
...
$CONV --fmk=ONNX --modelFile=./internvl3_5_4b_flash_onnx/internvl_vision.onnx      --outputFile=./internvl3_5_4b_flash_onnx/internvl_vision      --optimize=ascend_oriented --saveType=MINDIR --configFile=./configs/internvl_vision.config
$CONV --fmk=ONNX --modelFile=./internvl3_5_4b_flash_onnx/internvl_llm_prefill.onnx --outputFile=./internvl3_5_4b_flash_onnx/internvl_llm_prefill --optimize=ascend_oriented --saveType=MINDIR --configFile=./configs/internvl_llm_prefill.config
$CONV --fmk=ONNX --modelFile=./internvl3_5_4b_flash_onnx/internvl_llm_decode.onnx  --outputFile=./internvl3_5_4b_flash_onnx/internvl_llm_decode  --optimize=ascend_oriented --saveType=MINDIR --configFile=./configs/internvl_llm_decode.config
```
...
```bash
HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 python infer_internvl3_5_4b_flash_mslite.py \
  --mindir-dir ./internvl3_5_4b_flash_onnx --model-dir ./InternVL3_5-4B-Flash \
...
--- Performance ---
  Vision encode:   89.84 ms
  LLM prefill:     675.45 ms (seq=320)
  LLM decode:      20799.12 ms (94 steps)
  End-to-end:      21564.41 ms
  Avg decode step: 301.44 ms
  Throughput:      3.32 tok/s (decode)
```
...
| **端到端（128 tok 上限）** | **~21.6 s** |
| **decode 吞吐** | **3.32 tok/s** |
...
HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1 python align_internvl3_5_4b_flash.py \
  --model-dir ./InternVL3_5-4B-Flash --image ./test.jpg --dump-dir ./align_dump --max-new-tokens 32
# 推理环境（mindspore-lite）：MSLite prefill 首 token 余弦对齐
python align_internvl3_5_4b_flash.py --mindir-dir ./internvl3_5_4b_flash_onnx --dump-dir ./align_dump --check-mslite
```
...
- 上游：<https://github.com/OpenGVLab/InternVL>、ModelScope `OpenGVLab/InternVL3_5-4B-Flash`
