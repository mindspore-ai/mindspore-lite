...
"""
Infer Qwen3-Reranker-4B with MindSpore Lite.
"""
...
class Qwen3RerankerInferencer:
    ...
    def _compute_scores(self, inputs):
        ...
        for i in range(batch_size):
            ...
            print(
                f"Pair {i + 1}: seq_len={seq_len}, latency={elapsed:.3f}ms"
            )
...
def main():
    """
    Main function for Qwen3-Reranker-4B inference with MindSpore Lite.
    """
    parser = argparse.ArgumentParser(
        description="Qwen3-Reranker-4B Inference with MindSpore Lite"
    )
    parser.add_argument(
        "--model", type=str, required=True, help="Path to qwen3_reranker_4b.mindir"
    )
    parser.add_argument(
        "--tokenizer",
        type=str,
