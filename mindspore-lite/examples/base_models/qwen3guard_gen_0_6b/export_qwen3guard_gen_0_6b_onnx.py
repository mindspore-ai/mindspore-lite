...
# ============================================================================
"""
Export Qwen3Guard-Gen-0.6B model to ONNX format.
"""
...
class Qwen3LlmPrefill(torch.nn.Module):
    """
    Qwen3Guard-Gen-0.6B LLM Preffill model for ONNX inference.
    """
...
class Qwen3LlmDecode(torch.nn.Module):
    """
    Qwen3Guard-Gen-0.6B LLM Decode model for ONNX inference.
    """
...
def export_llm_prefill_decode(model, output_dir, device="cpu"):
    """
    Export Qwen3Guard-Gen-0.6B LLM prefill and decode models to ONNX format.
    """
    ...
    prefill_path = Path(output_dir) / "qwen3guard_gen_llm_prefill.onnx"
    decode_path = Path(output_dir) / "qwen3guard_gen_llm_decode.onnx"
...
def main():
    """
    Main function to export Qwen3Guard-Gen-0.6B LLM prefill and decode models to ONNX format.
    """
    parser = argparse.ArgumentParser(description="Export Qwen3Guard-Gen-0.6B to ONNX")
    parser.add_argument(
        "--model-id", type=str, default="Qwen/Qwen3Guard-Gen-0.6B", help="HuggingFace model ID"
    )
    parser.add_argument(
