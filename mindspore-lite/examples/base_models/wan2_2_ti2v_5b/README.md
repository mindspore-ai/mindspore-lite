# Wan2.2-TI2V-5B ONNX 模型导出与 MindSpore Lite 推理部署教程

本教程介绍如何将 [Wan-AI/Wan2.2-TI2V-5B-Diffusers](https://www.modelscope.cn/models/Wan-AI/Wan2.2-TI2V-5B-Diffusers) 文本+图像生成视频（Text+Image-to-Video）模型按网络结构拆分导出为 ONNX，使用 ONNX Runtime 验证子模型推理结果，并将 ONNX 转换为 MindSpore Lite MindIR 后在 **Ascend Atlas 300I Duo** 上完成端到端推理与测速。

Wan2.2-TI2V-5B 在 Wan2.1-T2V 的纯文本条件之外，额外引入 **CLIP 图像条件**（image embedding），并采用 Wan2.2 的 `expand_timesteps` 调度：标量时间步被展开为每个时空 token 一个时间步（首帧作为条件图被驱动到 `t=0`）。本目录基于已验证的 `wan2_1_t2v_1_3b` 模板（仅 T2V、无图像条件）改写适配。
...
python -c "from modelscope import snapshot_download as s; print(s('Wan-AI/Wan2.2-TI2V-5B-Diffusers', cache_dir='/home/yf/modelscope_cache'))"
ln -sfn /home/yf/modelscope_cache/Wan-AI/Wan2.2-TI2V-5B-Diffusers Wan2.2-TI2V-5B-Diffusers
```
...
| `wan_transformer` (DiT 5B) | hidden_states[1,16,21,60,104], timestep[1,32760], encoder_hidden_states[1,512,4096], encoder_hidden_states_image[1,257,1280] | noise_pred[1,16,21,60,104] |
| `wan_vae_decoder` (3D VAE) | latents[1,16,21,60,104] | video[1,3,81,480,832] |

固定配置：480×832、81 帧（21 个 latent 帧）。`timestep` 形状 `[1, 32760]`，即 Wan2.2 `expand_timesteps` 下每个时空 token 一个时间步（21 × 30 × 52 = 32760，对应 patch_size (1,2,2) 的 token 数）。若需其它分辨率/帧数，需重新导出并重新转换。

### 导出命令

```bash
cd mindspore-lite/examples/base_models/wan2_2_ti2v_5b

python export_wan2_2_ti2v_5b_onnx.py \
  --model-dir ./Wan2.2-TI2V-5B-Diffusers \
  --output-dir ./wan2_2_ti2v_5b_onnx \
...
| --- | --- | --- |
| `--model-dir` | 权重目录 | 必填 |
| `--output-dir` | ONNX 输出目录 | `./wan2_2_ti2v_5b_onnx` |
...
m = ort.InferenceSession("wan2_2_ti2v_5b_onnx/wan_clip_image_encoder.onnx",
...
# 文本编码器
$CONV --fmk=ONNX --modelFile=./wan2_2_ti2v_5b_onnx/wan_text_encoder.onnx \
  --outputFile=./wan2_2_ti2v_5b_onnx/wan_text_encoder \
  --optimize=ascend_oriented --saveType=MINDIR --configFile=./configs/wan_text_encoder.config

# CLIP 图像编码器
$CONV --fmk=ONNX --modelFile=./wan2_2_ti2v_5b_onnx/wan_clip_image_encoder.onnx \
  --outputFile=./wan2_2_ti2v_5b_onnx/wan_clip_image_encoder \
  --optimize=ascend_oriented --saveType=MINDIR --configFile=./configs/wan_clip_image_encoder.config

# transformer
$CONV --fmk=ONNX --modelFile=./wan2_2_ti2v_5b_onnx/wan_transformer.onnx \
  --outputFile=./wan2_2_ti2v_5b_onnx/wan_transformer \
  --optimize=ascend_oriented --saveType=MINDIR --configFile=./configs/wan_transformer.config

# VAE 解码器
$CONV --fmk=ONNX --modelFile=./wan2_2_ti2v_5b_onnx/wan_vae_decoder.onnx \
  --outputFile=./wan2_2_ti2v_5b_onnx/wan_vae_decoder \
  --optimize=ascend_oriented --saveType=MINDIR --configFile=./configs/wan_vae_decoder.config
```

### 配置文件

`configs/wan_transformer.config`（注意 Wan2.2 `expand_timesteps` 的 per-token timestep 与新增的 `encoder_hidden_states_image`）：
...
```bash
python infer_wan2_2_ti2v_5b_mslite.py \
  --mindir-dir ./wan2_2_ti2v_5b_onnx \
  --model-dir ./Wan2.2-TI2V-5B-Diffusers \
  --image ./condition.jpg \
...
说明（固定 shape 约束）：`ascend_oriented` 转换按固定 shape 编译，推理侧 `--height/--width/--num-frames` 必须与导出一致；变更需重新导出+转换。文本编码器与 CLIP 图像编码器在 dev1，transformer/VAE 解码器在 dev0（组件级分芯）。VAE 编码器（仅用于编码条件图，运行一次）在 CPU 上用 torch 执行，不进入 Ascend MindIR 集合。
...
> 性能数据以推理脚本端到端打印为准；下表为**待实测填入**（运行 `infer_wan2_2_ti2v_5b_mslite.py` 后回填真实数值，不使用估算或占位假数据）。
...
端到端对比 HF diffusers `WanImageToVideoPipeline`（CPU float32 基线，`expand_timesteps=True`）与 MSLite（Ascend）的生成视频：使用相同 prompt、相同条件图、相同初始噪声（seed 固定的 torch 生成器，存 npy 后两路共用）、相同调度器参数，逐帧比较 max/mean abs 误差与 PSNR。

```bash
# 注意：HF CPU 基线较慢，建议先用较少帧/步数跑通端到端对齐
python align_wan2_2_ti2v_5b.py \
  --mindir-dir ./wan2_2_ti2v_5b_onnx \
  --model-dir ./Wan2.2-TI2V-5B-Diffusers \
  --image ./condition.jpg \
...
5. 现象：transformer 输入 `timestep` 维度对不上。
   - 原因：Wan2.2 TI2V 使用 `expand_timesteps`，时间步为 per-token（`[1, 32760]`），不是 Wan2.1 的标量 `[1]`。
   - 解决方案：确认导出/转换/推理三处的 `timestep` 形状一致（`[1, 32760]` 对应 480×832×81）；推理脚本已在 numpy 侧用 `first_frame_mask[:, ::2, ::2] * t` 重建该时间步。
6. 现象：缺少 `feature_extractor/` 子目录。
   - 原因：CLIP 图像预处理依赖 `CLIPImageProcessor` 配置。
   - 解决方案：确认 `MODEL_DIR` 含 `feature_extractor/`（与 Wan2.1-I2V 同构）；缺则从 `openai/clip-vit-large-patch14` 复用。

---

## 9. 参考资源与许可证

- 上游模型：<https://www.modelscope.cn/models/Wan-AI/Wan2.2-TI2V-5B-Diffusers>
- MindSpore Lite 文档：<https://www.mindspore.cn/lite>
- 同构参考（本目录改写模板）：`../wan2_1_t2v_1_3b/`（Wan2.1-T2V-1.3B，纯文本生成视频）
