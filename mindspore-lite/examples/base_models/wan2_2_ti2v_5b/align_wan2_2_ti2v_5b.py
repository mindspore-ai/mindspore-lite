"""End-to-end accuracy alignment for Wan2.2-TI2V-5B: HF diffusers vs MindSpore Lite.
...
image, fixed initial latents, scheduler settings):
  ...
  1. HF ``WanImageToVideoPipeline`` on CPU (ground truth, float32). The pipeline
     is constructed with ``expand_timesteps=True`` so it follows the Wan2.2 TI2V
     schedule (per-token timesteps, first-frame conditioning).
  2. MindSpore Lite pipeline on Ascend (infer_wan2_2_ti2v_5b_mslite).
...
from infer_wan2_2_ti2v_5b_mslite import WanTI2VInferencer
...
def _hf_baseline(model_dir, image, prompt, negative_prompt, latents, height, width,
    ...
    """Run the HF WanImageToVideoPipeline (expand_timesteps) on CPU.
    ...
    # Wan2.2 TI2V uses the expand_timesteps schedule; force it on (it is normally
    # set from the pipeline config.json, but we set it explicitly for clarity).
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Wan2.2-TI2V-5B HF vs MSLite alignment")
    ...
    print("[align] running HF baseline (WanImageToVideoPipeline, expand_timesteps) on CPU ...")
