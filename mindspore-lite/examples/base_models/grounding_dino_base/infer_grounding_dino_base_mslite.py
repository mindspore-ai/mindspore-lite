...
class GroundingDinoMsLiteInferencer:
    ...
    def __init__(self, mindir_path: str, model_dir: str,
                 device: str = "ascend", device_id: int = 0):
        if mslite is None:
    ...
    def run(self, img: Image.Image, text_labels: List[str],
            ...
            text_threshold: float = 0.25) -> Tuple[List[dict], float, float]:
        """Run inference and return (results, preprocess_ms, infer_ms)."""
        ...
        return results, (t1 - t0) * 1000.0, (t2 - t1) * 1000.0
...
def _run_once(inferencer: GroundingDinoMsLiteInferencer, img: Image.Image,
              text_labels: List[str], args) -> Tuple[List[dict], float, float, float]:
    """Pre-process, infer, post-process one sample; return timings + results."""
    t0 = time.perf_counter()
    results, prep_ms, infer_ms = inferencer.run(img, text_labels, args.threshold,
                                                args.text_threshold)
    t1 = time.perf_counter()
    return results, prep_ms, infer_ms, (t1 - t0) * 1000.0
...
def main():
    ...
    for _ in range(int(args.runs)):
        res, prep_ms, infer_ms, e2e_ms = _run_once(inferencer, img,
                                                   text_labels, args)
    ...
    print(f"  preprocess_ms_mean: {float(np.mean(prep_times)):.3f}")
