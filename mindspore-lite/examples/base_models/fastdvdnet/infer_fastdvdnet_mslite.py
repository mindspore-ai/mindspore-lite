#!/usr/bin/env python3
"""FastDVDnet MindSpore Lite 推理脚本(MindIR)。
...
def _check_memory_budget(device_id, limit_pct=80):
    ...
    if total is not None:
        print(f"used {used}/{total} MB ({pct}%)")
    else:
        print("unavailable")
    hu, ht = _npu_hbm_for_device(int(device_id))
    hpct = None
    if ht:
        hpct = round(hu / ht * 100.0, 1)
...
class FastDvdNetMsliteInferencer:
    """FastDVDnet MindSpore Lite 推理封装。"""
    ...
    def forward(self, seq, noise_sigma):
        out = self.model.predict([mslite.Tensor(seq), mslite.Tensor(noise_sigma)])
        return out[0].get_data_to_numpy()


def _parse_args():
    p = argparse.ArgumentParser(description="FastDVDnet MindSpore Lite 推理(MindIR)")
    p.add_argument("--mindir", type=str, required=True)
    p.add_argument("--input", type=str, required=True, help="含噪帧图像")
    p.add_argument("--output", type=str, default="./fastdvdnet_denoised_mslite.png")
...
def main():
    ...
    infer = FastDvdNetMsliteInferencer(args.mindir, device=args.device, device_id=args.device_id)
