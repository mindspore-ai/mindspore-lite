...
# ============================================================================
"""
Infer Qwen2.5-0.5B with MindSpore Lite using MindIR model.
"""
...
def _build_mslite_inputs(model: mslite.Model, feed_dict, preferred_order=None):
    """Build mslite input tensors from a feed dict, matching by name or preferred order."""
...
class Qwen25Inferencer:
    """
    Qwen2.5-0.5B inferencer using split MindIR (prefill + decode).
    """
    ...
    def _prepare_inputs(self, text: str, max_length: int):
        """Tokenize input text and compute position ids."""
        enc = self.tokenizer(
            text,
            return_tensors="np",
            padding=False,
    ...
    def generate(
        ...
        """Generate text from input prompt using prefill-decode pipeline."""
        ...
        prefill_inputs = _build_mslite_inputs(
            self.prefill_model,
            prefill_feed,
...
def main():
    """Main entry point for MindSpore Lite inference."""
    parser = argparse.ArgumentParser(
        description="Qwen2.5-0.5B Inference with MindSpore Lite using split MindIR (prefill + decode)"
    )
    parser.add_argument(
        "--prefill-model",
        type=str,
        required=True,
        help="Path to qwen25_llm_prefill.mindir",
    )
    parser.add_argument(
        "--decode-model",
        type=str,
        required=True,
        help="Path to qwen25_llm_decode.mindir",
    )
    parser.add_argument(
        "--tokenizer",
        type=str,
