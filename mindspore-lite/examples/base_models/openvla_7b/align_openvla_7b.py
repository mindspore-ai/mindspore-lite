#!/usr/bin/env python3
"""Accuracy alignment for the OpenVLA regression skeleton (single forward)."""
...
def _torch_baseline(args, image, task_tokens):
    ...
    from export_openvla_7b_onnx import OpenVLARegPolicy  # noqa: E402

    torch.manual_seed(args.seed)
    np.random.seed(args.seed)
    model = OpenVLARegPolicy(args.img_size, args.patch, args.dim, args.depth, args.heads,
                             args.horizon, args.action_dim, args.horizon, args.vocab_size, args.task_len).eval()
    with torch.no_grad():
...
def _mslite_infer(mindir_path, image, task_tokens, device, device_id):
    ...
    if str(here) not in sys.path:
        sys.path.insert(0, str(here))
    from infer_openvla_7b_mslite import _build_model, _run_model  # noqa: E402
...
def main():
    p = argparse.ArgumentParser(description="OpenVLA alignment (PyTorch vs ONNX vs MSLite)")
    p.add_argument("--onnx", type=str, default="./openvla_7b_onnx/openvla_7b_policy.onnx")
    p.add_argument("--mindir", type=str, default="./openvla_7b_onnx/openvla_7b_policy_graph.mindir")
    ...
    print("=" * 78)
    print("OpenVLA alignment: PyTorch vs ONNX vs MindSpore Lite")
    print("=" * 78)
    print(f"input: image {image.shape}, task_tokens {task_tokens.shape}")

    print("\n[PyTorch] OpenVLARegPolicy on CPU (fp32) ...")
