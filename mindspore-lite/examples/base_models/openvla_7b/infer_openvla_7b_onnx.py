#!/usr/bin/env python3
"""OpenVLA policy ONNXRuntime inference: image + task -> action chunk (single forward)."""
...
def _parse_args():
    p = argparse.ArgumentParser(description="OpenVLA policy ONNXRuntime inference")
...
def main():
    ...
    for _ in range(int(args.warmup)):
        sess.run(None, {"image": image, "task_tokens": task_tokens})
    lat = []
    for _ in range(int(args.runs)):
        t0 = time.perf_counter()
        outs = sess.run(None, {"image": image, "task_tokens": task_tokens})
