...
# ============================================================================
"""
Export Qwen2-1.5B model to ONNX format.
"""
...
def _rms_norm_layer(norm_mod, x):
    """Custom RMSNorm layer for ONNX export."""
...
def _text_attn_forward(
    ...
    """
    Text attention forward function for Qwen2-1.5B inference.
...
class Qwen2LlmPrefill(torch.nn.Module):
    """Qwen2-1.5B LLM Prefill wrapper."""
    ...
    def forward(self, input_ids, attention_mask, position_ids):
        """Qwen2-1.5B LLM Prefill forward pass."""
...
class Qwen2LlmDecode(torch.nn.Module):
    ...
    def forward(self, input_ids, attention_mask, position_ids, past_key_cache, past_value_cache):
        """Qwen2-1.5B LLM Decode forward pass."""
        inputs_embeds = self.model.embed_tokens(input_ids)
...
def _prepare_llm_modules(model, device: str):
    """Prepare LLM modules for ONNX export."""
...
def _get_kv_cache_config(model):
    """Get KV cache configuration from model config."""
...
def _create_decode_dummy_inputs(
    ...
    """Create dummy inputs for LLM decode."""
    del dummy_seq
    dummy_step = 1
...
def main():
    """Main function to export Qwen2-1.5B to ONNX."""
    parser = argparse.ArgumentParser(description="Export Qwen2-1.5B to ONNX")
    parser.add_argument(
        "--model-id",
        type=str,
        default="./Qwen2-1.5B",
    ...
    parser.add_argument(
        "--output-dir", type=str, default="./qwen2_1_5b_onnx", help="Output directory"
    )
    parser.add_argument(
