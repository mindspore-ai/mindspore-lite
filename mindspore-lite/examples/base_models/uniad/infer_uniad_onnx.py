...
# ============================================================================
"""UniAD ONNX inference (random multi-view perf + output shape)."""
...
def parse_args():
    """parse args"""
    parser = argparse.ArgumentParser(description="UniAD ONNX inference")
...
def main():
    ...
    net = UniADOnnx(args.model, args.device)
    ...
    try:
        ...
        for name, out in zip(["planning_traj", "det_cls"], outs):
            print(f"  {name}: {tuple(out.shape)}")
        lat_np = np.array(lat, dtype=np.float32)
        print(f"\nlatency_ms_mean: {float(lat_np.mean()):.3f}")
    except Exception as e:  # noqa
        print(f"(Custom deformable/multi-module op cannot run on ORT: {e})")
        print("Use the MindIR path via infer_uniad_mslite.py instead.")
