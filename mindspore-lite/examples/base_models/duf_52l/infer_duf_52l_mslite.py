#!/usr/bin/env python3
"""TOFlow MindSpore Lite 推理脚本(MindIR)。
...
class Duf52lMsliteInferencer:
    """TOFlow MindSpore Lite 推理封装。"""
...
def _parse_args():
    p = argparse.ArgumentParser(description="TOFlow MindSpore Lite 推理(MindIR)")
    p.add_argument("--mindir", type=str, required=True)
    p.add_argument("--input", type=str, required=True, help="低清帧图像")
    p.add_argument("--output", type=str, default="./duf_52l_sr_mslite.png")
...
def main():
    ...
    infer = Duf52lMsliteInferencer(args.mindir, device=args.device, device_id=args.device_id)
