#!/usr/bin/env python3
"""Export DUF-52L (Deep Video Super-Resolution, 3D 卷积+动态滤波) to ONNX。
...
class Duf52lSR(nn.Module):
    """DUF-52L 超分封装:多帧低清 -> 4x 超分。"""
...
def main():
    parser = argparse.ArgumentParser(description="Export DUF-52L to ONNX.")
    parser.add_argument("--repo-dir", type=str, default="./duf_src")
    parser.add_argument("--model-file", type=str, default="model.DUF")
    parser.add_argument("--ckpt", type=str, default="./DUF-52L.pth")
    parser.add_argument("--depth", type=int, default=_DUF_DEPTH, help="DUF depth(52)")
    parser.add_argument("--output-dir", type=str, default="./duf_52l_onnx")
    ...
    wrapper = Duf52lSR(net).to(args.device).eval()
    lr = torch.randn(1, args.num_frames, 3, args.lr_height, args.lr_width,
                     dtype=torch.float32, device=args.device)
    _export_one(wrapper, output_dir / "duf_52l.onnx", (lr,),
