...
def _compute_position_ids(attention_mask: np.ndarray) -> np.ndarray:
    """
    Compute position ids for ONNX inference.
    """
...
def main():
    """
    Qwen3-0.6B ONNX inference (prefill + decode).
    """
    parser = argparse.ArgumentParser(
        description="Qwen3-0.6B ONNX inference (prefill + decode)"
    )
    parser.add_argument(
