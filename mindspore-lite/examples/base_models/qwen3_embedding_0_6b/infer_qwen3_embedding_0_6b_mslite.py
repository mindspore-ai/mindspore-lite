...
"""
Infer Qwen3-Embedding-0.6B with MindSpore Lite.
...
class Qwen3EmbeddingInferencer:
    """
    Qwen3-Embedding-0.6B inferencer with MindSpore Lite.
    """
    ...
    def encode(self, texts, instruction=None, max_length=8192, batch_size=1):
        ...
        for i in range(0, len(formatted), batch_size):
            ...
            # L2 normalization (done here instead of in ONNX for compatibility)
...
def main():
    """
    Main function for Qwen3-Embedding-0.6B inference with MindSpore Lite.
    """
    parser = argparse.ArgumentParser(
        description="Qwen3-Embedding-0.6B Inference with MindSpore Lite"
    )
    parser.add_argument(
    ...
    print("\nEncoding documents...")
    doc_embeddings = inferencer.encode(
        documents, max_length=args.max_length
    )
