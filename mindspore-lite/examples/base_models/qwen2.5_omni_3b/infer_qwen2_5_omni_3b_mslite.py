...
# ============================================================================
"""
Infer Qwen2.5-Omni-3B with MindSpore Lite using split MindIR (prefill + decode).
"""
...
KV_CACHE_LEN = 512
# Must align with ge.dynamicDims in prefill config
...
class Qwen215BInferencer:
    """Qwen2.5-Omni-3B inferencer with MindSpore Lite."""
    ...
    def _select_prefill_gear(self, seq_len: int) -> int:
        """Select the smallest gear >= seq_len from PREFILL_GEARS."""
        seq_len = int(seq_len)
    ...
    def _prepare_inputs(self, text: str, max_length: int):
        """Prepare inputs for Qwen2.5-Omni-3B inference."""
    ...
    def generate(
        ...
        """Generate text using Qwen2.5-Omni-3B."""
        ...
        print(f"\n[Decode] Running {max_new_tokens - 1} decode steps (KV_CACHE_LEN={KV_CACHE_LEN}) ...")
        decode_times = []
        for step_i in range(max_new_tokens - 1):
            ...
            # Print first step and every 32 steps
...
def main():
    parser = argparse.ArgumentParser(
        description="Qwen2.5-Omni-3B Inference with MindSpore Lite (prefill + decode)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--tokenizer", type=str, default="./Qwen2.5-Omni-3B", help="Tokenizer path"
    )
