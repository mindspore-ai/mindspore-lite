# EDVR-M ONNX 模型导出与 MindSpore Lite 推理部署教程

本教程介绍如何将 **EDVR-M**(Video Restoration,DCNv1 对齐 + 时空注意力,中等规模)导出为单 ONNX,经 ONNX Runtime 验证后转换为 MindIR,在 **Atlas 300I Duo(310P3)** 上推理测速。输入 7 帧低清,输出 4x 超分中心帧。EDVR-L 同源仅换规模。
...
```bash
cd mindspore-lite/examples/base_models/edvr_m
python export_edvr_m_onnx.py \
  --repo-dir ./mmagic_src --model-file mmagic.models.edvr_net.EDVRNet \
  --ckpt ./edvr_m_x4.pth --output-dir ./edvr_m_onnx --device cpu \
...
| `--ckpt` | 权重 | `./edvr_m_x4.pth` |
| `--num-frames` | 输入帧数 | `7` |
| `--lr-height/--lr-width` | 低清尺寸(须被 4 整除) | `64` / `64` |

```text
./edvr_m_onnx/
└── edvr_m.onnx   # 输入 lr_seq [1,7,3,64,64],输出 sr_frame [1,3,256,256]
```
...
```bash
python infer_edvr_m_onnx.py --onnx ./edvr_m_onnx/edvr_m.onnx \
...
```bash
converter_lite --fmk=ONNX --modelFile=./edvr_m_onnx/edvr_m.onnx \
  --outputFile=./edvr_m_onnx/edvr_m --optimize=ascend_oriented \
...
```bash
python infer_edvr_m_mslite.py --mindir ./edvr_m_onnx/edvr_m.mindir \
