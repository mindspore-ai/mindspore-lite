...
class Qwen3EmbeddingWrapper(torch.nn.Module):
    """
    Wrapper for Qwen3-Embedding-4B that outputs pooled embeddings.
    Uses last-token pooling. L2 normalization is done in the inference script.
    """
...
def _parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="Export Qwen3-Embedding-4B to ONNX")
    parser.add_argument(
        "--model-id",
        type=str,
...
def main():
    ...
    output_path = os.path.join(args.output_dir, "qwen3_embedding_4b.onnx")
