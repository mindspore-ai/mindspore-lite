#!/usr/bin/env python3
"""Accuracy alignment for the CogACT regression skeleton (single forward)."""
...
def _torch_baseline(args, image, task_tokens):
    ...
    from export_cogact_8b_onnx import CogACTPolicy  # noqa: E402
...
def _mslite_infer(mindir_path, image, task_tokens, device, device_id):
    ...
    if str(here) not in sys.path:
        sys.path.insert(0, str(here))
    from infer_cogact_8b_mslite import _build_model, _run_model  # noqa: E402
...
def main():
    p = argparse.ArgumentParser(description="CogACT alignment (PyTorch vs ONNX vs MSLite)")
    p.add_argument("--onnx", type=str, default="./cogact_8b_onnx/cogact_8b_policy.onnx")
    p.add_argument("--mindir", type=str, default="./cogact_8b_onnx/cogact_8b_policy_graph.mindir")
    ...
    print("=" * 78)
    print("CogACT alignment: PyTorch vs ONNX vs MindSpore Lite")
    print("=" * 78)
    print(f"input: image {image.shape}, task_tokens {task_tokens.shape}")

    print("\n[PyTorch] CogACTPolicy on CPU (fp32) ...")
