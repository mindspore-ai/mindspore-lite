"""End-to-end accuracy alignment for Kandinsky-5.0-I2V-Lite-5s: HF vs MindSpore Lite.

Generates the SAME image-to-video output twice from identical inputs (prompt,
source image, and fixed initial noise):

  1. HF diffusers ``Kandinsky5I2VPipeline`` on CPU (ground truth, float32).
  2. MindSpore Lite pipeline on Ascend (``infer_kandinsky5_i2v_lite_5s_mslite``).
...
from infer_kandinsky5_i2v_lite_5s_mslite import Kandinsky5I2IInferencer
...
def _hf_latents(noise_cl, height, width):
    ...
    The HF ``Kandinsky5I2VPipeline`` consumes initial noise in NCDHW-ish latent
...
def _hf_baseline(pipe, prompt, negative_prompt, source_image, noise, height, width,
    ...
    """Run the HF Kandinsky5I2VPipeline on CPU, return image in [0,1] (H,W,3)."""
    out = pipe(
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Kandinsky-5.0-I2V-Lite-5s HF vs MSLite alignment")
    ...
    print("[align] running HF Kandinsky5I2VPipeline on CPU ...")
    pipe = Kandinsky5I2VPipeline.from_pretrained(args.k5_model, torch_dtype=torch.float32)
