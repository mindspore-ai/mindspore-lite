> ⚠️ **待验证（图生视频/时序）**：本目录由 I2I 示例克隆。K5 图生视频在 `Kandinsky5Transformer3DModel` 上启用时序维度（temporal>1，约 5 秒帧数），VAE 解码为 3D 视频解码，图像条件为首帧（沿时间复制首帧 latent + 首帧 mask）。**时序帧数、首帧条件注入方式、Kandinsky5I2VPipeline 的具体参数需在 Phase B 验证时依据 diffusers `Kandinsky5I2VPipeline` 源码与上游 `kandinsky-5` 仓库确认与修正**；导出切分、MSLite 推理骨架、配置与计时约定可直接复用。

# Kandinsky-5.0-I2V-Lite-5s ONNX 模型导出与 MindSpore Lite 推理部署教程

本教程介绍如何将 [kandinskylab/Kandinsky-5.0-I2V-Lite-5s](https://www.modelscope.cn/models/kandinskylab/Kandinsky-5.0-I2V-Lite-5s) 图生视频（image-to-video）模型按网络结构拆分导出为 ONNX，将 ONNX 转换为 MindSpore Lite MindIR，并在 **Ascend Atlas 300I Duo** 上完成端到端图生视频推理与精度对齐。
...
| diffusers | 0.38.0（含 Kandinsky5I2VPipeline） |
...
```bash
# K5 I2I Lite DiT 权重
python -c "from modelscope import snapshot_download as s; print(s('kandinskylab/Kandinsky-5.0-I2V-Lite-5s', cache_dir='/home/yf/modelscope_cache'))"
...
ln -sfn /home/yf/modelscope_cache/kandinskylab/Kandinsky-5___0-I2I-Lite ./kandinsky5lite_i2v
...
```bash
cd mindspore-lite/examples/base_models/kandinsky5_i2v_lite_5s

python export_kandinsky5_i2v_lite_5s_onnx.py \
  --k5-model ./kandinsky5lite_i2v \
  --qwen-dir ./Qwen2.5-VL-7B-Instruct \
  --clip-dir ./clip-vit-large-patch14 \
  --vae-dir ./flux_vae \
  --output-dir ./kandinsky5_i2v_lite_5s_onnx \
...
for c in text_encoder clip_encoder transformer dcae_decoder; do
  $CONV --fmk=ONNX \
    --modelFile=./kandinsky5_i2v_lite_5s_onnx/kandinsky_${c}.onnx \
    --outputFile=./kandinsky5_i2v_lite_5s_onnx/kandinsky_${c} \
...
```bash
python infer_kandinsky5_i2v_lite_5s_mslite.py \
  --mindir-dir ./kandinsky5_i2v_lite_5s_onnx \
  --qwen-dir ./Qwen2.5-VL-7B-Instruct \
  --clip-dir ./clip-vit-large-patch14 \
  --vae-dir ./flux_vae \
...
> 性能数据以推理脚本端到端打印为准；下表为**待实测填入**（运行 `infer_kandinsky5_i2v_lite_5s_mslite.py` 后回填真实数值，不使用估算或占位假数据）。
...
端到端对比 HF diffusers `Kandinsky5I2VPipeline`（CPU float32）与 MSLite（Ascend）：相同 prompt、相同源图、相同 seed 初始噪声，比较输出图像 max/mean abs 误差与 PSNR。

```bash
python align_kandinsky5_i2v_lite_5s.py \
  --mindir-dir ./kandinsky5_i2v_lite_5s_onnx \
  --qwen-dir ./Qwen2.5-VL-7B-Instruct --clip-dir ./clip-vit-large-patch14 \
  --vae-dir ./flux_vae --k5-model ./kandinsky5lite_i2v \
...
- 上游：<https://github.com/kandinskylab/kandinsky-5>、ModelScope `kandinskylab/Kandinsky-5.0-I2V-Lite-5s`
