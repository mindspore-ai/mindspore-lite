...
# ============================================================================
"""VectorNet ONNX inference (random-input perf + output shape)."""
...
class VectorNetOnnx:
    """VectorNet ONNXRuntime inference."""
    ...
    def forward(self, agent_hist, map_polyline):
        """run forward."""
        feed = {self.input_names[0]: agent_hist, self.input_names[1]: map_polyline}
        return self.sess.run(self.output_names, feed)


def parse_args():
    """parse args"""
    parser = argparse.ArgumentParser(description="VectorNet ONNX inference")
...
def main():
    ...
    net = VectorNetOnnx(args.model, args.device)
