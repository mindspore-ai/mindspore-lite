...
python -c "from modelscope import snapshot_download; \
  print(snapshot_download('AI-ModelScope/sv4d', \
...
```bash
ln -sfn /home/yf/modelscope_cache/AI-ModelScope/sv4d \
  ./sv4d
```
...
```bash
cd ./mindspore-lite/examples/base_models/sv4d

python export_sv4d_onnx.py \
  --model-id ./sv4d \
  --output-dir ./sv4d_onnx \
  --resolution 576 1024 \
  --num-frames 25 \
  --dtype fp32
```
...
| 参数 | 说明 | 默认值 |
|---|---|---|
| `--model-id` | 本地 diffusers 权重目录 | `./sv4d` |
| `--output-dir` | ONNX 输出目录 | `./sv4d_onnx` |
...
$CONV --fmk=ONNX --modelFile=./sv4d_onnx/svd_image_encoder.onnx \
  --outputFile=./sv4d_onnx/svd_image_encoder \
  --optimize=ascend_oriented --saveType=MINDIR \
  --configFile=./configs/svd_image_encoder.config

# unet
$CONV --fmk=ONNX --modelFile=./sv4d_onnx/svd_unet.onnx \
  --outputFile=./sv4d_onnx/svd_unet \
  --optimize=ascend_oriented --saveType=MINDIR \
  --configFile=./configs/svd_unet.config

# vae_decoder
$CONV --fmk=ONNX --modelFile=./sv4d_onnx/svd_vae_decoder.onnx \
  --outputFile=./sv4d_onnx/svd_vae_decoder \
  --optimize=ascend_oriented --saveType=MINDIR \
...
python infer_sv4d_mslite.py \
  --mindir-dir   ./sv4d_onnx \
  --model-dir    ./sv4d \
...
提供 `align_sv4d.py`，在固定条件图像/seed/latent 下对 HF(CPU fp32) 与 MindIR(Ascend fp16) 做逐帧端到端比对：用 seeded torch generator 生成共享初始 latents [1,25,4,72,128]，分别喂给 HF `StableVideoDiffusionPipeline`（CPU）与 MSLite 推理，比较输出视频的逐帧与聚合 `max_abs`、`mean_abs`、`PSNR`。

```bash
python align_sv4d.py \
  --mindir-dir ./sv4d_onnx \
  --model-dir  ./sv4d \
  --image      ./conditioning.jpg \
  --seed 0 --steps 25 --fps 7 \
...
- [SV4D 模型页（HuggingFace）](https://huggingface.co/stabilityai/sv4d)
...
SV4D 遵循 [Stability AI Community License](https://huggingface.co/stabilityai/sv4d/blob/main/LICENSE.md)。本教程遵循相应依赖的许可证要求。
