#!/usr/bin/env python3
"""Accuracy alignment for the MiniGPT-4 skeleton (single forward logits)."""
...
def _torch_baseline(args, image, input_ids):
    ...
    from export_minigpt4_7b_onnx import MiniGPT4Skel  # noqa: E402

    torch.manual_seed(args.seed)
    np.random.seed(args.seed)
    model = MiniGPT4Skel(args.img_size, args.patch, args.dim, args.depth, args.heads,
                         args.num_query, args.vocab_size, args.seq_len).eval()
    with torch.no_grad():
...
def _mslite_infer(mindir_path, image, input_ids, device, device_id):
    ...
    if str(here) not in sys.path:
        sys.path.insert(0, str(here))
    from infer_minigpt4_7b_mslite import _build_model, _run_model  # noqa: E402
...
def main():
    p = argparse.ArgumentParser(description="MiniGPT-4 alignment (PyTorch vs ONNX vs MSLite)")
    p.add_argument("--onnx", type=str, default="./minigpt4_7b_onnx/minigpt4_7b.onnx")
    p.add_argument("--mindir", type=str, default="./minigpt4_7b_onnx/minigpt4_7b_graph.mindir")
    ...
    print("=" * 78)
    print("MiniGPT-4 alignment: PyTorch vs ONNX vs MindSpore Lite")
    print("=" * 78)
    print(f"input: image {image.shape}, input_ids {input_ids.shape}")

    print("\n[PyTorch] MiniGPT4Skel on CPU (fp32) ...")
