#!/usr/bin/env python3
"""Accuracy alignment for the CogVLM skeleton (single forward logits)."""
...
def _torch_baseline(args, image, input_ids):
    ...
    from export_cogvlm_onnx import CogVLMSkel  # noqa: E402
...
def _mslite_infer(mindir_path, image, input_ids, device, device_id):
    ...
    if str(here) not in sys.path:
        sys.path.insert(0, str(here))
    from infer_cogvlm_mslite import _build_model, _run_model  # noqa: E402
...
def main():
    p = argparse.ArgumentParser(description="CogVLM alignment (PyTorch vs ONNX vs MSLite)")
    p.add_argument("--onnx", type=str, default="./cogvlm_onnx/cogvlm.onnx")
    p.add_argument("--mindir", type=str, default="./cogvlm_onnx/cogvlm_graph.mindir")
    ...
    print("=" * 78)
    print("CogVLM alignment: PyTorch vs ONNX vs MindSpore Lite")
    print("=" * 78)
    print(f"input: image {image.shape}, input_ids {input_ids.shape}")

    print("\n[PyTorch] CogVLMSkel on CPU (fp32) ...")
