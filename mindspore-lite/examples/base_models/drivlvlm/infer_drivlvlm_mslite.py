...
# ============================================================================
"""DriveVLM vision encoder MindSpore Lite inference (numpy-only, no torch)."""
...
def _random_input(c, h, w):
    """fixed-seed random image."""
    rng = np.random.RandomState(SEED)
    return np.ascontiguousarray(rng.rand(1, c, h, w).astype(np.float32))


class DriveVLMMsLite:
    """DriveVLM vision encoder MindSpore Lite inference."""
    ...
    def forward(self, pixel_values):
        """run forward."""
        inputs = [mslite.Tensor(np.ascontiguousarray(pixel_values))]
...
def parse_args():
    """parse args"""
    parser = argparse.ArgumentParser(description="DriveVLM vision encoder MindSpore Lite inference")
...
def main():
    ...
    net = DriveVLMMsLite(args.model, args.device, args.device_id)
    images = _random_input(3, args.img_h, args.img_w)
    ...
    for name, out in zip(["image_embeds"], outs):
