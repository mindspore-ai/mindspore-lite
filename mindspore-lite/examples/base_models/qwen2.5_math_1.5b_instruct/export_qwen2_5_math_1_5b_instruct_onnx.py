...
# ============================================================================
"""
Export Qwen2.5-Math-1.5B-Instruct model to ONNX format.
"""
...
class _RotaryMulCustom(torch.autograd.Function):
    ...
    def symbolic(g, x, cos4, sin4):
        """ONNX symbolic for rotary embedding multiplication."""
        y = g.op(
            "Custom",
            x,
            cos4,
...
class _RmsNormCustom(torch.autograd.Function):
    ...
    def symbolic(g, x, gamma, epsilon: float):
        """ONNX symbolic for RMS normalization."""
        y, rstd = g.op(
            "Custom",
            x,
            gamma,
...
class _SwiGluCustom(torch.autograd.Function):
    ...
    def symbolic(g, x, dim: int):
        """ONNX symbolic for SwiGLU activation."""
        y = g.op(
            "Custom",
            x,
            type_s="SwiGlu",
...
class _ScatterCustom(torch.autograd.Function):
    ...
    def forward(ctx, var, indices, updates, reduce: str, axis: int):
        """Eager fallback for scatter operation."""
        del ctx
    ...
    def symbolic(g, var, indices, updates, reduce: str, axis: int):
        """ONNX symbolic for scatter operation."""
        y = g.op(
            "Custom",
            var,
            indices,
...
def _text_attn_forward(
    ...
    """
    Text attention forward function for Qwen2.5-Math-1.5B-Instruct inference.
    Qwen2 does NOT have q_norm/k_norm (unlike Qwen3).
    """
    ...
    # Qwen2 does NOT have q_norm/k_norm
...
class Qwen2LlmPrefill(torch.nn.Module):
    """Qwen2.5-Math-1.5B-Instruct LLM Prefill wrapper."""
    ...
    def forward(self, input_ids, attention_mask, position_ids):
        """Forward pass for prefill: embed tokens, run transformer layers, compute logits."""
...
class Qwen2LlmDecode(torch.nn.Module):
    """Qwen2.5-Math-1.5B-Instruct LLM Decode wrapper."""
    ...
    def forward(self, input_ids, attention_mask, position_ids, past_key_cache, past_value_cache):
        """Forward pass for decode: single-token step with KV cache update."""
        inputs_embeds = self.model.embed_tokens(input_ids)
...
def _prepare_output_paths(output_dir):
    ...
    prefill_path = prefill_dir / "qwen2_5_math_1_5b_instruct_llm_prefill.onnx"
    decode_path = decode_dir / "qwen2_5_math_1_5b_instruct_llm_decode.onnx"
...
def _export_prefill_onnx(prefill, prefill_path: Path, dummy_inputs, use_dynamo: bool):
    """Export the prefill model to ONNX format."""
...
def _create_decode_dummy_inputs(
    ...
    """Create dummy inputs for decode ONNX export."""
    del dummy_seq
    dummy_step = 1
...
def _export_decode_onnx(decode, decode_path: Path, dummy_inputs, use_dynamo: bool):
    """Export the decode model to ONNX format."""
...
def export_llm_prefill_decode(
    ...
    """Export LLM prefill and decode models to ONNX format."""
...
def main():
    parser = argparse.ArgumentParser(description="Export Qwen2.5-Math-1.5B-Instruct to ONNX")
    parser.add_argument(
        "--model-id",
        type=str,
        default="./Qwen2.5-Math-1.5B-Instruct",
        help="HuggingFace model ID or local path",
    )
    parser.add_argument(
        "--output-dir", type=str, default="./qwen2_5_math_1_5b_instruct_onnx", help="Output directory"
    )
    parser.add_argument(
