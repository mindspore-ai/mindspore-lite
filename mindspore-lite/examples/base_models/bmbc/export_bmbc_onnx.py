#!/usr/bin/env python3
"""Export BMBC (Bilateral Motion Estimation with Cost volume) to ONNX.

封装 ``forward(img0, img1) -> mid_frame``。上游 junheum/BMBC,
forward 签名需 Phase 2 用实际源码核对(部分版本接受 timestep)。
"""
...
def main():
    parser = argparse.ArgumentParser(description="Export BMBC to ONNX.")
    parser.add_argument("--repo-dir", type=str, default="./bmbc_src")
    parser.add_argument("--model-file", type=str, default="model.BMBC")
    parser.add_argument("--ckpt", type=str, default="./bmbc.pth")
    parser.add_argument("--output-dir", type=str, default="./bmbc_onnx")
    ...
    wrapper = BmbcInterp(net).to(args.device).eval()
    ...
    _export_one(wrapper, output_dir / "bmbc.onnx", (d0, d1),
