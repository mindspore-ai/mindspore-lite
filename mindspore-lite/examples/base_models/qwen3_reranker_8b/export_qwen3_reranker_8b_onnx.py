...
class Qwen3Reranker8B(torch.nn.Module):
    """
...
def _prepare_model_for_export(model, device):
    ...
    """
    reranker = Qwen3Reranker8B(model).to(device).eval()
    return reranker
...
def _export_to_onnx(model, output_path, dummy_input_ids, dummy_attention_mask):
    ...
    # dynamo=False forces the legacy (TorchScript) exporter. torch>=2.9 defaults
    # to the dynamo exporter (opset 18), which cannot down-convert to opset 17
    # and crashes ("axes_input_to_attribute ... No initializer or constant input").
    # The legacy exporter directly produces the opset-17 graph expected by
    # converter_lite's ascend_oriented optimization.
    export_kwargs = {
        "export_params": True,
        "opset_version": 17,
    ...
    try:
        ...
        # torch < 2.7 has no `dynamo` kwarg (legacy exporter is already default)
        torch.onnx.export(
            model,
            (dummy_input_ids, dummy_attention_mask),
            output_path,
    ...
    # Consolidate external data into single file for large models
    ...
    # load_external_data=False: only the graph is needed to edit nodes. Loading
    # external weights into memory and re-saving would inline all ~16GB of
    # weights back into the .onnx, producing an invalid (>2GB) protobuf.
...
def _consolidate_onnx_external_data(onnx_path):
    ...
    for f in onnx_path.parent.iterdir():
        if f.name.startswith("onnx__") or f.name.startswith("model."):
...
def main():
    ...
    output_path = os.path.join(args.output_dir, "qwen3_reranker_8b.onnx")
