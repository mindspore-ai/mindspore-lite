...
function Run_Build_x86() {
  ...
  if [ $? = 0 ]; then
    ...
    cp ${mindspore_top_dir}/build/_deps/opencv-4.2-for-dpico-src/lib/* ${x86_path}/lib
    cp ${mindspore_top_dir}/build/_deps/protobuf-3.9-for-dpico-src/lib/* ${x86_path}/lib
    cp ${mindspore_top_dir}/build/_deps/pico_mapper-src/lib/* ${x86_path}/lib
  else
...
chmod +x ${mindspore_top_dir}/build/tools/benchmark/benchmark
cp -a ${mindspore_top_dir}/build/tools/benchmark/benchmark ${benchmark_test_path}/benchmark || exit 1
...
cp -a ${mindspore_top_dir}/build/_deps/34xx_sdk-src/lib/*so* ${benchmark_test_path} || exit 1
