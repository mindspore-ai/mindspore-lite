...
TEST_F(TestTfliteParserReverse, OpType) {
  ...
  ASSERT_EQ(meta_graph->nodes.front()->primitive->value.type, schema::PrimitiveType_ReverseV2) << "wrong Op Type";
}

TEST_F(TestTfliteParserReverse, AttrValue) {
  ASSERT_NE(meta_graph->nodes.front()->primitive->value.AsReverseV2(), nullptr);
  auto val = meta_graph->nodes.front()->primitive->value.AsReverseV2();
  std::vector<int64_t> axis = {3};
