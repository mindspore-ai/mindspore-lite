...
TEST_F(TestDSP_Range, Range_Fp32) {
  InitDSPRuntime();
...
TEST_F(TestDSP_Range, Range_Int32) {
  InitDSPRuntime();
  std::vector<lite::Tensor *> inputs;
  std::vector<lite::Tensor *> outputs;
...
TEST_F(TestDSP_Range, Range_Int16) {
  InitDSPRuntime();
  std::vector<lite::Tensor *> inputs;
  std::vector<lite::Tensor *> outputs;
...
#ifdef SUPPORT_FT04
TEST_F(TestDSP_Range, Range_Fp16) {
  InitDSPRuntime();
  std::vector<lite::Tensor *> inputs;
  std::vector<lite::Tensor *> outputs;
  ...
  for (int i = 0; i < num; ++i) {
    actual[i] = Fp16ToFp32(out_fp16[i]);
  }
...
#ifdef SUPPORT_FT78
TEST_F(TestDSP_Range, Range_Fp64) {
  InitDSPRuntime();
  std::vector<lite::Tensor *> inputs;
  std::vector<lite::Tensor *> outputs;
...
TEST_F(TestDSP_Range, Range_Int8) {
  InitDSPRuntime();
  std::vector<lite::Tensor *> inputs;
  std::vector<lite::Tensor *> outputs;
