...
TEST_F(TestDSP_ApplyMomentum, ApplyMomentum_Fp32_NesterovFalse) {
  ...
  auto weight_after = reinterpret_cast<float *>(inputs[0]->MutableData());
...
TEST_F(TestDSP_ApplyMomentum, ApplyMomentum_Fp16_NesterovTrue) {
  ...
  for (int i = 0; i < kTensorLength; ++i) {
    weight_half[i] = Fp32ToFp16(initial_weight_fp32[i]);
    accumulate_half[i] = Fp32ToFp16(initial_accumulate_fp32[i]);
    gradient_half[i] = Fp32ToFp16(gradients_fp32[i]);
  }

  reinterpret_cast<uint16_t *>(lr_tensor->MutableData())[0] = Fp32ToFp16(kLearningRate);
  reinterpret_cast<uint16_t *>(momentum_tensor->MutableData())[0] = Fp32ToFp16(kMomentum);
  ...
  for (int i = 0; i < kTensorLength; ++i) {
    expected_weight_quantized[i] = Fp16ToFp32(Fp32ToFp16(expected_weight[i]));
  }
  ...
  auto weight_after_half = reinterpret_cast<uint16_t *>(inputs[0]->MutableData());
  std::vector<float> weight_after_fp32(kTensorLength);
  for (int i = 0; i < kTensorLength; ++i) {
    weight_after_fp32[i] = Fp16ToFp32(weight_after_half[i]);
  }
