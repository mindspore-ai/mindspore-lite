...
class TestPNNA_Exp : public CommonTest {};

TEST_F(TestPNNA_Exp, shape_8_fp32) {
  ...
  node->primitive->value.type = schema::PrimitiveType_ExpFusion;
  auto primitive = new schema::ExpFusionT;
  ...
  std::vector<float> expect = {1.000000, 2.718281, 7.389056, 20.08553, 54.59815, 148.4131, 403.4287, 1096.633};
...
TEST_F(TestPNNA_Exp, shape_8_int8) {
  auto meta_graph = std::make_shared<schema::MetaGraphT>();
  meta_graph->name = "graph";
  auto node = std::make_unique<schema::CNodeT>();
  node->inputIndex = {0};
  ...
  meta_graph->inputIndex = {0};
  meta_graph->outputIndex = {1};
  std::vector<float> input_data = {0, 1, 2, 3, 0, 1, 2, 3};
  std::vector<int8_t> input_quant_data(input_data.size());
  float in_scale;
  int in_zp;
  QuantProcess(input_data.data(), input_data.size(), 0.0, 3.0, &in_scale, &in_zp, input_quant_data.data());
  std::vector<float> expect = {1.000000, 2.718281, 7.389056, 20.08553, 1.000000, 2.718281, 7.389056, 20.08553};
  float out_scale;
  int out_zp;
  QuantProcess(expect.data(), expect.size(), 0.0, 21.0, &out_scale, &out_zp, nullptr);
  ...
  std::vector<float> output_dequant(expect.size());
  Dequantize(outData, outputs[0].ElementNum(), out_scale, out_zp, output_dequant.data());
  ASSERT_EQ(0, CompareOutputData(output_dequant.data(), expect.data(), expect.size(), 1e-1));
}
}  // namespace mindspore::lite::dsp::test
