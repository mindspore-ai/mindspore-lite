...
class TestPNNA_Erf : public CommonTest {};

TEST_F(TestPNNA_Erf, shape_3_2_fp32) {
  ...
  node->primitive->value.type = schema::PrimitiveType_Erf;
  auto primitive = new schema::ErfT();
  ...
  std::vector<float> a_data = {1, 2, 3, 0, -1, -2};
  std::vector<float> expect = {0.8427007, 0.9953223, 0.999978, 0, -0.8427007, -0.9953223};  // correct answer
...
// CPU Unsupported op_type index: 178, op_type: Erf, data_type: 32.
TEST_F(TestPNNA_Erf, DISABLED_shape_3_2_int8) {
  auto meta_graph = std::make_shared<schema::MetaGraphT>();
  meta_graph->name = "graph";
  auto node = std::make_unique<schema::CNodeT>();
  node->inputIndex = {0};
  ...
  meta_graph->inputIndex = {0};
  meta_graph->outputIndex = {1};
  std::vector<float> input_data = {1, 2, 3, 0, -1, -2};
  std::vector<int8_t> input_quant_data(input_data.size());
  float in_scale;
  int in_zp;
  QuantProcess(input_data.data(), input_data.size(), -3.0, 3.0, &in_scale, &in_zp, input_quant_data.data());
  std::vector<float> expect = {0.8427007, 0.9953223, 0.999978, 0, -0.8427007, -0.9953223};
  float out_scale;
  int out_zp;
  QuantProcess(expect.data(), expect.size(), -1.0, 1.0, &out_scale, &out_zp, nullptr);
