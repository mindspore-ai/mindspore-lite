...
class TestPNNA_DepthToSpace : public CommonTest {};

TEST_F(TestPNNA_DepthToSpace, CRDMode_nhwc_fp32) {
  ...
  node->primitive->value.type = schema::PrimitiveType_DepthToSpace;
  auto primitive = new schema::DepthToSpaceT();
  primitive->block_size = 2;
  primitive->mode = "CRD";
  node->primitive->value.value = primitive;
  node->name = "DepthToSpace";
  ...
  std::vector<float> expect = {1, 5, 2, 6, 3, 7, 4, 8};  // correct answer
...
TEST_F(TestPNNA_DepthToSpace, CRDMode_nhwc_int8) {
  auto meta_graph = std::make_shared<schema::MetaGraphT>();
  meta_graph->name = "graph";
  auto node = std::make_unique<schema::CNodeT>();
  node->inputIndex = {0};
  ...
  float in_scale;
  int in_zp;
  QuantProcess(input_data.data(), input_data.size(), 0.0, 8.0, &in_scale, &in_zp, input_quant_data.data());
  std::vector<float> expect = {1, 5, 2, 6, 3, 7, 4, 8};  // correct answer
  float out_scale;
  int out_zp;
  QuantProcess(expect.data(), expect.size(), 0.0, 8.0, &out_scale, &out_zp, nullptr);
  out_zp = -128;
  ...
  Dequantize(outData.data(), outputs[0].ElementNum(), out_scale, out_zp, output_dequant.data());
