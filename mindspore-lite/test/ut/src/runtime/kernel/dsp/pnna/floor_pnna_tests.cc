...
class TestPNNA_Floor : public CommonTest {};

TEST_F(TestPNNA_Floor, shape_8_fp32) {
  ...
  node->primitive->value.type = schema::PrimitiveType_Floor;
  auto primitive = new schema::FloorT;
  ...
  std::vector<float> expect = {0, 0, 0, 0, -1, -1, -1, 0};
...
TEST_F(TestPNNA_Floor, shape_8_int8) {
  auto meta_graph = std::make_shared<schema::MetaGraphT>();
  meta_graph->name = "graph";
  auto node = std::make_unique<schema::CNodeT>();
  node->inputIndex = {0};
  ...
  float in_scale;
  int in_zp;
  QuantProcess(input_data.data(), input_data.size(), -1.0, 1.0, &in_scale, &in_zp, input_quant_data.data());
  std::vector<float> expect = {0, 0, 0, 0, -1, -1, -1, 0};
  float out_scale;
  int out_zp;
  QuantProcess(expect.data(), expect.size(), -1.0, 0.0, &out_scale, &out_zp, nullptr);
  ...
  std::vector<float> output_dequant(input_data.size());
