...
class TestPNNA_Dropout : public CommonTest {};

// CPU Unsupported parameter type in Create : Dropout
TEST_F(TestPNNA_Dropout, DISABLED_shape_2_2_fp32) {
  ...
  node->primitive->value.type = schema::PrimitiveType_Dropout;
  auto primitive = new schema::DropoutT();
  primitive->keep_prob = 0.5;
  ...
  std::vector<float> a_data = {20, 16, 50, 50};
  std::vector<float> expect = {1, 1, 1, 1};  // correct answer
