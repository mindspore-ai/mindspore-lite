...
class TestPNNA_Neg : public CommonTest {};

TEST_F(TestPNNA_Neg, shape_8_fp32) {
  ...
  node->primitive->value.type = schema::PrimitiveType_Neg;
  auto primitive = new schema::NegT;
...
TEST_F(TestPNNA_Neg, DISABLED_shape_8_int8) {
  auto meta_graph = std::make_shared<schema::MetaGraphT>();
  meta_graph->name = "graph";
  ...
  std::vector<int8_t> input_quant_data(length);
  QuantProcess(input_data.data(), length, -1.100000f, 0.958924f, &in_scale, &in_zp, input_quant_data.data());
  ...
  float out_scale;
  int out_zp;
  QuantProcess(nullptr, 0, -0.958924f, 1.100000f, &out_scale, &out_zp, nullptr);
  ...
  std::vector<float> expect = {1.100000, 0.841470, -0.909297, 0.141120, 0.756802, -0.958924, 0.279415, -0.656986};
  ASSERT_EQ(0, CompareOutputData(out.data(), expect.data(), expect.size(), 0.01));
}
}  // namespace mindspore::lite::dsp::test
