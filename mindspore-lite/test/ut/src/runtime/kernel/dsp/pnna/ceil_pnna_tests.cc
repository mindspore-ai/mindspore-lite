...
class TestPNNA_Ceil : public CommonTest {};

TEST_F(TestPNNA_Ceil, shape_8_fp32) {
  ...
  node->primitive->value.type = schema::PrimitiveType_Ceil;
  auto primitive = new schema::CeilT;
  ...
  std::vector<float> input_data = {0.000000, 0.841470, 0.909297, 0.141120, -0.756802, -0.958924, -0.279415, 0.656986};
  std::vector<float> expect = {0, 1, 1, 1, 0, 0, 0, 1};
...
TEST_F(TestPNNA_Ceil, shape_8_int8) {
  auto meta_graph = std::make_shared<schema::MetaGraphT>();
  meta_graph->name = "graph";
  auto node = std::make_unique<schema::CNodeT>();
  node->inputIndex = {0};
  ...
  float in_scale;
  int in_zp;
  QuantProcess(input_data.data(), input_data.size(), -1, 1, &in_scale, &in_zp, input_quant_data.data());
  std::vector<float> expect = {0, 1, 1, 1, 0, 0, 0, 1};
  float out_scale;
  int out_zp;
  QuantProcess(expect.data(), expect.size(), 0, 1, &out_scale, &out_zp, nullptr);
  ...
  ASSERT_EQ(kSuccess, ret.StatusCode());
  ASSERT_EQ(outputs.size(), 1);
  auto out = reinterpret_cast<const int8_t *>(outputs.front().Data().get());
  std::vector<float> output_dequant(outputs.front().ElementNum());
  Dequantize(out, outputs.front().ElementNum(), out_scale, out_zp, output_dequant.data());
  ASSERT_EQ(0, CompareOutputData(output_dequant.data(), expect.data(), expect.size(), 0.01));
}
}  // namespace mindspore::lite::dsp::test
