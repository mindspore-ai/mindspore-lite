...
OpParameter *CreateParameter(schema::ReduceMode mode, bool keep_dims) {
...
TEST_F(TestOpenCL_Reduce, Mean) {
  ...
  for (auto fp16_enable : {false, true}) {
    auto *param = CreateParameter(schema::ReduceMode_ReduceMean, false);
...
TEST_F(TestOpenCL_Reduce, Sum) {
  ...
  for (auto fp16_enable : {false, true}) {
    auto *param = CreateParameter(schema::ReduceMode_ReduceSum, false);
    TestMain({{input_shape, input_data, VAR, kNumberTypeFloat32},
              {{static_cast<int>(axis.size())}, axis.data(), CONST_TENSOR, kNumberTypeInt32}},
             {output_shape, output_data}, param, fp16_enable);
  }
...
TEST_F(TestOpenCL_Reduce, MeanWC) {
  ...
  for (auto fp16_enable : {false, true}) {
    auto *param = CreateParameter(schema::ReduceMode_ReduceMean, true);
    TestMain({{input_shape, input_data, VAR, kNumberTypeFloat32},
              {{static_cast<int>(axis.size())}, axis.data(), CONST_TENSOR, kNumberTypeInt32}},
             {output_shape, output_data}, param, fp16_enable);
  }
...
TEST_F(TestOpenCL_Reduce, SumWC) {
  ...
  for (auto fp16_enable : {false, true}) {
    auto *param = CreateParameter(schema::ReduceMode_ReduceSum, true);
    TestMain({{input_shape, input_data, VAR, kNumberTypeFloat32},
              {{static_cast<int>(axis.size())}, axis.data(), CONST_TENSOR, kNumberTypeInt32}},
             {output_shape, output_data}, param, fp16_enable);
  }
