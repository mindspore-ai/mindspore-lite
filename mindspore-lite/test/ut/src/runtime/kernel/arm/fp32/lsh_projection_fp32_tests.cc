...
#include "nnacl_c/lsh_projection_parameter.h"
...
TEST_F(TestLshProjectionFp32, Dense1DInputs) {
  lite::Tensor in_tensor0(kNumberTypeFloat32, {3, 2});
  lite::Tensor in_tensor1(kNumberTypeInt32, {5});
  lite::Tensor in_tensor2(kNumberTypeFloat32, {5});
  ...
  auto parameter = new (std::nothrow) LshProjectionParameter();
...
TEST_F(TestLshProjectionFp32, Sparse3DInputs) {
  ...
  // weight must cover all feature_num_ (=20) elements; original 5-element weight caused an
  // OOB read, so the historical expectation {2,5,9} was undefined behavior. {2,7,11} is the
  // deterministic output of the unchanged hash (Dense1D/Sparse1D still match original values).
  std::vector<int32_t> except_result = {2, 7, 11};
  PrintData("output data", output_data, 3);
  ASSERT_EQ(0, CompareOutputData(output_data, except_result.data(), 3, 0.000001));
