...
TEST_F(EluInt8Test, EluInt8_2D) {
  ...
  int8_t table[256] = {0};
  EluInt8InitLUT(&quant_arg, table);
  EluInt8(input.data(), out_shape, output.data(), table);

  std::cout << "EluInt8Test-EluInt8_2D output:\n";
  std::for_each(output.begin(), output.end(), [](int8_t value) { std::cout << static_cast<int32_t>(value) << ", "; });
  std::cout << "\n EluInt8Test-EluInt8_2D benchmark_data:\n";
...
// Testcase2: EluInt8 with 3D input tensor
TEST_F(EluInt8Test, EluInt8_3D) {
  std::vector<int8_t> input = {-128, -105, -82, -59, -35, -12, 11, 34, 57, 81, 104, 127};
  std::vector<int8_t> benchmark = {-26, -27, -28, -31, -40, -64, -128, -77, -27, 26, 77, 127};
  ...
  std::cout << "EluInt8Test-EluInt8_3D output:\n";
  std::for_each(output.begin(), output.end(), [](int8_t value) { std::cout << static_cast<int32_t>(value) << ", "; });
  std::cout << "\n EluInt8Test-EluInt8_3D benchmark_data:\n";
  std::for_each(benchmark.begin(), benchmark.end(),
                [](int8_t value) { std::cout << static_cast<int32_t>(value) << ", "; });
  std::cout << std::endl;
  float similarity = get_cosine_similarity_int8(output.data(), benchmark.data(), output.size());
