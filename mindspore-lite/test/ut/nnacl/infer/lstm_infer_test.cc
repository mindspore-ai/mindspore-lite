...
TEST_F(LstmInferTest, LstmInferTest0) {
  ...
  // CheckInputShapeValid requires every input to be rank 3
  inputs[2] = new TensorC();
  inputs[2]->shape_size_ = 3;
  inputs[2]->shape_[0] = 1;
  inputs[2]->shape_[1] = hidden_size * 4;
  inputs[2]->shape_[2] = hidden_size;
  // the bias input must be rank 2
  inputs[3] = new TensorC();
  inputs[3]->shape_size_ = 2;
  inputs[3]->shape_[0] = 1;
  inputs[3]->shape_[1] = hidden_size * 8;
  inputs[4] = new TensorC();
  inputs[4]->shape_size_ = 3;
  inputs[4]->shape_[0] = 1;
  inputs[4]->shape_[1] = batch;
  inputs[4]->shape_[2] = hidden_size;
  inputs[5] = new TensorC();
  inputs[5]->shape_size_ = 3;
  inputs[5]->shape_[0] = 1;
  inputs[5]->shape_[1] = batch;
  inputs[5]->shape_[2] = hidden_size;
